...
\[
	V_r : A[x] \to R \hspace{1cm} V_r\big[f(x)\big] = f(r), \ \forall f(x) \in A[x]
\] 
...
\begin{proof}
	...
	\[
	\begin{split}
		&V_r \big[f(x) g(x)\big]= f(r) g(r) = V_r\big[f(x)\big]V_r\big[g(x)\big]\\
		&V_r \big[f(x) + g(x)\big] = f(r) + g(r) = V_r\big[f(x)\big] + V_r\big[g(x)\big]\\
		&V_r\big[u(x)\big] = V_r\big[1_A\big] = 1_A
	\end{split}
	\] 
\end{proof}
