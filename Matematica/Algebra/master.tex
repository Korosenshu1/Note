...
\end{prop}
\begin{osservazione}
	Ponendo l'$n$-agono regolare nel piano $\mathbb{R}^2$ gli elementi di $D_n$, si possono mettere in relazione con $\mathrm{GL} _2(\mathbb{R})$, visto che si possono vedere come applicazioni lineari da $\mathbb{R}^2$ in se stesso, cio\`e possono essere rappresentate tramite matrici\footnote{Le riflessioni cos\`i definite devono essere rispetto ad un asse opportuno, cio\`e che sia un asse di simmetria per l'$n$-agono in questione.}:
\begin{equation}
...
\begin{prop}
	Si ha $H=(H \cap C_n) \bigsqcup (\tau H \cap C_n) $ ed esiste una mappa biettiva tra $(H\cap C_n)$ e $(\tau H\cap C_n)$.
	\begin{proof}
		Si considera 
	\[
\begin{tikzcd}
...
La mappa $\varphi = \gamma\circ \det$ \`e un omomorfismo suriettivo proprio per come \`e definita $\gamma$ (cio\`e \`e un omomorfismo) e per il teorema di Binet per cui $\det (M^i_\rho M^j_\sigma ) = \det(M_\rho )^i \det (M_\sigma )^j = 1 ^i (-1)^j = 1 \iff j= 0$.
...
\end{osservazione}
\begin{teorema}
	[Teorema di scomposizione semidiretta]
