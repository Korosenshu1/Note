...
\begin{definizione}
	{Gruppo}{}
	Un \textit{gruppo} $G$ \`e un insieme su cui \`e definita una \textit{legge di composizione} $\circ : G \to G$ che soddisfa le seguenti condizioni per gli elementi di $G$:
	\begin{enumerate}[GR 1.]
		\item $(x\circ y)\circ z =  z\circ (y \circ z)$ (\textit{associativit\`a});
		\item $\exists e \in G : x\circ e = e\circ x = x$ (elemento neutro);
		\item $\forall x \in G, \ \exists y \in G$ tale che $x\circ y = y\circ x = e$ (elemento inverso).
	\end{enumerate}
\end{definizione}
\noindent Quando $\circ$ \`e la moltiplicazione, $G $ si dice \textbf{gruppo moltiplicativo}; quando $\circ $ \`e l'addizione, $G $ si dice \textbf{gruppo additivo}.
\begin{definizione}
	{Gruppo commutativo}{}
...
\begin{definizione}
	...
	\begin{itemize}
		\item  $e \in H$;
		\item $\forall x,y \in H, \ x\circ y \in H$;
		\item $\forall x \in H, \ x^{-1}  \in H$.
	\end{itemize}
\end{definizione}
\begin{definizione}
	{Generazione di un sottogruppo}{}
	Sia $S = \left\{ x_1,\ldots,x_n \right\} \subset  G$ un sottoinsieme di un gruppo $G$; l'insieme $H:= \left\{ x \in G : x = x_1 \circ \ldots\circ x_n  \right\}\cup \left\{ x^{-1} \in G : x \in S \right\} \cup \left\{ e \in G \right\}  $ \`e un sottogruppo di $G$ ed \`e detto \textit{generato} da $S$, dove gli elementi di $S$ sono detti i \textit{generatori di} $H$.

	In questo caso, si scriver\`a che $H = \langle S \rangle\equiv \langle x_1,\ldots,x_n \rangle$.
\end{definizione}
\noindent Si nota che $\left\{ 1 \right\} $ \`e un generatore per il gruppo additivo degli interi, visto che ogni $z \in \mathbb{Z} \setminus\left\{ 0 \right\} $ si pu\`o scrivere come $1 + 1+ \ldots +1$, o $-1 - 1-\ldots-1$, mentre l'elemento neutro ne fa parte per definizione.
...
\begin{equation}
	x^ n \equiv \underbracket{x \circ x \circ \ldots \circ x}_{n \text{ volte}}
\end{equation}
