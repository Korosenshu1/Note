...
\titleformat{\subsubsection}[block]{\centering\fontsize{13}{20}\bfseries}{\color{nred}\normalfont\S\thesubsubsection}{.5em}{}
...
Perch\'e questo sia verificato, si assume che:
\begin{itemize}
	\item $h \in C^\infty([a,b])$;
	\item $h'(t) \neq 0, \ \forall t \in [a,b]$;
	\item $h(a) = c$ e $h(b) = d$ (o viceversa, per avere un verso di percorrenza opposto).
\end{itemize}
In questo modo, $h$ \`e un diffeomorfismo $C^\infty$ su $[a,b]$ e, quindi, $\exists h^{-1}$ di classe $C^\infty$ tale che $\beta = \alpha \circ h^{-1}$ sia liscia e sia verificata la relazione $\operatorname{Tr} \alpha =\operatorname{Tr} \beta $.
...
\[
\alpha '''= T'' s'^3 + 3T's's'' + Ts'''\hspace{.2cm}\text{ con } \hspace{.4cm}T'' = k' N + k N'
\] 
...
\begin{boxenv}[]
\[
\tau = \frac{\alpha ' \times \alpha ''}{\left\lVert \alpha ' \times \alpha '' \right\rVert ^2 } \cdot \alpha '''
\] 
\end{boxenv}
\begin{osservazione}
...
\] 
Facendo uso del fatto che le derivate parziali miste sono uguali, essendo $\underline{x}$ in $C^\infty$, e unendo le due relazioni ottenute, si ha: 
\[
S_p(\underline{x}_u(p)) \cdot \underline{x}_v(p) = \underline{x}_u(p) \cdot S_p(\underline{x}_v(p))
\] 
\end{proof}
...
\begin{itemize}
	\item gli elicoidi ($K<0$) non sono localmente isometrici n\'e alle sfere, n\'e al piano;
...
Visto che sia $\alpha '(s)$ che $\underline{n}$ sono in $T_{\alpha (s) } S^2_a$\footnote{Il primo perch\'e la curva \`e sulla superficie, il secondo perch\'e \`e ortogonale $\pi$, il quale non coincide con $T_{\alpha (s)} S^2_a$.}, si ha che $\alpha ''(s) \in \left(T_{\alpha (s)} S^2_a\right) ^\perp$.
...
\begin{array}
	{c c c c c}
	U'_p &\longrightarrow&U_p &\longrightarrow & \Sigma\\
...
\begin{esercizio}
	...
	\begin{enumerate}[(1).]
		...
		\item Verificare il teorema di Gauss-Bonnet per la regione $R = \left\{ 0 \le z \le 1 \right\} \cap \Sigma $.
...
\[
	\begin{split}
		\int_{\partial R}  k_g ds &= \int_{\beta _e} k_g ds - \int_{\beta _1} k_g ds\\
...
\begin{proof}
	...
	\[
	\begin{split}
		&\mathrm{Id} _{\mathbb{R}^k} = d (\iota_U)_x = d(g \circ f)_x = (dg)_{f(x)} \circ df_x\\
		& \mathrm{Id} _{\mathbb{R}^\ell } = d(\iota_V)_{f(x)} = d(f\circ g)_{f(x)} = (df)_x\circ dg_{f(x)} 
	\end{split}
	\] 
	per cui $df_x : \mathbb{R}^{k} \to \mathbb{R}^\ell $ \`e iniettiva e suriettiva, quindi $df_x$ \`e un isomorfismo e $k=\ell $.
\end{proof}
\noindent Ora si pu\`o estendere la definizione di spazio tangente a varit\`a generiche.
\begin{definizione}
	[Spazio tangente]
	Sia $M \subseteq \mathbb{R}^k$ una variet\`a differenziabile di dimensione $\dim M = m$.
	Allora, dato $x \in M$, si ha una parametrizzazione locale $g : U \subseteq \mathbb{R}^m \stackrel{\cong}{\longrightarrow} W \cap M \subseteq \mathbb{R}^k$ tale che $x = g(u) \in W \cap M$ e si definisce
	\[
...
\end{prop}
\begin{proof}
	Visto che $\dim T_x M = \dim dg_u(\mathbb{R}^m)$, allora $\dim T_xM \le m$; si mostra che vale l'uguaglianza, facendo vedere che $dg_u$ \`e un isomorfismo. 
	Sia $g : U \stackrel{\cong}{\longrightarrow} W \cap M$ una parametrizzazione locale attorno a $x \in M$, con $g(u) = x$; visto che $g ^{-1}$ \`e $C^\infty$, allora si trovano $ W' \subseteq W$ aperto, con $x \in W'$, e $F:W'\to \mathbb{R}^m$ $C^\infty$ tali che $F|_{W'\cap M} = g^{-1}|_{W'\cap M} $, per cui:
	\[
...
\end{tikzcd}
\] 
Allora $dg_u$ \`e iniettiva perch\'e $dF_x\circ dg_u$ lo \`e (essendo pari a $\operatorname{Id} _{\mathbb{R}^m} $), quindi $\dim T_x M = m$.
\end{proof}
\begin{definizione}
...
Ma quindi
\[
f |_{g(U')}  = h|_{V'} \circ (h^{-1}\circ f\circ g)|_{U'} \circ g^{-1}|_{g(U')} 
\] 
...
\item Ora si assume $M$ compatta. 
	...
	\] 
	per cui $f^{-1}(y) \cap U = \left\{ x \right\} $, essendo $f$ iniettiva in $U$ (motivo per cui ci pu\`o essere un solo punto di $U$ mappato in $y$, che \`e proprio $x$ per costruzione). 
	Questo permette di concludere che ogni punto di $f^{-1}(y)$ \`e isolato e che $\forall p \in f^{-1}(y)$, si trova un certo aperto $U_p$ tale che $f^{-1}(y) \cap U_p = \left\{ p \right\} $.
	
	Per concludere, si nota che $\left\{ U_p \right\} _{p \in f^{-1}(y)} $ forma un ricoprimento di $f^{-1}(y)$; essendo quest'ultimo compatto, ammette un sottoricoprimento finito, dove ogni insieme di tale sottoricoprimento contiene un solo punto di $f^{-1}(y)$, e quindi si deduce che $\lvert f^{-1}(y) \rvert < + \infty$.
	\end{enumerate}
\end{proof}
\begin{lemma}
...
\begin{lemma}\label{lemret}
	...
	Allora
	\[
	\sum_{i}^{}\mathrm{vol} (B_i) \ge \mathrm{vol} (R)
	\] 
...
\begin{proof}
	...
	\] 
	Si fa vedere che, data $(g,Z\cap N)$ una carta su $N$, l'insieme $g(F(C) \cap Z)$ ha misura nulla, con $C = \mathrm{Crit} (F)$ insieme dei punti critici di $F$ (quindi $F(C)$ \`e l'insieme dei valori critici di $F$).
...
\end{corollario}
\begin{proof}
	Per il teorema di Sard, l'insieme $g\big(Z\cap F(C)\big)$ ha misura nulla; allora, se per assurdo $Z\cap N$ non contenesse punti di $N\setminus F(C)$, allora $Z\cap N = Z \cap F(C)$, quindi l'aperto $g(Z\cap N) = g\big(Z\cap F(C)\big)$ conterrebbe la chiusura di un rettangolo di volume positivo.
	Per\`o, per il lemma \ref{lemret}, l'insieme $g\big(Z\cap F(C)\big)$ non avrebbe misura nulla, da cui l'assurdo. 	
...
\end{prop}
\begin{proof}
	Siano $n = \dim N$ e $n' = \dim N'$. 
	Dato un punto $(y,y') \in N \times N'$, si considerano le carte locali $(f,W\cap N)$ e $(f' , W' \cap N')$ definite attorno a $y\in N$ e $y' \in N'$ rispettivamente.
	Allora si definisce la mappa
	\[
	F: 
	\begin{array}
	{c c c}
	(W\cap N) \times (W'\cap N') & \longrightarrow & \mathbb{R}^n \times \mathbb{R}^{n'} \\
	(p,p')&\longmapsto&\big(f(p) , f'(p')\big)
	\end{array}
	\] 
	che, per costruzione, \`e una carta locale di $N \times N'$ attorno a $(y,y')$, da cui si conclude che 
	\[
	\dim N\times N' = \dim N + \dim N' = n +n'
	\] 
	Per lo spazio tangente, si fa vedere che $ d\iota _{y'} (T_yN) \oplus d\iota_y(T_{y'} N') \subseteq T_{(y,y')} (N \times N')$ e che hanno stessa dimensione.
	\begin{itemize}
		\item L'inclusione deriva dal fatto che se $(t,t')$ \`e nella somma diretta, allora $(t,t') = (t,0) + (0,t')$, con $t \in T_y N$ e $t' \in T_{y'} N'$, pertanto $(t,t') \in T_{(y,y') } (N\times N')$.
			Questo deriva dal fatto che l'immagine di $d \iota_{y'} $, per esempio, \`e proprio $T_y N \times \left\{ 0 \right\} $.
		\item Per la dimensione, si osserva che, essendo $d\iota _{y'} (T_yN)\cap d\iota_y(T_{y'} N')=\left\{ (0,0) \right\} $, allora la dimensione della somma diretta \`e la somma delle dimensioni degli spazi tangente $T_y N$ e $T_{y'} N'$.
			Questa, per\`o, coincide esattamente con la dimensione del prodotto diretto degli spazi per quanto detto nella prima parte della dimostrazione; infatti, date due parametrizzazioni locali $g: U \to W\cap N$ e $g':U'\to W'\cap N'$ attorno a $y \in N$ e $y' \in N'$ rispettivamente, allora si definisce la mappa 
			\[
G :
\begin{array}{c c c}
	U \times U' &\longrightarrow& N \times N'\\
	(u,u') &\longmapsto & \big(g(u), g'(u')\big)
			\end{array}
			\] 
			con $dG_{(y,y')} : \mathbb{R}^n \times \mathbb{R}^{n'}\to T_{(y,y')} (N \times  N') $.
			Quindi, la sua immagine ha dimensione $n + n'$, ossia la somma delle dimensioni di $T_y N $ e $T_{y'} N'$.
	\end{itemize}
\end{proof}
\begin{teorema}
...
\begin{proof}
	...
	Sia, allora, $x \in f^{-1}(y)$, per cui $df_x : T_x M \to T_y N$ \`e suriettiva, visto che $y$ \`e regolare; se $K = \operatorname{Ker} df_x \subseteq T_x M $, allora $\dim K = m-n$. 
Si vorrebbe trovare una mappa $F:M\to N\times \mathbb{R}^{m-n} $.
A tal proposito, si considera l'applicazione lineare $L :\mathbb{R}^k \to \mathbb{R}^{m-n} $, dove $M \subseteq \mathbb{R}^k$ (quindi $K \subseteq \mathbb{R}^k$) e tale che
\[
...
\end{array}
\] 
Allora $F$ \`e $C^\infty$ e $dF_x(v) = \big(df_x(v),L(v)\big)$; inoltre, se $v \in T_xM$ \`e tale che $dF_x(v) = (0,0)$, allora $v=0$ perch\'e \`e l'unico che verifica $df_x(v) = L(v) = 0$, quindi $x \in M$ \`e un punto regolare per $F$.
Pertanto, \`e possibile applicare \ref{vrab}a, secondo cui $\exists U \subseteq M$ intorno di $x$ tale per cui $F|_U :U \stackrel{\cong}{\longrightarrow} V$, con $V$ intorno di $F(x) = \big(y,L(x)\big)$.
La restrizione, allora, mappa $f^{-1}(y) \cap U$ in $V\cap (\left\{ y \right\}  \times \mathbb{R}^{m-n}) $, dove $V \cap \left(\left\{ y \right\}  \times \mathbb{R}^{m-n} \right) $ \`e diffeomorfo a un aperto di $\mathbb{R}^{m-n} $ perch\'e $V$ \`e aperto e $\left\{ y \right\}  \times  \mathbb{R}^{m-n} $ \`e diffeomorfo a $\mathbb{R}^{m-n} $ tramite $\left\{ y \right\} \times \mathbb{R}^{m-n} \ni (y,c) \longmapsto c \in \mathbb{R}^{m-n} $.
Dunque, potendo ripetere il discorso $\forall x \in f^{-1}(y)$, si conclude che $f^{-1}(y)$ \`e una variet\`a di dimensione $m-n$.
\end{proof}
	\noindent Il teorema appena dimostrato permette di caratterizzare la sfera $S^{n-1}\subset \mathbb{R}^n$ come una variet\`a.
	Infatti, sia $f : \mathbb{R}^n \to \mathbb{R}$ data da $f(x) = x_1^2 + \ldots + x_n^2$; questa \`e liscia e il suo differenziale $df_x : \mathbb{R}^n \to \mathbb{R}$ \`e rappresentato da $(Jf)_x = (2x_1,\ldots,2x_n)$.
Ne segue che il suo unico punto critico \`e $(0,\ldots,0)$, per cui $1 \in \mathbb{R}$ \`e regolare, in quanto sono tutti valori regolari gli elementi di $\mathbb{R}\setminus f(C) = \mathbb{R}\setminus\left\{ 0 \right\} $, dove $C = \left\{ (0,\ldots,0) \right\} $ \`e l'insieme dei punti critici di $f$.
Il precedente teorema permette di concludere che 
\[
f^{-1}(1) = \left\{ x \in \mathbb{R}^n \mid  x_1^2 + \ldots + x_n^2 = 1 \right\} = : S^{n-1} \subseteq \mathbb{R}^n
\] 
\`e una variet\`a di dimensione $n-1$.
\begin{lemma}
	Siano $M,N$ due variet\`a di dimensioni $\dim M = m \ge \dim N = n$ e sia $f : M \to  N$ liscia, con $y \in N$ valore regolare; allora, indicando con $M':= f^{-1}(y)$, si ha $T_x M' = \operatorname{Ker} df_x, \ \forall x \in f^{-1}(y)$ e
	\[
		df_x |_{(T_x M')^\perp} : (T_xM')^\perp \stackrel{\cong}{\longrightarrow}T_y N
	\] 
\end{lemma}
\begin{proof}
	Si osserva preliminarmente che una mappa costante tra due variet\`a ha differenziale nullo perch\'e si estende localmente alla mappa costante.
	Ora, osservando che la composizione
	\[
		M ' = f^{-1}(y) \stackrel{\iota}{\longhookrightarrow } M \stackrel{f}{\longrightarrow} N
	\] 
	\`e una mappa costante, si ha che $df_x|_{T_x M'} = 0 \iff T_x M' \subseteq \operatorname{Ker} df_x, \ \forall x \in  f^{-1}(y)$.
	Usando, poi, che $\dim M' = \dim \operatorname{Ker} df_x = m-n$, si conclude che $T_x M ' = \operatorname{Ker} df_x$.
	Ma, allora, la mappa
	\[
	df_x |_{(T_x M')^\perp}  :(T_xM')^\perp \longrightarrow T_y N 
	\] 
	\`e iniettiva, quindi \`e un isomorfismo.
\end{proof}
\noindent Questo lemma, invece, permette di studiare gli spazi tangenti delle sfere.
Riprendendo la funzione 
\[
f: \mathbb{R}^n \to \mathbb{R} , \ f(x) = \sum_{i=1}^{n} x_i^2
\] 
con $S^{n-1} = f^{-1}(1)$ e $df_x$ dato da $(Jf)_x = (2x_1,\ldots,2x_n)$, si vede che:
\[
df_x(v)  =2 \sum_{i=1}^{n} x_i v_i = 2x \cdot v = 0 \iff v\perp x
\] 
Allora $T_x S^{n-1}=\operatorname{Ker} df_x = x ^\perp \subseteq \mathbb{R}^n$.

Un altro esempio di applicazione di quanto appena visto \`e riguardante il gruppo delle matrici ortogonali; infatti, si pu\`o dimostrare che questo \`e una variet\`a di dimensione $\frac{n(n-1)}{2}$.
Per vederlo, indicando con $M(n)$ lo spazio delle matrici reali $n\times n$, che \`e isomorfo a $\mathbb{R}^{n^2} $, si pu\`o definire
\[
f: 
\begin{array}
	{c c c}
	M(n) & \longrightarrow & S(n) \cong \mathbb{R}^{n(n+1) / 2} \\
	A & \longmapsto & A A^t
\end{array}
\] 
con $S(n)$ spazio delle matrici simmetriche.
Allora $O(n) = f^{-1}(I)$ \`e il gruppo delle matrici ortogonali.
Data $A \in f^{-1}(I)$, si ha
\[
	df_A(B) = \lim_{\varepsilon  \to 0} \frac{f(A+\varepsilon B) - f(A)}{\varepsilon }= \lim_{\varepsilon  \to 0} \frac{(A+\varepsilon B)(A+\varepsilon B)^t - A A^t}{\varepsilon }=AB^t + BA^t
\] 
Questa \`e suriettiva; data $C \in S(n)$ e $A \in O(n) = f^{-1}(I)$, si pu\`o trovare $B\in M(n)$ tale che 
\[
AB^t + BA^t = C
\] 
Scrivendo $C = \frac{1}{2}C + (\frac{1}{2}C)^t$, basta trovare $B$ tale che $BA ^t= \frac{1}{2}C$; per farlo, si prende $B := \frac{1}{2}CA$.
Questo permette di concludere che $df_A : T_AM(n) \to  T_IS(n)$ \`e suriettiva $\forall A \in f^{-1}(I)$ e, quindi, $O(n)$ \`e una variet\`a di dimensione $n^2 - \frac{n(n+1)}{2}=\frac{n(n-1)}{2}$.

\subsection{Variet\`a con bordo e teorema di Brouwer}
L'obiettivo \`e quello di arrivare a dimostrare il teorema di Brouwer, il quale afferma che una generica mappa continua 
\[
D^n := \left\{ x\in \mathbb{R}^n  \mid x_1^2 + \ldots + x_n^2 \le 1 \right\} \longrightarrow D^n
\] 
ha almeno un punto fisso.
Per fare ci\`o, si deve trovare un modo per trattare le variet\`a con bordo in modo da includere $D^n$.

Per introdurre le variet\`a con bordo, \`e necessario trovare un sostituto per gli spazi euclidei che possa contenere informazioni sul bordo.
A questo scopo, si introduce il \textit{semispazio euclideo} 
\[
H^m : = \left\{ x \in \mathbb{R}^m  \mid x_m \ge 0 \right\} 
\] 
per rimpiazzare $\mathbb{R}^m$ nella definizione di $m$-variet\`a. 
\begin{definizione}
	[Variet\`a con bordo]
	Dato $M \subseteq \mathbb{R}^k$, si dice che \`e una $m$-\textit{variet\`a con bordo} se ogni $x\in M$ ha un intorno diffeomorfo ad un aperto di $H^m$.
\end{definizione}
\noindent Come per le variet\`a senza bordo, i diffeomorfismi $g : U \subseteq H^m \to g(U) \subseteq M$ si dicono \textit{parametrizzazioni locali}, mentre le rispettive inverse si chiamano \textit{carte locali}.

Il bordo di $M$, indicato con $\partial M$, \`e l'insieme dei punti ottenuti dall'immagine di un punto di $\partial H^m := \left\{ x \in \mathbb{R}^m  \mid x_m = 0 \right\} $ tramite una qualche parametrizzazione locale. 

\subsubsection{Spazi tangenti e bordi}
Per introdurre gli spazi tangenti di variet\`a con bordo, si deve prima caratterizzare il differenziale.
Se $g : U \subseteq H^m \to \mathbb{R}^k$ \`e una mappa liscia, con $U$ aperto, allora $dg_u$ \`e il differenziale gi\`a utilizzato quando $u \in U \setminus\partial H^m$, in quanto $g$ \`e definita su un intorno di $x \in \mathbb{R}^m$; se, invece, $u \in \partial U = U \cap \partial H^m$, allora $dg_u$ \`e definito come $d\widetilde{g}_u$, dove $\widetilde{g}$ \`e un'estensione di $g$ ad un intorno di $u$ in $\mathbb{R}^m$.

\begin{prop}
	La definizione di $dg_u$ non dipende dalla scelta dell'estensione di $g$.
\end{prop}
\begin{proof}
	Siano $\widetilde{g}$ e $\widetilde{g}'$ due estensioni di $g$ a un intorno di $u$ in $\mathbb{R}^m$ e sia $\left\{ u_i \right\} \to u$ una successione tale che la coordinata $x_m$ di $u_i$ sia positiva; allora $d \widetilde{g}'_u = d\widetilde{g}_u, \ \forall i \gg 0$ perch\'e $\widetilde{g}$ e $\widetilde{g}'$ coincidono con $g$ su intorni di $u_i$.
	Ma questo implica che
	\[
	d \widetilde{g}'_u = \lim_{i \to +\infty} d\widetilde{g}'_{u_i}  = \lim_{i \to +\infty} d\widetilde{g}_{u_i} = d\widetilde{g}_u
	\] 
\end{proof}
\begin{prop}
	Sia $g : U \to  M$ una parametrizzazione locale per la $m$-variet\`a $M$; allora $\forall u \in U$, il differenziale $dg_u$ \`e iniettivo.
\end{prop}
\begin{proof}
	Nel caso di $u \in U \setminus\partial U$ \`e ovvio.
	Se, invece, $u \in \partial U$, con $x = g(u) \in M$, \`e possibile estendere l'inversa $g^{-1}$ attorno a $x$ alla funzione 
	\[
	F : W' \to \mathbb{R}^m \ C^\infty\ , \hspace{.3cm} F|_{W' \cap M} = g^{-1}
	\] 
	Pertanto si pu\`o costruire il seguente diagramma:
	\[
	\begin{tikzcd}
		 &  W ' & \\
		 \\
		U & & \mathbb{R}^m
		\arrow[from=3-1, to=1-2, "\displaystyle g"]
		\arrow[from=1-2, to=3-3, "\displaystyle F"]
		\arrow[from=3-1, to=3-3, "\displaystyle\iota",hook]
	\end{tikzcd}
	\] 
	Allora, se $\left\{ u_i \right\} _i \subseteq U \setminus \partial U$ \`e una successione convergente a $u$, si ha:
	\[
		\operatorname{Id} _{\mathbb{R}^m} = d\iota _{u_i} = dF_{g(u_i)} \circ dg_{u_i} \stackrel{u_i \to u}{\longrightarrow} \operatorname{Id} _{\mathbb{R}^m} = dF_x \circ dg_u
	\] 
	perci\`o $dg_u$ \`e iniettiva.
\end{proof}
\begin{definizione}
	Sia $M\subseteq \mathbb{R}^k$ una $m$-variet\`a con bordo e sia $x \in \partial M$, con $g : U \to M$ una parametrizzazione locale attorno a $x=g(u)$ e $u\in \partial U$; allora si definisce 
	\[
	T_x M := dg_u(\mathbb{R}^m) \subseteq \mathbb{R}^k
	\] 
\end{definizione}
\begin{teorema}
	Sia $M$ una $m$-variet\`a con bordo; allora $\partial M$ \`e una $(m-1)$-variet\`a senza bordo.
\end{teorema}
\begin{proof}
	L'idea \`e che $\partial M \cong \mathbb{R}^{m-1} $.
	Per vederlo, si prende $x \in \partial M$ e $g : U \to M $ una parametrizzazione locale attorno a $x = g(u)$, dove $u \in \partial U := U \cap \partial H^m$.
	Evidentemente, $\partial U$ \`e un aperto di $\partial H^m$ con la topologia di sottospazio indotta in $\partial H^m$ da $H^m$, visto che $\partial U = U \cap H^m$, con $U$ aperto in $H^m$ e $\partial H^m \subset H^m$; inoltre, $\partial H^m \cong \mathbb{R}^{m-1} $ tramite la proiezione al quoziente
	\[
	\Pi : 
	\begin{array}
		{c c c}
		\partial H^m & \longrightarrow & \mathbb{R}^{m-1} \\
		(x_1,\ldots,x_{m-1} ,0) &\longmapsto&(x_1,\ldots,x_{m-1})
	\end{array}
	\] 
	da cui si ottiene che $\partial U$ \`e un aperto di $\mathbb{R}^{m-1}$.
	Si nota, dunque, che essendo
	\[
	g|_{\partial U}  : \partial U \longrightarrow g(\partial U)
	\] 
	un diffeomorfismo, allora se $g(\partial U) = g(U) \cap \partial M$, con $g(\partial U)$ intorno di $x$ in $\partial M$, si pu\`o concludere che $\partial M$ \`e una variet\`a di dimensione $m-1$.
	Per vederlo, si inizia col notare che $g(\partial U) \subseteq g(U) \cap \partial M$ perch\'e $\partial M$ contiene tutti i punti mappati da $\partial H^m$ in $M$ tramite $g$.
	Per mostrare l'inclusione inversa e, quindi, l'uguaglianza, si procede per assurdo: sia $x \in g(U) \cap \partial M$ tale che $x \not \in g(\partial U)$, che equivale a richiedere $u \not\in \partial U$.
	Per definizione di $\partial M$, allora, esiste una parametrizzazione locale $h : V \to M$, con $h(v) = x$ e $v \in \partial V$, tramite cui si pu\`o definire la seguente funzione di transizione
	\[
	h^{-1}\circ g : g^{-1}\big(h(V)\big) \longrightarrow h^{-1}\big(g(V)\big)
	\] 
	che \`e un diffeomorfismo, con $d\left(h^{-1}\circ g\right) _u = dh^{-1}_v \circ dg_u$ isomorfismo lineare.
	\begin{center}
		\begin{tikzpicture}[scale=1.1,>=stealth]
% --- Left H^m ---
\draw[] (-4,0) -- (-.5,0);
...
% U region
\draw[] (-3.5,0) arc (180:0:1.25 and 1.05);
\node at (-3,0.5) {$U$};

% u point
\fill (-1.8,0.35) circle (1.5pt);
\node[left] at (-1.8,0.35) {$u$};

% g arrow
\draw[->] (-2,1.4) to[bend left=15] (0.2,2.3);
\node[above] at (-0.9,2.2) {$g$};

% --- Middle: M ---
\draw[] (0.8,2.3) ellipse (1.5 and 1.2);
\node at (0.8,3.8) {$g(U)$};

\draw[] (3.2,2.3) ellipse (1.5 and 1.2);
\node at (2.9,3.8) {$h(V)$};

% x point
\fill (2,2.4) circle (1.5pt);
\node[below] at (2,2.4) {$x$};

% transition map
\draw[->] (0.5,0.3) -- (3.5,0.3);
\node[below] at (2,0.3) {$h^{-1}\circ g$};
...
% V region
\draw[] (5,0) arc (180:0:1.25 and 1.05);
\node at (7,0.5) {$V$};

% v point
\fill (6.1,0) circle (1.5pt);
\node[below] at (6.1,0) {$v$};

% h arrow
\draw[->] (6.3,1.4) to[bend right=15] (3.8,2.3);
\node[above] at (4.9,2.2) {$h$};

\end{tikzpicture}
	\end{center}
	Usando il lemma della funzione inversa, visto che $h^{-1}\circ g$ \`e una mappa $C^\infty$ da un intorno di $u$ in $\mathbb{R}^m$ contenuto in $U$, si conclude che ha immagine contenente un intorno di $v$ in $\mathbb{R}^m$; questo, per\`o, \`e impossibile perch\'e l'immagine di $h^{-1}\circ g$ \`e contenuta in $V \subseteq H^m$ per costruzione.
\end{proof}
\subsubsection{Valori regolari}
\begin{lemma}\label{vbvr1}
	Siano $M$ una variet\`a senza bordo\footnote{Lo stesso vale per variet\`a con bordo, ma la dimostrazione andrebbe leggermente sistemata di conseguenza.} e $g : M \to \mathbb{R}$ una mappa liscia; se $0 \in \mathbb{R}$ \`e un valore regolare di $g$, allora $\left\{ x \in M  \mid g(x) \ge 0 \right\} $ \`e una variet\`a con bordo $g^{-1}(0)$.
\end{lemma}
\begin{proof}
	L'insieme $\left\{ x \in M  \mid g(x) > 0 \right\} \subset M$ \`e aperto, quindi \`e una variet\`a con carte locali date dalle intersezioni con le carte di $M$.
	Se $m = \dim M$ e $M \subset \mathbb{R}^k$, visto che $0$ \`e un valore regolare per $g$, si ha $\dim  \operatorname{Ker} dg_x = m -1$.
	Ora si definisce l'applicazione lineare $L : \mathbb{R}^k \to  \mathbb{R}^{m-1} $ tale che $L |_{\operatorname{Ker} dg_x} $ sia un isomorfismo lineare e si introduce la mappa
	\[
	G: 
	\begin{array}
		{c c c}
		M & \longrightarrow & \mathbb{R}^{m-1} \times \mathbb{R}\\
		x & \longmapsto & \big(L(x), g(x)\big)
	\end{array}
	\] 
	Per costruzione, $dG_x$ \`e un isomorfismo, quindi $G$ induce un diffeomorfismo tra un intorno $U\subset M$ di $x$ e un intorno $V \subset \mathbb{R}^{m-1} \times \mathbb{R}$ di $\big(L(x), 0\big)$.
	Inoltre, notando che
	\[
	G^{-1}\Big(\left(\mathbb{R}^{m-1} \times \mathbb{R}^{\ge 0} \right) \cap V = H^m \cap V\Big)= U \cap \left\{x \in M \mid  g(x)\ge 0 \right\} 
	\] 
	si conclude che $G$ \`e una carta locale attorno a $x$.
\end{proof}
\noindent Questo lemma si pu\`o utilizzare per concludere che $D^m$ \`e una variet\`a con bordo; infatti, ricordando che
\[
D^m : = \left\{ x \in \mathbb{R}^m  \Bigg\lvert \sum_{i=1}^{m} x_i^2 \le 1 \right\}  \subseteq \mathbb{R}^m
\] 
si pu\`o definire
\[
g : 
\begin{array}
	{c c c}
	\mathbb{R}^m & \longrightarrow & \mathbb{R}\\
	x & \longmapsto & 1 - \sum_{i=1}^{m} x_i^2
\end{array}
\] 
che \`e una mappa $C^\infty$, per ottenere che $D^m = \left\{ x \in \mathbb{R}^m  \mid g(x) \ge 0 \right\} $.
Si vede che $0 \in \mathbb{R}$ \`e un valore regolare per $g$; infatti
\[
dg_x = -\big(2x_1,\ldots,2x_m\big) = 0 \iff x_1=\ldots=x_m =0 
\] 
ma $(0,\ldots,0)$ non risolve $g=0$. 
Pertanto, sfruttando il lemma precedente, si conclude che $D^m$ \`e una variet\`a, il cui bordo \`e dato da $g^{-1}(0) = S^{m-1}$.
\begin{lemma}\label{vbvr2}
	Siano $M,N$ due variet\`a con $m = \dim M > \dim N = n$ e sia $f : M \to N$ una mappa $C^\infty$.
	Se $M$ ha bordo e $y \in N$ \`e un valore regolare sia per $f$ che per $f|_{\partial M} $, con $f^{-1}(y) \neq \varnothing$, allora $f^{-1}(y)$ \`e una variet\`a di dimensione $m-n$, il cui bordo \`e dato da $\partial f^{-1}(y) = f^{-1}(y)\cap \partial M$.
\end{lemma}
\begin{proof}
	La dimostrazione si basa su verifiche locali, pertanto \`e possibile trattare il caso specifico per $M = H^m$ e $N = \mathbb{R}^n$ senza perdere generalit\`a.
	Sia, dunque, $f: H^m \to \mathbb{R}^n$ una mappa liscia con $y \in \mathbb{R}^n$ valore regolare per $f$ e $f|_{\partial H^m} $.
	Sia, inoltre, $\overline{x}\in f^{-1}(y)$; se $\overline{x}\in \operatorname{Int} \left(H^m\right) $, allora si pu\`o usare quanto visto per le variet\`a senza bordo per concludere che $f^{-1}(y)$ \`e una variet\`a di dimensione $m-n$ in un intorno di $\overline{x}$.

	Nel caso in cui, invece, $\overline{x}\in \partial H^m$, si definisce $F:U \subseteq \mathbb{R}^m \to \mathbb{R}^n$ come l'estensione $C^\infty$ di $f$ a $\mathbb{R}^m$, cio\`e \`e tale che $F|_{U\cap H^m} = f|_{U\cap H^m} $.
	Visto che $y$ \`e regolare per $f$, allora $df_{\overline{x}} = dF_{\overline{x}} $ \`e suriettiva e, quindi (a meno di restringere $U$) si pu\`o assumere, senza perdita di generalit\`a, che $F$ non abbia punti critici in $U$.
	Ne segue che $F^{-1}(y) \subset U$ \`e una variet\`a di dimensione $m-n$ per quanto gi\`a visto.
	...
	\end{array}
	\] 
	e se ne considera la restrizione a $F^{-1}(y)$ (d'ora in avanti, si assumer\`a che avr\`a dominio su $F^{-1}(y)$); allora
	\[
	f^{-1}(y) \cap U = \left\{ x \in F^{-1}(y)  \mid \pi(x) \ge 0 \right\} 
	\] 
	Facendo uso del lemma precedente (\ref{vbvr1}), sar\`a sufficiente dimostrare che $0 \in \mathbb{R}$ \`e un valore regolare per $\pi$ per poter concludere.
	Si nota che
	\[
	\left(\pi|_{F^{-1}(y)} \right) ^{-1}(0) = f^{-1}(y) \cap \partial H^m
	\] 
	Se $\overline{x}\in f^{-1}(y) \cap \partial H^m$, allora
	\[
		T_{\overline{x}}F^{-1}(y) = \operatorname{Ker} dF_{\overline{x}} = \operatorname{Ker} df_{\overline{x}} 
	\] 
	ha dimensione $m-n$ perch\'e $\dim F^{-1}(y) = m-n$.
...
\begin{tikzcd}
	...
	\arrow[from=3-1, to=1-3, "\displaystyle f|_{\partial H^m}"' ]
\end{tikzcd}
\] 
Si vede che $y$ valore regolare per $f|_{\partial H^m}$ implica che
\[
	K:= \operatorname{Ker} \Big(d\left(f|_{\partial H^m} \right) _{\overline{x}} \Big) = \operatorname{Ker} \Big(df_{\overline{x}}\big|_{T_{\overline{x}}\partial H^m} \Big)
\] 
ha dimensione $\dim K = m-n-1$.
Visto che $T_{\overline{x}} \partial H^m = \operatorname{Ker} d\pi_{\overline{x}} $, allora $K = \operatorname{Ker} df_{\overline{x}} \cap \operatorname{Ker} d\pi_{\overline{x}} $, quindi $m-n-1 = \dim K < \dim \operatorname{Ker} df_{\overline{x}} = m -n$, da cui
\[
\operatorname{Ker} df_{\overline{x}} = \operatorname{Ker} dF_{\overline{x}} \not \subseteq \operatorname{Ker} d\pi_{\overline{x}} 
\] 
che equivale a dire che
\[
d\pi_{\overline{x}} \big|_{T_{\overline{x}} F^{-1}(y)} \neq 0
\] 
Quindi, $0$ \`e un valore regolare per $\pi$ e, allora, $f^{-1}(y) \cap U$ \`e una variet\`a con bordo di dimensione $m-n$ e $\partial \big(f^{-1}(y) \cap U\big) = f^{-1}(y) \cap \partial H^m$.
\end{proof}
\subsubsection{Teorema di Brouwer}
Per proseguire con la trattazione, si considerer\`a il seguente enunciato come \textit{fatto}.
\vspace{.3cm}

\noindent \textbf{Fatto.} Ogni variet\`a compatta di dimensione $1$ \`e diffeomorfa ad un'unione finita di copie di $S^{1}$ e di intervalli chiusi di $\mathbb{R}$.

\vspace{.3cm}
\begin{lemma}\label{lb1}
	Sia $M$ una variet\`a compatta con $\partial M \neq \varnothing$; allora non pu\`o esistere una mappa $f:M\to  \partial M\ C^\infty$ tale che $f |_{\partial M} = \operatorname{Id} _{\partial M} $.
\end{lemma}
\begin{proof}
	Si procede per assurdo, assumendo che tale mappa esista.
	Si sa che i valori regolari di $f$ in $\partial M$ sono densi; sia $y \in \partial M$ uno di questi.
	Visto che $f|_{\partial M} = \operatorname{Id} _{\partial M} $, allora $y$ \`e regolare per $f |_{\partial M} $ e, per il lemma precedente (\ref{vbvr2}), $f^{-1}(y)$ \`e una variet\`a di dimensione $1$ con 
	\[
	\partial f^{-1}(y) = f^{-1}(y) \cap \partial M = \left\{ y \right\} 
	\] 
	dove la seconda uguaglianza deriva dal fatto che $f|_{\partial M} = Id_{\partial M} $.
	Per il \textit{fatto} appena enunciato, questo \`e impossibile perch\'e i bordi delle variet\`a di dimensione $1$ hanno sempre un numero pari di punti.
\end{proof}
\begin{osservazione}
Come diretta conseguenza di questo lemma, si ha che $\operatorname{Id} _{S^{m-1} } : S^{m-1}\to S^{m-1} $ non si pu\`o estendere ad un'applicazione liscia $D^m\to S^{m-1} $.
\end{osservazione}
\begin{lemma}\label{lb2}
	Ogni mappa $C^\infty$ $g:D^{m} \to D^m$ ha almeno un punto fisso, cio\`e $\exists x \in D^m $ tale che $ g(x)  = x$.
\end{lemma}
\begin{proof}
	Si procede per assurdo.
	Sia, quindi, $g : D^m \to  D^m $ una mappa liscia senza punti fissi; allora la si pu\`o usare per costruire la mappa $C^\infty$
	\[
		f:
		\begin{array}
			{c c c}
			D^m&  \longrightarrow  &S^{m-1}\\
			x & \longmapsto& x + tu
		\end{array}\,  \hspace{.5cm} \text{ tale che } \hspace{.5cm} f|_{S^{m-1}}  = \operatorname{Id} _{S^{m-1}} 
	\] 
	con
	\[
	u : = \frac{x - g(x)}{\left\lVert x-g(x) \right\rVert }
	\] 
	dove si osserva che $\left\lVert x - g(x) \right\rVert \neq 0$ per ogni $x$ perch\'e $g$ non ha punti fissi per assunzione.
	Per ottenere $t$, si usa che\footnote{La condizione \`e assicurata dal fatto che $f$ deve mappare elementi di $D^m$ in $S^{m-1}$.}
	\[
	1 = (x + tu ) \cdot (x + tu ) = x \cdot  x + t^2 + 2t( x \cdot  u) \implies t = - x \cdot  u \pm \sqrt{(x\cdot u)^2 - x \cdot  x + 1} 
	\] 
	dove si sceglie la soluzione con il segno positivo davanti la radice.
	Perch\'e $t$ sia $C^\infty$, deve risultare $h(x) := (x\cdot u) ^2 - x \cdot x + 1 > 0$; visto che $x \in D^m$, si ha $h(x) \ge (x\cdot u)^2 \ge 0$ e $h(x) = (x\cdot u)^2\iff x \cdot  x = 1$.
	In questo caso, si ha:
	\[
	x \cdot  u = \frac{x \cdot x - x \cdot g(x)}{\left\lVert x - g(x) \right\rVert } = \frac{1 - x \cdot g(x)}{\left\lVert x - g(x) \right\rVert }>0
	\] 
	visto che $x \neq g(x)$.
	Inoltre, $x \cdot  x = 1 \implies t = 0 \implies f(x) = x$, da cui si ottiene che $f $ \`e $C^\infty$ e $f|_{S^{m-1}} = \operatorname{Id} _{S^{m-1}} $, che \`e in contraddizione con il precedente lemma (\ref{lb1}).
\end{proof}
\begin{teorema}
	[Teorema di Brouwer]
	Ogni mappa continua $G: D^m \to D^m$ ha almeno un punto fisso.
\end{teorema}
\begin{proof}
	Visto che $D^m$ \`e compatto, per il teorema di approssimazione di Weierstrass, si trova una funzione polinomiale $P_1:\mathbb{R}^m \to \mathbb{R}$ tale che $\left\lVert P_1(x) - G(x) \right\rVert <\varepsilon , \ \forall x \in D^m$.
	Si nota anche che, per ogni $x \in  D^m$, vale
	\[
	\left\lVert P_1(x) \right\rVert \le \left\lVert P_1(x) - G(x) \right\rVert + \left\lVert G(x) \right\rVert < \varepsilon  + 1
	\] 
	Allora, si definisce 
	\[
		P:
	\begin{array}
		{c c c}
		D^m & \longrightarrow & D^m\\
		x&\longmapsto &\displaystyle \frac{P_1(x)}{\varepsilon +1}
	\end{array}
	\] 
	Assumendo per assurdo che $G(x) - x \neq 0, \ \forall x \in D^m$, si ha
	\[
	\mu := \min_{x \in D^m} \left\lVert G(x) - x  \right\rVert > 0
	\] 
	Prendendo $\varepsilon >0 $ tale che $\mu > 2 \varepsilon /(1+\varepsilon )$, si ha
	\[
		\begin{split}
			\left\lVert P(x) - G(x) \right\rVert &= \left\lVert \frac{P_1(x)}{\varepsilon +1} - G(x) \right\rVert = \frac{1}{\varepsilon +1} \left\lVert P_1(x) - (\varepsilon +1) G(x) \right\rVert \\
							     &\le \frac{1}{\varepsilon +1}\Big[\left\lVert P_1(x) - G(x) \right\rVert + \varepsilon \left\lVert G(x) \right\rVert \Big] \\
							     & <\frac{2 \varepsilon }{\varepsilon +1}<\mu 
		\end{split}
	\] 
	per cui $P(x) \neq x , \ \forall x \in D^m$.
	Ma visto che $P:D^m \to D^m $ \`e $C^\infty$, allora il lemma precedente (\ref{lb2}) \`e contraddetto.
\end{proof}
\subsection{Teoria del grado mod 2}
Data $f : M \to  N$ mappa $C^\infty$ tra variet\`a, con $M$ compatta, $\partial M = \varnothing$, $N$ connessa e $\dim M = \dim N$, si vuole dimostrare che, dati $y,y' \in N$ valori regolari di $f$, vale
\[
\lvert f^{-1}(y) \rvert \equiv \lvert f^{-1}(y') \rvert \pmod{2} 
\] 
\begin{esercizio}
	{\color{nred}(Da svolgere\ldots)} Sia $M$ una variet\`a; allora $M \times [0,1]$ \`e una variet\`a con bordo $\partial M = M \times \left\{ 0 \right\} \cup M \times \left\{ 1 \right\} $.
\end{esercizio}
\begin{svolgimento}
	Suggerimento: usare che $f : M \times \mathbb{R} \to \mathbb{R}, \ f(x,t) = 1-t^2 $ \`e $C^\infty$.
\end{svolgimento}
\begin{definizione}
	[Omotopia $C^\infty$]
	Siano $M,N$ due variet\`a e $f, g : M \to  N $ due mappe $C^\infty$.
	Si dice che queste sono $C^\infty$\textit{-omotope} se esiste un'applicazione $C^\infty$ $F:M\times [0,1] \to N	$ tale che $F(x,0) = f(x), \ F(x,1) = g(x), \ \forall x \in M$.
\end{definizione}
\begin{lemma}[Lemma di omotopia]\label{lom}
	Siano $M,N$ due variet\`a, con $M$ compatta e $\dim M = \dim N$.
	Siano, poi, $f,g : M \to N$ due mappe $C^\infty$-omotope e sia $y \in N$ un loro valore regolare; allora $\lvert f^{-1}(y) \rvert \equiv \lvert g^{-1}(y) \rvert \pmod{2} $.
\end{lemma}
\begin{proof}
	Sia $F:M \times [0,1]\to N$ l'omotopia $C^\infty$.
	Siano, poi, $V_1\subseteq N$ un intorno di $y$ tale che $\lvert f^{-1}(y') \rvert = \lvert f^{-1}(y) \rvert ,\ \forall y' \in V_1$ e $V_2\subseteq N$ un intorno analogo che soddisfa la stessa condizione per $g$, cio\`e $\lvert g^{-1}(y') \rvert = \lvert g^{-1}(y) \rvert , \ \forall y ' \in V_2$.
	\`E sufficiente mostrare che la tesi vale in un punto di $V_1\cap V_2$.
	Sia, quindi, $y \in V_1\cap V_2$ un valore regolare di $F$; allora $F^{-1}(y)$ \`e una variet\`a di dimensione $1$, con bordo
	\[
		\begin{split}
			\partial F^{-1}(y) &= F^{-1}(y) \cap \partial \big(M\times [0,1]\big) = F^{-1}(y) \cap \big(M \times \left\{ 0 \right\} \cup M \times \left\{ 1 \right\} \big)\\
					   & = f^{-1}(y) \cup g^{-1}(y)
		\end{split}
	\] 
	Ma questo implica che
	\[
	0 \equiv \lvert \partial F^{-1}(y) \rvert = \lvert f^{-1}(y) \rvert + \lvert g^{-1}(y) \rvert \pmod{2} 
	\] 
	da cui la tesi.
\end{proof}
\begin{definizione}
	[Isotopia]
	Sia $F : M \times [0,1]\to N$ un'omotopia $C^\infty$ tra diffeomorfismi $f,g : M \to N$.
	Si dice che $F$ \`e un'\textit{isotopia} se, $\forall t \in [0,1]$, la mappa 
	\[
		F_t : 
	\begin{array}
		{c c c}
...
\end{definizione}
\begin{lemma}
	[Lemma di omogeneit\`a]\label{log}
	Sia $N$ una variet\`a connessa e siano $y,z \in N$.
	Allora esiste un diffeomorfismo $h : N \to N$ tale che:
	\begin{enumerate}[(a).]
		\item $h(y) = z$;
		\item $h$ \`e isotopo a $\operatorname{Id} _N$ tramite un'isotopia a supporto compatto, cio\`e $\exists K \subset N$ compatto con $y,z \in K$ e $h_t|_{N\setminus K} = \operatorname{Id} _{N \setminus K} , \ \forall t \in [0,1]$.
	\end{enumerate}
\end{lemma}
\begin{teorema}
	Siano $M,N$ due variet\`a, con $\dim M = \dim N$, $M$ compatta e $N$ connessa.
	Se $y,z \in N$ sono valori regolari di $f : M \to  N$ mappa $C^\infty$, allora:
	\begin{enumerate}[(a).]
		\item $\lvert f^{-1}(y) \rvert \equiv \lvert f^{-1}(z) \rvert \pmod{2} $ e $\deg_2 f := \lvert f^{-1}(y) \rvert \pmod{2} $ \`e il \textit{grado di $f$ modulo 2};
		\item data $g : M \to N$ $C^\infty$ omotopa a $f$, allora $\deg_2 g = \deg _2 f$.
	\end{enumerate}
\end{teorema}
\begin{proof}
Si divide la dimostrazione nei due punti.
\begin{enumerate}[(a).]
	\item Per il lemma di omogeneit\`a (\ref{log}), esiste $h : N \to N$ diffeomorfismo isotopo a $\operatorname{Id} _N$ con $h(y) = z$; se $y$ \`e un valore regolare per $f$, dunque, si ha che $z$ \`e un valore regolare per $h\circ g$.
Definendo 
\[
\begin{array}
	{c c c c c}
	M \times [0,1] & \xrightarrow{\quad \displaystyle f \times \operatorname{Id} \quad} & N \times [0,1] & \xrightarrow{\quad \displaystyle H \quad} &N\\
	(x,t) & \xmapsto{\qquad\qquad} & \big(f(x) , t \big) & \xmapsto{\qquad\quad} & H\big(f(x), t\big)
\end{array}
\] 
dove $(x,t)\longmapsto \big(f(x),t\big)$ \`e un'omotopia $C^\infty$ da $h\circ f$ a $f$ e $H$ \`e un'isotopia da $h$ a $\operatorname{Id} _n$, si pu\`o usare il lemma di omotopia (\ref{lom}) per concludere che
\[
	\lvert f^{-1}(z) \rvert \equiv \lvert (h\circ f)^{-1}(z) \rvert = \lvert f^{-1}(y) \rvert \pmod{2} 
\] 
\item Sia $g : M \to N$ una mappa $C^\infty$-omotopa a $f$ e sia $C$ l'insieme dei punti critici di $f$, quindi $C$ \`e chiuso in un compatto e, allora, \`e anch'esso compatto.
	Perci\`o, $f(C) \subseteq N$ sar\`a compatto, quindi $N \setminus f(C)$ (cio\`e l'insieme dei valori regolari di $f$) \`e aperto.
	Usando il fatto che i valori regolari sono densi all'insieme dei valori regolari di $g$, si ha che $\exists y \in N$ che \`e un valore regolare sia per $f$, che per $g$; allora, usando il lemma di omotopia (\ref{lom}), si pu\`o concludere che
	\[
	\deg_2 g = \lvert g^{-1}(y) \rvert \operatorname{mod} 2  = \lvert f^{-1}(y) \rvert \operatorname{mod} 2 = \deg _2 f
	\] 
\end{enumerate}
\end{proof}
\noindent Ora si dimostra il lemma di omogeneit\`a; per farlo, per\`o, \`e necessaria la seguente definizione.
\begin{definizione}
	[Elementi isotopi]
	Data $N$ una variet\`a e $y,z \in N$, si dice che $y$ e $z$ sono \textit{isotopi} se esiste un diffeomorfismo $h:N\to N$ isotopo a $\operatorname{Id} _N$ tramite isotopia a supporto compatto, con $h(y) = z$.
\end{definizione}
\begin{prop}
	Data $N$ una variet\`a differenziabile, l'isotopia tra punti \`e una relazione di equivalenza su $N$.
\end{prop}
\begin{proof}
	Per definizione, si ha che ogni $y\in N$ \`e isotopo a se stesso.
	Ora si dimostra che se $y$ \`e isotopo a $z$, allora $z$ \`e isotopo a $y$.
	Si ha, per $H : N \times [0,1] \to  N$ isotopia da $h$ a $\operatorname{Id} _N$, che
	\[
	\begin{array}
		{c c c c c c c}
		N \times [0,1] & \xrightarrow{\qquad}& N \times [0,1] &\xrightarrow{\quad \displaystyle H\quad} & N &\xrightarrow{\quad \displaystyle h^{-1} \quad} & N \\
		(x,t) & \xmapsto{\qquad}& (x,1-t) & & & &
	\end{array}
	\] 
	definisce un'isotopia a supporto compatto da $h^{-1}$ a $\operatorname{Id} _N$.

	Per finire, si dimostra che se $y$ \`e isotopo a $z$ e $z$ \`e isotopo a $w$, allora $y$ \`e isotopo a $w$.
	Si hanno le isotopie a supporto compatto
	\[
	\begin{array}
		{c c}
		\begin{array}
			{c c c}
			N \times [0,1]& \xrightarrow{\quad \displaystyle H \quad} & N
		\end{array} 
		&\hspace{1cm}
		\begin{array}
			{c c c}
			N \times [0,1] & \xrightarrow{\quad \displaystyle G \quad} & N
		\end{array}
	\end{array}
	\] 
	rispettivamente da $h$ a $\operatorname{Id}_N $, con $h(y) = z$, e da $g$ a $\operatorname{Id} _N$, con $g(z) = w$.
	Allora si definisce 
	\[
	\begin{array}
		{c c c c c}
		N \times [0,1] & \xrightarrow{\quad \displaystyle H \times \operatorname{Id}\quad } & N \times [0,1] & \xrightarrow{\quad \displaystyle G \quad} & N
	\end{array}
	\] 
	che \`e un'isotopia a supporto compatto da $g\circ h$ a $\operatorname{Id} _N$, con $g \big(h(y)\big) = w$.
\end{proof}
\begin{boxenv}[]
	\centering \color{nred} Riprendere dalla lezione 26
\end{boxenv}
...
\subsection{Continuit\`a e differenziabilit\`a}
\begin{prop}
	[Criterio per la continuit\`a in $\mathbb{R}^n$]
	Siano $A \subseteq \mathbb{R}^n$ un aperto e $f: A \to \mathbb{R}$; si dice che $f$ \`e \textit{continua} in $x_0 \in A$ se e solo se $\forall \gamma:[0,1]\to \mathbb{R}^n$ curva continua, con $\gamma(0) = x_0$, si ha $f \circ \gamma : [0,1] \to \mathbb{R}$ continua in $0$.
\end{prop}
\begin{proof}
\ 

\begin{itemize}
	\item[$(\Rightarrow )$] Se $f$ \`e continua in $x_0$ e $\gamma$ \`e continua in $0$, con $\gamma(0) = x_0$, allora $f\circ \gamma$ \`e continua in $0$ perch\'e la composizione di funzioni continue \`e continua.
	\item[$(\Leftarrow )$] Per assurdo, si assume che $f$ non sia continua e si considera la successione $\left\{ x_n \right\} $ tale che $x_n \to x_0$, con $f(x_n) \not \to f(x_0)$.
		Sia $\gamma$ una curva tale che, per $\left\{ t_n \right\} \subset [0,1]$ successione decrescente tendente a $0$, si abbia $\gamma(t_n) = x_n$; allora:
		\[
		f\circ \gamma(t_n) = f(x_n) \not \to f(x_0) = f\circ \gamma(0)
		\] 
	da cui l'assurdo $f\circ \gamma$ discontinua in $0$.
\end{itemize}
\end{proof}
\begin{prop}
	Sia $f : \mathbb{R}^n \to \mathbb{R}$; se esiste $g : [0,1] \to \mathbb{R}$ continua, con $g(0) = 0$ e $g\ge 0$ tale che 
	\[
	\lvert f(x) - f(x_0) \rvert \le g(\lvert x-x_0 \rvert )
	\] 
	allora $f$ \`e continua in $x_0$.
\end{prop}
\begin{proof}
Si ha
\[
0\le \lim_{x \to x_0} \lvert f(x) - f(x_0) \rvert \le \lim_{x \to x_0} g(\lvert x-x_0 \rvert ) = 0 \implies \lim_{x \to x_0} f(x) = f(x_0)
\] 
\end{proof}
\begin{definizione}
	[Derivata parziale]
	Sia $f : A \to \mathbb{R}$, con $A \subseteq \mathbb{R}^n$ aperto e $x_0 \in A$. 
	Si definisce la \textit{derivata parziale} $i$-esima di $f$ come 
	\[
...
\begin{definizione}
	[Gradiente]
	Sia $f : \mathbb{R}^n\to \mathbb{R}$ una funzione per cui le derivate parziali sono tutte ben definite nel $x_0$; allora si definisce \textit{gradiente} di $f$ in $x_0$ come il vettore
	\[
	\nabla f(x_0) = \begin{pmatrix} \partial _{x_1} f(x_0) \\ \vdots\\ \partial _{x_n} f(x_0) \end{pmatrix} = \begin{pmatrix} \partial _{x_1}  \\ \vdots\\ \partial _{x_n} \end{pmatrix} f(x_0)
	\] 
\end{definizione}
\begin{definizione}
	[Derivata direzionale]
	Sia $v \in \mathbb{R}^n \setminus \left\{ 0 \right\} $; allora si definisce
	\[
	\lim_{t \to 0} \frac{f(x_0 + tv) - f(x_0)}{t}= D_v f(x_0) = \frac{\partial f}{\partial v} (x_0)
	\] 
	come la \textit{derivata direzionale} di $f$ lungo $v$.
\end{definizione}
\begin{definizione}
	[Differenziabilit\`a]
	Una funzione $f$ \`e differenziabile in $x_0$ se esiste un vettore $v \in \mathbb{R}^n$ tale che
	\[
	f(x) = f(x_0) + v \cdot (x -x_0) + \operatorname{o} (\lvert x-x_0 \rvert )
	\] 
	Se tale vettore $v$ esiste, questo si indica con $\mathcal{D} f(x_0)$.
\end{definizione}
\begin{prop}
Se $f$ \`e differenziabile in $x_0$, allora il suo gradiente \`e ben definito e si ha
\[
f(x) = f(x_0) + \nabla f(x_0) \cdot (x-x_0) + \operatorname{o} (\lvert x-x_0 \rvert )
\] 
\end{prop}
\begin{proof}
	Sapendo che $f(x) = f(x_0) + v \cdot (x-x_0) + \operatorname{o} (\lvert x-x_0 \rvert )$, la derivata parziale $i$-esima \`e data da:
	\[
	\frac{f(x_0+te_i) - f(x_0)}{t} = \frac{v \cdot t e_i + \operatorname{o} (\lvert t e_i \rvert )}{t} = v_i + \operatorname{o} (1) \longrightarrow v_i
	\] 
	Allora le componenti del gradiente in $x_0$ coincidono con le componenti di $v$, proprio come da tesi.
\end{proof}
\begin{osservazione}
Tramite lo stesso conto della dimostrazione precedente, si vede che $\forall w \in \mathbb{R}^n$ esiste la derivata direzionale lungo $w$ ed \`e data da
\[
\frac{\partial f}{\partial w} (x_0) = \nabla f(x_0) \cdot w
\] 
\end{osservazione}
\begin{teorema}
Sia $f : A \to \mathbb{R}$, con $A \subseteq \mathbb{R}^n$ aperto, e sia $z_0=(x_0,y_0) \in A$ tale che, per ogni $(x,y)=z \in B_r(z_0)$, esistono tutte le derivate parziali in $z$ e sono continue in $z_0$; allora $f$ \`e differenziabile in $z_0$.
\end{teorema}
\noindent Data $f : A \to \mathbb{R}^m$, con $A \subseteq \mathbb{R}^n$ aperto, si scrive
\[
f = (f_1,\ldots,f_m)
\] 
dove le funzioni $f_i : A \to \mathbb{R}$ sono le \textit{componenti} di $f$.
Allora $f$ \`e continua in $x_0$ se e solo se ogni $f_i$ lo \`e; analogamente, $f$ \`e differenziabile in $x_0$ se ogni $f_i$ lo \`e.
\begin{definizione}
	[Matrice jacobiana]
	Data $f : A\subseteq \mathbb{R}^n \to \mathbb{R}^m$, si ha
	\[
	f(x) = f(x_0) + \mathcal{D} f(x_0) (x-x_0) + \operatorname{o} (\lvert x-x_0 \rvert )
	\] 
	dove la componente $(i,j)$ della matrice $\mathcal{D} f(x_0)$ \`e $\partial _{x_j} f_i(x_0)$. 
	Questa matrice \`e detta \textit{matrice jacobiana} in $x_0$ ed \`e, quindi, data da
	\[
		\mathcal{D} f =\begin{pmatrix} \partial _{x_1} f_1 & \cdots & \partial _{x_n} f_1\\ \vdots & \ddots & \vdots\\ \partial _{x_1} f_m & \cdots & \partial _{x_n} f_m\end{pmatrix} = \begin{pmatrix} \nabla f_1 ^\top \\ \vdots \\ \nabla f_m^\top \end{pmatrix} 
	\] 
	
\end{definizione}

\subsection{Invertibilit\`a e teorema di Dini}
\begin{definizione}
	[Diffeomorfismo]
	Sia $f : M \to  N$, con $M,N$ due aperti di $\mathbb{R}^n$, una mappa.
	Questa \`e detta \textit{diffeomorfismo} di classe $C^k$ se \`e di classe $C^k$, biettiva e la sua inversa \`e di classe $C^k$.
\end{definizione}
\begin{definizione}
	[Invertibilit\`a locale]
	Data $f: U \to \mathbb{R}^n$, con $U \subseteq \mathbb{R}^n$ aperto e fissato $x_0 \in U$, si dice che $f$ \`e \textit{localmente invertibile} in $x_0$ se esistono un intorno $V$ di $x_0$ e un intorno $W=f(V)$ di $f(x_0)$ tali che $f|_V : V \to W$ \`e invertibile.
\end{definizione}
\begin{teorema}
Siano $U \subseteq \mathbb{R}^n$ un aperto e $f : U \to \mathbb{R}^n$ una funzione di classe $C^k (U)$, con $k\ge 1$.
Dato $x_0 \in U$, se $\det (\mathcal{D} f(x_0))\neq 0$, allora esistono $V, W$ due intorni aperti di $x_0$ e $f(x_0)$ tali che $f|_V :V\to W$ \`e un diffeomorfismo di classe $C^k$.
\end{teorema}
\begin{teorema}
	[Teorema di Dini]
	Sia $f : \Omega \to \mathbb{R}^m$, con $\Omega \subseteq \mathbb{R}^n \times \mathbb{R}^m$, una funzione di classe $C^k$ e sia $(x_0,y_0) \in \Omega $.
	Se $\det (\mathcal{D} _y f(x_0,y_0)) \neq 0$, allora esistono $U$ intorno di $x_0$ e $g:U\to \mathbb{R}^m$ di classe $C^k$ tali che, per ogni $x \in U$, si ha 
	\[
	f(x,g(x)) = f(x_0,y_0)
	\] 
\end{teorema}
