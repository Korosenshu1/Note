...
In coordinate locali su una superficie, una curva $\alpha (t) = \underline{x}(u(t),v(t))$ ha velocit\`a data da $\alpha '(t) = u'(t) \underline{x}_u + v' \underline{x}_v$; visto che un campo vettoriale parallelo lungo $\alpha $ si scrive come $X(\alpha (t)) = a(t) \underline{x}_u(\alpha (t)) + b(t) \underline{x}_v (\alpha (t))$, quello coincidente con $\alpha '$ lungo $\alpha $ dovr\`a avere $a(t) = u'(t)$ e $b(t) = v'(t)$. 
...
\begin{equation}
	...
	\end{array}
\end{equation}
Ora, fissata una base $\left\{ e_1,e_2 \right\} \subset T_p\Sigma$, si ha la biezione $\mathbb{R}^2 \longleftrightarrow T_p\Sigma$; questa permette di individuare un aperto $U'p \subseteq \mathbb{R}^2$ che \`e in biezione con $U_p \subseteq T_p\Sigma$.
Pertanto si pu\`o definire la mappa
\[
	\underline{x}:
\begin{array}
	{c c c c c}
	U'p &\longrightarrow&U_p &\longrightarrow & \Sigma\\
...
\begin{proof}
	...
	\] 
	dove $\alpha (u,t) = \exp_q(u v(t))$ \`e la funzione usata nel lemma precedente e $\tau (t)$ \`e una funzione ausiliare permette di controllare la direzione angolare in generale (e non vincolarla ad essere lineare).
	...
	Allora, usando che $\alpha _u \cdot \alpha _t = 0$ per il lemma di Gauss:
	\[
...
\item Caso 2: $r \cos \phi = \text{cost.}\neq 0$.
	...
	A questo scopo, si usa il fatto che $\left\lVert \gamma' \right\rVert = 0$ e si deriva.
	\[
		\begin{split}
...
\begin{definizione}
	[Diffeomorfismo]
	Una mappa $f:X\subseteq \mathbb{R}^n \to Y \subseteq \mathbb{R}^m$ \`e detta \textit{diffeomorfismo} se \`e un omeomorfismo e $f,f^{-1}$ sono entrambe lisce. 
...
\begin{enumerate}[(a).]
	\item se $X  \subseteq \mathbb{R}^n$ \`e un aperto, allora $f:X\to Y \subseteq \mathbb{R}^m$ \`e $C^\infty\iff f:U\to \mathbb{R}^m$ lo \`e nel senso usuale;
...
\begin{definizione}
	[Differenziale]
	Siano $U \subseteq \mathbb{R}^k$, $V \subseteq \mathbb{R}^\ell $ due aperti e sia $f : U \to V$ di classe $C^\infty$; allora, date $x \in U$ e $h \in \mathbb{R}^k$, il \textit{differenziale} di $f$ \`e definito da $df_x:\mathbb{R}^k\to \mathbb{R}^{\ell } $ come
	\[
...
\] 
quindi $\mathrm{Im} (dg_u) = \mathrm{Im} (dh_v)$.
\end{proof}
\begin{prop}
...
\end{proof}
\noindent Si vuole definire il differenziale di mappe $C^\infty$ tra variet\`a.
Si considera, allora, $f: M \subseteq \mathbb{R}^k \longrightarrow N \subseteq \mathbb{R}^{\ell } $ liscia, con $x\longmapsto y= f(x)$; per definizione, quindi, $\exists W \subseteq \mathbb{R}^k$ intorno aperto di $x$ e $F:W\to \mathbb{R}^\ell $ tale che $F|_{W\cap M} = f|_{W\cap M} $.
Quindi si pu\`o definire il differenziale di $f$ come
\begin{equation}
	df_x := dF_x|_{T_x M} :T_x M \longrightarrow \mathbb{R}^\ell 
\end{equation}
\begin{lemma}
	Il differenziale $df_x$ \`e ben definito e $df_x (T_xM) \subseteq T_{f(x)} N$.
\end{lemma}
\begin{proof}
...
\begin{prop}
	...
	\begin{enumerate}[(a).]
		\item Se $x \in <$ \`e un punto regolare di $f$, allora la restrizione di $f$ \`e un diffeomorfismo tra un intorno di $x \in M$ e un intorno di $f(x)$ in $N$.
