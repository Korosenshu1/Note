...
\end{equation}
Si considera, una curva regolare $\alpha :[a,b] \to \mathbb{R}^3$; visto che $s(t) = \int_{a} ^t \left\lVert \alpha '(u) \right\rVert du$, allora $s'(t) = \left\lVert \alpha '(t) \right\rVert >0$.
...
\end{definizione}
\noindent Visto che $k(s) = \left\lVert T'(s) \right\rVert $, allora si pu\`o scrivere 
\begin{boxenv}[]
\begin{equation}
	T'(s) = k(s)N(s)
\end{equation}
\end{boxenv}
\noindent che \`e nota come \textbf{I equazione di Frenet}. 
Si nota che $N(s)$ deve avere norma unitaria perch\'e la norma di $T'(s)$ \`e data interamente da $k(s)$; inoltre, $N(s)$ \`e un versore ortogonale a $T(s)$ perch\'e si \`e dimostrato (prop. \ref{ezprop}) che $T(s) \perp T'(s)$.
Se ne ricava la seguente definizione.
\begin{definizione}
	[Versori normale e binormale]
	Dato $T(s)$ versore tangente di una certa curva regolare $\alpha : I \to \mathbb{R}^3$, si definiscono $N(s)$ \textit{versore normale principale} e $B(s) = T(s) \times N(s)$ \textit{versore binormale}.
\end{definizione}
...
Si nota che, essendo un versore, si ha $N(s) \cdot N(s) = 1$, quindi $N'(s) \cdot N(s) = 0$ (per lo stesso ragionamento della prop. \ref{ezprop}), dunque $N'(s) \in \langle T(s) , B(s) \rangle$; inoltre, essendo $T(s)\cdot N(s) = 0$, si ha 
\[
...
\begin{prop}
	...
	\begin{proof}
		...
		\begin{itemize}
			\item ($\Leftarrow$) Se $\alpha (I) \subseteq P+ \langle v \rangle$, per $v$ versore generico, si considera il diagramma
				\[
				\begin{tikzcd}
					J & & I & & \mathbb{R}^3
					\arrow[from=1-1, to=1-5, bend right=30, "\displaystyle \beta "']
					\arrow[from=1-3, to=1-1, "\displaystyle s"']
					\arrow[from=1-3, to=1-5,"\displaystyle \alpha "]
				\end{tikzcd}
				\] 
				cio\`e $\alpha (I) = \beta (J) \subseteq P+\langle v \rangle$, con $\beta $ pla di $\alpha $ (continuare \ldots).
		\end{itemize}
	\end{proof}
\end{prop}
