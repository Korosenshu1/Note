...
\end{prop}
	...
	\end{proof}
\subsubsection{Utilizzo delle varie azioni di gruppo}
\begin{enumerate}
  \item \textbf{Azione su classi laterali (traslazione a sinistra/destra)}  
  Definizione: \(g \cdot (xH) = (gx)H\).  
  \begin{itemize}
    \item Si usa quando compare l’indice \([G:H]\).  
    \item Induce un omomorfismo \(G \to S_{[G:H]}\).  
    \item Il nucleo è il \emph{core} di \(H\): \(\bigcap_{g \in G} gHg^{-1}\).  
    \item Risultati tipici: un sottogruppo di indice primo (o il più piccolo primo che divide \(|G|\)) è normale.
  \end{itemize}

  \item \textbf{Azione per coniugio su elementi del gruppo}  
  Definizione: \(g \cdot x = gxg^{-1}\).  
  \begin{itemize}
    \item Si usa per studiare centro, classi di coniugio, centralizzatori e normalizzatori.  
    \item Porta alla \emph{class equation}: \(|G| = |Z(G)| + \sum [G:C_G(x)]\).  
    \item Risultato tipico: il centro di un \(p\)-gruppo è non banale.
  \end{itemize}

  \item \textbf{Azione per coniugio su sottogruppi}  
  Definizione: \(g \cdot H = gHg^{-1}\).  
  \begin{itemize}
    \item Si usa per studiare la normalità di \(H\).  
    \item L’orbita di \(H\) è l’insieme delle sue coniugate.  
    \item Il numero di coniugate divide \([G:H]\).  
    \item L’intersezione delle coniugate è un sottogruppo normale (il \emph{core}).  
  \end{itemize}

  \item \textbf{Azione regolare (su se stesso per moltiplicazione)}  
  Definizione: \(g \cdot x = gx\).  
  \begin{itemize}
    \item Ogni gruppo si immerge in \(S_{|G|}\) (Lemma di Cayley).  
    \item Risultato tipico: ogni gruppo è isomorfo a un sottogruppo di un simmetrico.
  \end{itemize}
