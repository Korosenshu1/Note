...
\titleformat{\subsection}[block]{\centering\fontsize{14}{20}\bfseries}{\color{nred}\normalfont\S\thesubsection}{.5em}{}


% Personalizza la formattazione della subsubsection
\titleformat{\subsubsection}[block]{\centering\fontsize{12}{20}\bfseries}{\color{nred}\normalfont\S\thesubsubsection}{.5em}{}
...
\end{teorema}
	\begin{proof}
		...
		Si verifica prima che $G \cong mG \times m' G$.
...
\end{lemma}
	\begin{proof}
		...
		Per ipotesi, si era assunto che $x_1$ ha ordine massimo, sia questo $p^{r_1} $; allora deve risultare che $r\le r_1$, quindi si pu\`o scrivere che
		\[
...
\[
H = \langle x_2,\ldots,x_t \rangle\cong \langle x_2 \rangle\times \ldots\times \langle x_t \rangle
\] 
...
\] 
cio\`e se e solo se $\operatorname{ord}_{G/\langle x_1 \rangle} (\overline{x}_i) = p^{r_i}  \mid a_i $.
...
\] 
quindi si deve avere $a_ix_i = 0$ nel gruppo di partenza e $a_1 x_1=0$, da cui $\langle x_1 \rangle\cap H = \left\{ 0 \right\} $.
...
Intanto deve valere $s=t$ perch\'e, considerando $G_p = \left\{ g \in G  \mid gp = 0 \right\} $ (sottogruppo degli elementi di $G$ il cui ordine divide $p$), che \`e caratteristico (perch\'e gli automorfismi conservano l'ordine), si nota che gli elementi di $G$ di ordine tale da dividere $p$ stanno tutti in $G_p$, quindi $G_p$ \`e isomorfo a un sottogruppo della forma $(\mathbb{Z} / p\mathbb{Z})^\ell $, con $\ell $ numero dei fattori $\mathbb{Z} / p^h \mathbb{Z}$ distinti:
\[
...
\begin{teorema}
	...
	\begin{enumerate}[(a).]
		...
		\item \textit{inclusione}: ogni $p$-gruppo di $G$ \`e contenuto in un $p$-Sylow;
	...
	\begin{proof}
		...
		\begin{enumerate}[(a).]
			\item Si fissa $0\le \alpha \le n$. 
				...
				\] 
				essendo $i\le p^{\alpha }-1 $ e $\alpha \le n$. Visto che $v_p$ conta l'esponente massimo $k$ per cui $p^k$ divide il suo argomento, si conclude che $p^{n-\alpha } $ divide esattamente $\lvert \mathcal{M}  \rvert $ e $n-\alpha $ \`e il massimo esponente con cui $p$ pu\`o divide $\lvert \mathcal{M}  \rvert $\footnote{Per vederlo pi\`u chiaramente, si assume che qualche potenza di $p$ divida $i$, altrimenti $p^nm - i$ e $p^\alpha - i$ non sarebbero divisibili per alcuna potenza di i e si avrebbe la tesi. Allora, si pu\`o scrivere $i = p^s j$, con $\operatorname{gcd}(p,j) =1$, da cui $p^{n} m - i = p^s(p^{n-s} m - j)$ e $p^\alpha - i = p^s (p^{\alpha - s} - j)$. Il loro rapporto semplifica $p^s$ e rimane il rapporto di due termini non divisibili per alcuna potenza di $p$ perch\'e $(j,p) = 1$.}.

				Ora si considera l'azione di $G$ su $\mathcal{M} $ data da 
				\[
					\phi :
				\begin{array}
					{c c c}
					G &\longrightarrow & S(\mathcal{M} ) \\
					g &\longmapsto & \phi _g
				\end{array} \hspace{.2cm}, \hspace{.4cm} \text{con} \hspace{.3cm} \phi _g(M)=gM 
				\] 
				
				Visto che le orbite di questa azione partizionano $\mathcal{M} $, si ha:
				\[
			...
			\item Quanto riportato in (b) dimostra anche la parte sul coniugio; infatti, se $H$ \`e un $p$-Sylow con $\lvert H\rvert = p^n$, allora \`e un $p$-gruppo, allora $H \subset gSg^{-1}$ e, visto che hanno stessa cardinalit\`a, segue che $H = g S g^{-1}$.
...
\textbf{Prodotto in $\mathbf{Q_8} $.} Convenzionalmente, si definisce $ij =k$; in questo modo, gli elementi del gruppo si possono riscrivere come:
\[
Q_8 = \left\{ \pm 1 , \pm i, \pm j, \pm k \right\} 
\] 
con $i^2 = -1$, $i^3 = -i$, $j^3 = -j$ e $i^3 j = - ij = - k$.
	In questo modo, si vede che i prodotti tra gli elementi di $Q_8$ soddisfano il seguente $3$-ciclo:
	\begin{center}
  ...
  % Frecce
  \draw[bend right=50] (i) to (j);
  \draw[bend right=50] (j) to (k);
  \draw[bend right=50] (k) to (i);
\end{tikzpicture}
	\end{center}
...
\[
ji = -k \hspace{1cm} ik = -j \hspace{1cm} kj = -i
\] 
Inoltre, si nota che:
\[
k^2 = (ij)^2 = ijij=i^2
\] 
Ne segue, quindi, che $\operatorname{ord}(1) = 1$ e $\operatorname{ord}(-1) =2$, mentre l'ordine di $\pm i, \pm j, \pm k$ \`e $4$.
...
\textbf{Sottogruppi di $\mathbf{Q_8}$.} 
Quanto ai sottogruppi di $Q_8$, si vede, intanto, che $\langle -1 \rangle= Z(Q_8)$ ed \`e caratteristico, visto che \`e il centro (oppure si pu\`o concludere osservando che \`e l'unico sottogruppo di ordine $2$).
Al contrario, $\langle i \rangle$, $\langle j \rangle$ e $\langle k \rangle$ sono sottogruppi di ordine $4$ e, come accennato, sono normali.
Questo permette di concludere che ogni sottogruppo di $Q_8$ \`e normale.

\textbf{Prodotto semidiretto.} Infine, si osserva che $Q_8$ non pu\`o essere ottenuto come prodotto semidiretto di alcun suo sottogruppo; infatti, $\forall H_1,H_2<Q_8$, si ha $H_1\cap H_2 \neq \left\{ 1 \right\} $ perch\'e l'intersezione contiene sempre anche $-1$.
...
\end{prop}
	\begin{proof}
		Si fa agire $G \curvearrowright X$, con $X = G / H$ insieme delle classi laterali di $H$ in $G$, quindi $\lvert X \rvert = p$. 
		L'azione \`e definita dall'omomorfismo
		\[
\phi :
		\begin{array}
			{c c c}
			G &\longrightarrow & S(X) \cong S_p\\
			 g &\longmapsto & \big(\phi _g:xH \mapsto (gx) H\big)
		\end{array}
		\] 
		Il nucleo di questo omomorfismo \`e dato da: 
		\[
			\operatorname{Ker} \phi = \left\{ g \in G  \mid  gxH = xH ,\ \forall x \in G \right\} = \left\{ g \in G  \mid x^{-1}gx H = H, \ \forall x \in G \right\} 
		\] 
		La condizione $x^{-1}gx \in G$ equivale a richiedere che $x^{-1}g x = h$, per qualche $h\in H$, ossia $g = xhx^{-1}$.
		Richiedendo che valga per ogni $x \in G$, si conclude che $\operatorname{Ker} \phi $ \`e l'insieme degli elementi di $G$ che si ottengono coniugando elementi di $H$ per coniugio tramite ogni elemento di $G$; questo si formalizza con la seguente scrittura:
		\[
		\operatorname{Ker} \phi = \bigcap_{x \in G} xHx^{-1}:=H_G
		\] 
L'idea \`e di dimostrare che $H_G \lhd G$ (essendo il nucleo di un omomorfismo $G \to S_p$) coincide con $H$.

Per il I teorema di omomorfismo, $G / H_G \longrightarrow S_p$ \`e iniettiva, quindi $\lvert G / H_G \rvert  \mid p !$.
Allo stesso tempo, per Lagrange, $\lvert G \rvert = n = [G:H_G]\lvert H_G \rvert $, quindi $\lvert G / H_G \rvert  \mid n$; essendo $p$ il pi\`u piccolo primo che divide $n$ e essendo $p$ il pi\`u grande primo presente in $p!$, allora $\lvert G / H_G \rvert = \left\{ 1,p \right\} $.
Per finire, si nota che $g \in H_G \Rightarrow gH = H$, cio\`e $g \in H$ e, quindi, $H_G \subseteq H$. 
Perci\`o, essendo $H_G$ al pi\`u $H$, deve valere $\lvert G / H_G \rvert \ge p$, quindi $\lvert G/H_G \rvert = p$ e $\lvert H_G \rvert =\lvert H \rvert $, da cui $H= H_G$.
	\end{proof}
...
\begin{prop}\label{idprop}
	Sia $A$ un anello; allora un suo ideale $I \subset A$ \`e proprio se e solo se $I \cap A^* = \varnothing$.
\end{prop}
	\begin{proof}
...
\[
f(a) = f(1_A a) \implies f(a) - f(1_A) f(a) = (1_B - f(1_A))f(a) = 0
\] 
...
\end{prop}
	\begin{proof}
		...
		Per quanto visto nella prop. \ref{idprmaxchar}, richiedere che $J $ sia primo \`e equivalente a richiedere che $A / J$ sia un dominio (campo) e, analogamente, deve risultare che $\frac{A / I}{J / I}$ \`e un dominio (campo).
...
\begin{definizione}
	...
	Sia $S$ la sua parte moltiplicativa; allora, si definisce il suo \textit{insieme delle frazioni}, o \textit{localizzazione di $A$ ad $S$} come
	\[
...
\end{prop}
	\begin{proof}
		Si verifica intanto che $Q(A)$ \`e un campo, per cui \`e sufficiente mostrare che esistono gli inverso moltiplicativi, cosa che segue direttamente dal fatto che, $\forall a \in A \setminus \left\{ 0 \right\} $, si ha $1 / a \in Q(A)$; in questo modo, ciascun elemento di $A\setminus\left\{ 0 \right\} $ si pu\`o scrivere come $1 / a$ e, quindi:
		\[
...
\begin{esempio}
	...
	\end{itemize}
\end{esempio}
\noindent Si nota che se $A$ \`e un dominio e $P$ \`e un suo ideale primo, allora $S = A \setminus P$ \`e una parte moltiplicativa; infatti: $0 \not \in S, \ 1 \in S$ e $\forall x,y \in S$, vale $x,y \not \in P \implies xy \not \in P$, visto che $P$ primo implica che se non contiene due elementi, non ne contiene neanche il prodotto (per controposizione), quindi il prodotto \`e nel complementare: $xy \in A \setminus P = S$.
\begin{definizione}
	[Localizzato]
		Dato un dominio $A$ e $P \subset A$ un suo ideale primo, si pu\`o considerare $S = A\setminus P$; allora si definisce $S^{-1}A = A_p$ il \textit{localizzato} di $A$ a $P$.
\end{definizione}
\begin{osservazione}
...
\begin{definizione}
	...
	Sia $K$ un campo e $L$ una sua estensione; allora $\alpha  \in L$ si dice \textit{algebrico} su $K$ se $\exists f(x) \in K[x]\setminus\left\{ 0 \right\} $ tale che $f(\alpha ) = 0$, mentre si dice \textit{trascendente} su $K$ se tale polinomio non esiste.
\end{definizione}
\noindent Dati un campo $K$ e una sua estensione $L$, per $\alpha \in L$, si pu\`o definire la mappa
\[
	\varphi _\alpha :
\begin{array}
	{c c c}
...
\end{array}
\] 
detta \textit{omomorfismo di valutazione}.
Questo soddisfa il seguente diagramma:
\[
\begin{tikzcd}
	K[x] & & & K[\alpha ]\\
	\\
	\\
	K[x] / \operatorname{Ker} \varphi _\alpha 
	\arrow[from=1-1, to=1-4,"\displaystyle \varphi _\alpha ",two heads]
	\arrow[from=1-1, to=4-1,"\displaystyle \pi"',two heads]
	\arrow[from=4-1, to=1-4,"\displaystyle \overline{\varphi }_\alpha "',"\displaystyle \sim"]
\end{tikzcd}
\] 
cio\`e $K[\alpha ] \cong K[x] / \operatorname{Ker} \varphi _\alpha $.
Si nota, da questo, che $\operatorname{Ker} \varphi _\alpha \subset K[x]$ \`e un ideale primo perch\'e $K[\alpha ]$ \`e un sottoanello di un campo, quindi \`e un dominio.
In termini dell'omomorfismo di valutazione, si ha:
\begin{itemize}
	\item $\alpha $ trascendente su $K \iff \operatorname{Ker} \varphi _\alpha  = \left\{ 0 \right\} \iff\varphi _\alpha$ \`e iniettivo $\iff K[x] \cong K[\alpha ]$;
	\item $\alpha $ \`e algebrico su $K \iff \operatorname{Ker} \varphi _\alpha \neq \left\{ 0 \right\} \iff \varphi _\alpha $ non \`e iniettivo.
\end{itemize}
\begin{osservazione}
	Se $\alpha \in L$ algebrico su $K$, visto che $K[x]$ \`e un ED, \`e un PID in particolare, allora $\operatorname{Ker} \varphi _\alpha $ deve essere un ideale massimale di $K[x]$, quindi $K[x] / \operatorname{Ker} \varphi _\alpha $ \`e un campo e, conseguentemente, lo \`e anche $K[\alpha ]$.
	In questo caso, allora, $K[x] = K(\alpha )$.
\end{osservazione}
\noindent Essendo $K[x]$ a ideali principali, allora $\operatorname{Ker} \varphi _\alpha  =\langle \mu _\alpha (x) \rangle$, con $\mu _\alpha (x) \in \operatorname{Ker} \varphi _\alpha $; visto che \`e un ideale massimale, allora $\mu _\alpha (x)$ \`e irriducibile in $K[x]$.
Inoltre, \`e possibile scegliere $\mu _\alpha (x)$ tale che sia l'unico generatore monico; infatti, essendo $K[x]$ un ED, si sa che i suoi ideali sono generati dagli elementi di grado minimo e tali elementi differiscono per un elemento di $K\setminus\left\{ 0 \right\} $.
\begin{definizione}
	[Grado di un'estensione]
	Data un'estensione $L$ di $K$, indicata con $L \ K$, se ne definisce il \textit{grado} come
	\[
		[L:K] = \dim _{K } L
	\] 
	cio\`e \`e la dimensione di $L$ come spazio vettoriale su $K$.
\end{definizione}
\begin{prop}\label{gradext}
	Sia $L / K$ un'estensione di $K$, con $\alpha \in L$; allora il grado dell'estensione semplice $K(\alpha )$ \`e
	\[
		[K(\alpha ):K]=\begin{cases}
			+\infty &,\ \text{ se } \alpha \text{ trascendente su } K\\
			\deg \mu _\alpha (x) &,\ \text{ se } \alpha \text{ algebrico su } K\\
		\end{cases}
	\] 
\end{prop}
\begin{proof}
	Se $\alpha $ \`e trascendente su $K$, allora $\operatorname{Ker} \varphi _\alpha  = \left\{ 0 \right\} $, il che vuol dire che non esiste alcuna relazione banale tra le varie potenze di $\alpha $ non esiste alcuna relazione di dipendenza lineare, quindi l'insieme $\left\{ 1, \alpha ,\ldots, \alpha ^k,\ldots \right\} $ \`e linearmente indipendente in $K[\alpha ] \subset K(\alpha )$ e, conseguentemente, $K[\alpha ]$ ha dimensione infinita, quindi anche $K(\alpha )$.

	Se $\alpha $ \`e algebrico su $K$, invece, si ha
	\[
		K(\alpha ) \cong K[\alpha ] \cong \frac{K[x]}{(\mu _\alpha (x))}
	\] 
	tramite l'isomorfismo che mappa la base $\left\{ \overline{1}, \overline{x},\ldots, \overline{x}^{n-1} \right\} $ di $K[x] / \langle \mu _\alpha (x) \rangle$ nella base $\left\{ 1 , \alpha  ,\ldots, \alpha  ^{n-1} \right\} $ di $K[\alpha ]$, perci\`o $\dim _K K(\alpha )= n = \deg \mu _\alpha (x)$.
\end{proof}
\begin{prop}
	\label{text}
	Data una torre di estensioni $K \subset  F \subset L$, allora $L / K$ \`e finita se e solo se $L / F$ e $F / K$ sono finite; inoltre vale
	\[
		[L:K] = [L:F] [F:K]
	\] 
\end{prop}
\begin{definizione}
	[Estensione composta]
	Dati $L,M \subset \Omega $ due sottocampi del campo $\Omega $, allora si definisce
	\[
	LM = L(M) = M(L)
	\] 
	come il pi\`u piccolo sottocampo di $\Omega $ che contiene sia $L$ e che $M$.
	Inoltre, se $M$ e $L$ sono estensioni finitamente generate con
	\[
	L = K(\alpha_1,\ldots,\alpha _n) \hspace{1cm}M = K(\beta _1,\ldots,\beta _m)
	\] 
	allora il composto \`e dato da
	\[
	LM = K(\alpha _1,\ldots,\alpha _n, \beta _1,\ldots,\beta _m)
	\] 
\end{definizione}
\begin{prop}\label{gradcompext}
	Siano date due torri di estensioni di $K$:
	\[
	K \subset  L \subset FL \hspace{2cm} K \subset F \subset FL
	\] 
	con $[L:K] = m$ e $[F:K]= n$; allora $[FL:K] = d < +\infty$ e $[m,n]  \mid d$.
\end{prop}
\begin{proof}
	Si ha il seguente diagramma di estensioni:
	\[
	\begin{tikzcd}
		&  FL  & \\
		L  & &  F \\
		  &  K  & 
		  \arrow[from=3-2, to=2-1, "\displaystyle m", no head]
		  \arrow[from=3-2, to=2-3, "\displaystyle n"', no head]
		  \arrow[from=3-2, to=1-2, "\displaystyle d", no head]
		  \arrow[from=2-1, to=1-2, no head] 
		  \arrow[from=2-3, to=1-2, no head]
	\end{tikzcd}
	\] 
	Intanto, le estensioni $[FL:L]$ e $[FL:F]$ sono finite perch\'e la base prodotto $\left\{ f_i\ell _j \right\}  $ ottenuta moltiplicando una base $\left\{ f_i \right\} _{i=1} ^n$ di $F$ su $K$ e $\left\{ \ell _j \right\}_{j=1} ^m $ di $L$ su $K$ genera gli elementi di $FL$; in particolare, \`e finita l'estensione $[FL:K]$.
	Infine, applicando due volte la proposizione \ref{text}, si ottiene che $m,n \mid d$. 
\end{proof}
\subsection{Estensioni algebriche}
\begin{definizione}
	[Estensione algebrica]
	Un'estensione $L / K$ \`e detta \textit{algebrica} se $\forall  \alpha  \in L$, si ha $\alpha $ algebrico su $K$.
\end{definizione}
\begin{prop}\label{algextfin}
	Ogni estensione di grado finito \`e algebrica.
\end{prop}
\begin{proof}
	Si verifica, quindi, che $\forall \alpha \in L$, $\alpha $ \`e algebrico su $K$.
	Si ha la torre $K \subset K(\alpha ) \subseteq L$ che, per ipotesi, \`e finita, quindi la sottoestensione $K \subset K(\alpha )$ \`e a sua volta finita e, per la proposizione \ref{gradext}, si conclude che $\alpha $ deve essere algebrico.
	Ma questo vale $\forall \alpha  \in L$, quindi $L$ \`e un'estensione algebrica di $K$.
\end{proof}
\begin{prop}
	Data un'estensione $L / K$, allora 
	\[
	A = \left\{ \alpha  \in L  \mid \alpha  \text{ algebrico su } K \right\} 
	\] 
	\`e un campo e un'estensione algebrica di $K$.
\end{prop}
\begin{proof}
	Siano $\alpha , \beta  \in  A$; allora $[K(\alpha ): K ], \ [ K(\beta):  K] < + \infty$ e si considera la torre
	\[
	K \subseteq K(\alpha ) \subseteq K(\alpha )(\beta ) = K(\alpha ,\beta )
	\] 
	La prima estensione \`e finita per ipotesi, mentre la seconda \`e finita per la proposizione \ref{gradcompext}, visto che \`e l'estensione composta di $K(\alpha )$ e $K(\beta )$, che sono entrambe semplici e, quindi, finite perch\'e algebriche.
	Visto che, allora, $K \subseteq K(\alpha ,\beta )$ \`e un'estensione finita, per la proposizione \ref{algextfin}, gli elementi $\alpha \pm \beta, \ \alpha \beta ,\ 1 / \alpha , \ 1 / \beta  $ sono algebrici su $K$, quindi $A$ \`e un campo.
\end{proof}
	Il viceversa della proposizione appena dimostrata \`e falso in generale, ma non nel caso di estensioni semplici, per quanto affermato dalla proposizione \ref{gradext}.
	Infatti, si pu\`o considerare l'estensione $\mathbb{Q}\subset \mathbb{C}$ e l'insieme 
	\[
	\overline{\mathbb{Q}}= \left\{ \alpha  \in \mathbb{C} \mid \alpha \text{ algebrico su }\mathbb{Q} \right\} 
	\] 
	Si avr\`a che $[\overline{\mathbb{Q}}: \mathbb{Q}] = + \infty$.
	Si considera la torre 
	\[
		\mathbb{Q}\subset \mathbb{Q}(\sqrt[n]{2} ) \subset \overline{\mathbb{Q}}, \ \forall n \ge 2
	\] 
	L'estensione $\mathbb{Q}\subset \mathbb{Q}(\sqrt[n]{2}) $ ha grado $n$ perch\'e il suo polinomio minimo \`e 
	\[
		\mu _{\sqrt[n]{2} } = x^n -2
	\] 
	Per la proposizione \ref{text}:
	\[
		[\overline{\mathbb{Q}}:\mathbb{Q}] = [\overline{\mathbb{Q}}:\mathbb{Q}(\sqrt[n]{2} )] [ \mathbb{Q}(\sqrt[n]{2} ):\mathbb{Q}] \ge n, \ \forall n \ge 2
	\] 
	quindi l'estensione $\mathbb{Q}\subset \overline{\mathbb{Q}}$ ha grado $+\infty$.

	\begin{prop}\label{efgag}
		$L / K$ \`e un'estensione finitamente generata da elementi algebrici $L = K(\alpha_1, \ldots, \alpha _n)$ se e solo se $L / K $ \`e finita.
	\end{prop}
	\begin{proof}
		Per dimostrare ($\Rightarrow $), si procede per induzione.
		Se $n=1$, la tesi \`e vera per la proposizione \ref{gradext}, visto che l'estensione \`e semplice ed algebrica, quindi finita.
		Si assume che la tesi sia vera per $n-1$.
		Si pu\`o scrivere 
		\[
		K(\alpha _1,\ldots,\alpha _n) = K(\alpha _1,\ldots,\alpha _{n-1} ) (\alpha _n)
		\] 
		e si considera la torre
		\[
		K \subset K(\alpha _1,\ldots,\alpha _{n-1} ) \subset K(\alpha _1,\ldots,\alpha _{n-1} ) (\alpha _n)
		\] 
		dove la prima estensione \`e finita per ipotesi induttiva, mentre la seconda lo \`e per la proposizione \ref{gradext}.

		Per il viceversa, si considera l'estensione finita $[L:K]=n$ e si considera $\left\{ v_1,\ldots,v_n \right\} $ una $K$-base di $L$; allora $L$ \`e un'estensione finitamente generata:
		\[
		L = \mathrm{Span} (v_1,\ldots,v_n)_K = K(v_1,\ldots,v_n)
		\] 
		quindi si ha la tesi.
	\end{proof}
	\begin{teorema}
		[Caratterizzazione delle estensioni algebriche]
		Valgono le due seguenti propriet\`a per estensioni algebriche.
		\begin{enumerate}[(a).]
			\item Data una torre di estensioni $K \subset L \subset F$, l'estensione $F / K$ \`e algebrica se e solo se $F / L$ e $L / K$ sono algebriche.
			\item Date due estensioni $L / K$ e $M / K$, queste sono algebriche se e solo se l'estensione composta $LM / K$ \`e algebrica.
		\end{enumerate}
	\end{teorema}
	\begin{proof}
		Si divide la dimostrazione nei due punti.
		\begin{enumerate}[(a).]
			\item Si dimostra l'implicazione ($\Rightarrow $).
				Se $F / K$ \`e algebrica, allora $\forall \alpha \in F$, si ha $\alpha $ algebrico su $K$, quindi ogni $\alpha \in L \subset F$ \`e algebrico su $K$ e, dunque, $L$ \`e algebrico su $K$.
				Inoltre, $F$ \`e algebrico su $L$ perch\'e contiene $K$: tutti gli elementi di $F$ sono gi\`a algebrici su $K$ e, a maggior ragione, sono algebrici anche su $L$, che \`e un campo pi\`u grande.

				Ora si dimostra ($\Leftarrow$).
				Sia $\alpha \in F$; per ipotesi $F / L$, $L / K$ algebriche, quindi $\alpha $ \`e algebrico su $L$ e, quindi
				\[
					\exists f(x) \in L[x] \setminus \left\{ 0 \right\} : f(\alpha ) = 0\hspace{.5cm} \text{con} \hspace{.5cm} f(x) = \sum_{i=0}^{n} a_i x^i
				\] 
				Sia $L_0 = K(a_0,\ldots,a_n)$ il campo generato su $K$ dai coefficienti di $f$; per costruzione, $f(x) \in L_0[x]$, quindi $\alpha $ \`e algebrico su $L_0$ e 
				\[
				K \subset L_0 \subset L_0(\alpha ) ,\ \forall \alpha \in F
				\] 
			La prima estensione \`e finita perch\'e \`e generata da elementi algebrici, mentre la seconda \`e ancora finita perch\'e \`e un'estensione algebrica semplice (vale la proposizione \ref{gradext}).
			Di conseguenza, la torre di estensioni \`e finita e $[L_0(\alpha ):K] < +\infty$, quindi $L_0(\alpha ) / K$ \`e algebrica per la proposizione \ref{algextfin}.
			Ne segue che $\alpha \in F$ \`e algebrico su $K$, per ogni $\alpha  \in F$, quindi $F / K $ \`e algebrica.
		\item Si dimostra ($\Leftarrow$), quindi si prende $LM / K$ algebrica. 
			Sia $\alpha  \in M \subset LM$; allora $\alpha $ \`e algebrico su $K$ perch\'e ogni elemento di $LM$ lo \`e.
			Ma il discorso vale per ogni elemento di $M$ e, analogamente, per ogni elemento di $L$, quindi $M / K$ e $L / K$ sono algebriche.

			Ora si dimostra ($\Rightarrow $), quindi si considerano $M / K$ e $L / K$ algebriche.
			Sia $\alpha  \in LM = L(M)$; visto che $M$ \`e algebrico su $K$ (quindi \`e algebrico a maggior ragione su $L$), allora $L(m_1,\ldots,m_n) / L$ \`e un'estensione finita (perch\'e finitamente generata da algebrici) e tale che $\alpha \in L(m_1,\ldots,m_n)$.
			In questo modo, si pu\`o far vedere che $\alpha $ appartiene ad un'estensione finita di $K$, quindi un'estensione algebrica e, quindi, $\alpha $ \`e algebrico su $K$.
			Si nota, infatti, che per il ragionamento fatto sopra:
			\[
			\alpha  \in  F = K(\lambda _1,\ldots,\lambda _n,m_1,\ldots,m_n)
			\] 
			dove i $\lambda _i\in L$ sono i coefficienti della decomposizione di $\alpha $ tramite la base di $L(m_1,\ldots,m_n)$ come spazio vettoriale\footnote{Tale spazio ha dimensione finita perch\'e l'estensione $L(m_1,\ldots,m_n) / L$ \`e finita, quindi la decomposizione ha senso.} su $L$. 
			Questa \`e un'estensione algebrica di $K$ perch\'e sono algebrici su $K$ tutti gli elementi di $L$ e $M$ per ipotesi, quindi $\alpha $ \`e algebrico su $K$.
			Visto che questo discorso si pu\`o ripetere per ogni $\alpha \in LM $, si ha la tesi.
		\end{enumerate}
	\end{proof}
	\subsection{Chiusura algebrica}
	\begin{definizione}
		[Campo algebricamente chiuso]
		Un campo $\Omega $ si dice \textit{algebricamente chiuso} se ogni polinomio $f(x) \in \Omega [x]$ non costante ha almeno una radice in $\Omega $.
	\end{definizione}
	\noindent Come conseguenza del teorema fondamentale dell'algebra, $\mathbb{C}$ \`e algebricamente chiuso.
	Si nota, inoltre, che se $\Omega $ \`e algebricamente chiuso, gli unici polinomi irriducibili in $\Omega [x]$ sono quelli di grado $1$.
	\begin{definizione}
		[Chiusura algebrica]
		Un'estensione $\Omega / K$ \`e una \textit{chiusura algebrica} di $K$ se:
		\begin{enumerate}[(a).]
			\item $\Omega $ \`e algebricamente chiuso;
			\item $\Omega  / K$ \`e un'estensione algebrica.
		\end{enumerate}
	\end{definizione}
	\noindent Per definizione, si osserva che $\mathbb{C}$ \`e la chiusura algebrica di $\mathbb{R}$, ma non di $\mathbb{Q}$ perch\'e non tutti gli elementi di $\mathbb{C}$ sono algebrici su $\mathbb{Q}$.
	\begin{teorema}
		[Esistenza e unicit\`a della chiusura]
		Sia $K$ un campo; allora esiste sempre una sua chiusura algebrica e due qualunque di queste sono isomorfe, nel senso che gli isomorfismi tra le chiusure fissano $K$.
	\end{teorema}
	\noindent A scopo di esempio, si pu\`o considerare $\overline{\mathbb{Q}}=\left\{ \alpha \in \mathbb{C} \mid \alpha \text{ algebrico su } \mathbb{Q} \right\} $, che \`e un campo, per quanto visto. 
	\`E, inoltre, una chiusura algebrica di $\mathbb{Q}$ perch\'e l'estensione $\overline{\mathbb{Q}}/ \mathbb{Q}$ \`e algebrica per definizione; si verifica, ora, che $\overline{\mathbb{Q}}$ \`e algebricamente chiuso.
	Sia, allora, $f(x) \in \overline{\mathbb{Q}}[x]$ un polinomio non costante; allora $f(x)$ ammette almeno una radice $\alpha  \in \mathbb{C}$, visto che quest'ultimo \`e algebricamente chiuso.
	Si deve far vedere che $\alpha \in \overline{\mathbb{Q}}$, perci\`o si deve mostrare che $\alpha $ \`e algebrico su $\mathbb{Q}$; a tal proposito, si considera la torre $\mathbb{Q} \subset \overline{\mathbb{Q}} \subset \overline{\mathbb{Q}}(\alpha )$.
	Allora basta osservare che l'estensione $\overline{\mathbb{Q}}/\mathbb{Q}$ \`e algebrica per definizione, quindi \`e finita in particolare, e l'estensione $\overline{\mathbb{Q}}(\alpha ) / \overline{\mathbb{Q}}$ \`e algebrica, in quanto estensione semplice finita, pertanto l'estensione $\overline{\mathbb{Q}}(\alpha ) / \mathbb{Q}$ \`e finita, quindi $\alpha $ \`e algebrico su $\mathbb{Q}$.
	Questo, in aggiunta al fatto che $\alpha  \in \mathbb{C}$, permette di concludere che $\alpha  \in \overline{\mathbb{Q}}$ e, quindi, che quest'ultimo \`e algebricamente chiuso perch\'e il discorso vale per ogni polinomio in $\overline{\mathbb{Q}}[x]$.
\begin{osservazione}
Il ragionamento appena fatto permette di costruire una chiusura algebrica $\overline{K}$ di un campo $K$ ogni volta che $K \subset \Omega $, con $\Omega $ algebricamente chiuso, semplicemente prendendo 
\[
\overline{K} = \left\{ \alpha \in \Omega  \mid \alpha \text{ algebrico su } K \right\} 
\] 
\end{osservazione}
\begin{definizione}
	[Campo di spezzamento]
	Sia $f(x) \in K[x]$, con $\deg f(x) \ge 1$, e siano $\alpha _1,\ldots,\alpha _n \in \overline{K}$ radici di $f(x)$; allora si definisce \textit{campo di spezzamento} di $f(x)$ su $K$ il sottocampo di $\overline{K}$ dato da $K(\alpha _1,\ldots,\alpha _n)$.
\end{definizione}
\begin{esempio}
	[Campo di spezzamento di $x^3 - 2$]
	Sia $f(x) = x^3 - 2 \in \mathbb{Q}[x]$. 
	Le sue radici in $\mathbb{C}$ sono $\sqrt[3]{2}$, $ \sqrt[3]{2} \zeta_3$ e $\sqrt[3]{2} \zeta_3^2$, perci\`o il suo campo di spezzamento su $\mathbb{Q}$ \`e dato da $\mathbb{Q}(\sqrt[3]{2},\sqrt[3]{2}\zeta_3,\sqrt[3]{2}\zeta_3^2)$.
	In realt\`a, questo campo di spezzamento coincide con $\mathbb{Q}(\sqrt[3]{2} ,\zeta_3)$; infatti, sicuramente $\mathbb{Q}(\sqrt[3]{2} ,\zeta_3)\subseteq \mathbb{Q}(\sqrt[3]{2} ,\sqrt[3]{2}\zeta_3,\sqrt[3]{2}\zeta_3^2)$ perch\'e quest'ultimo contiene sia $\sqrt[3]{2}$, che
	\[
		\zeta_3 = \frac{\sqrt[3]{2} \zeta_3 }{\sqrt[3]{2} }
	\] 
	ma \`e anche vera l'inclusione opposta perch\'e, in $\mathbb{Q}(\sqrt[3]{2}, \zeta_3) $, ci sono:
	\begin{itemize}
		\item $\sqrt[3]{2} $ per costruzione;
		\item $\zeta_3$ per costruzione, quindi anche $\sqrt[3]{2} \zeta_3 $ per chiusura rispetto alla moltiplicazione;
		\item sempre per chiusura rispetto alla moltiplicazione, $\zeta_3 ^2$ e, quindi, $\sqrt[3]{2} \zeta_3^2$.
	\end{itemize}
\end{esempio}


\noindent Ora si vuole studiare il seguente problema: dato un campo $K$ e la sua chiusura $\overline{K}$, per $\alpha \in \overline{K}$, si vuole capire quante iniezioni
\[
\varphi :K(\alpha )\longhookrightarrow \overline{K} \hspace{.5cm} \text{ con }\hspace{.5cm}\varphi |_K = \mathrm{Id} _K
\] 
si possono costruire, cio\`e in quanto modi $K(\alpha )$ si pu\`o immergere in $\overline{K}$.
Ora, si ricorda preliminarmente che un omomorfismo fra campi pu\`o avere solo nucleo banale, oppure tutto il campo, visto che tale nucleo sarebbe un ideale del campo, il quale ha solo ideale banale e se stesso come ideali.
La seguente proposizione fornisce la risposta.
\begin{prop}\label{imext}
	Sia $K$ un campo e $\alpha \in \overline{K}$, con $\overline{K}$ chiusura algebrica di $K$.
	Dato $\alpha  \in \overline{K}$ e detto $k$ il numero di radici distinte di $\mu _\alpha (x)$ in $\overline{K}$, allora
	\[
	\exists \varphi _1,\ldots,\varphi _k : K(\alpha ) \longhookrightarrow \overline{K}
	\] 
	estensioni dell'immersione $K\longhookrightarrow \overline{K}$ data dall'identit\`a, cio\`e con $\varphi _i|_K = \mathrm{Id} _K$.
\end{prop}
\begin{proof}
	Tramite omomorfismo di valutazione, si sa che $K(\alpha ) \cong K[x] /\langle \mu _\alpha (x) \rangle$, quindi ci si pu\`o limitare a trovare un omomorfismo da $K[x]$ in $\overline{K}$ e poi usare il I teorema di omomorfismo.
	Si considera l'immersione $K \longhookrightarrow \overline{K}$, data dall'identit\`a; questa si pu\`o estendere in una mappa da $K[x]$ a $\overline{K}$ tramite 
	\[
		\widetilde{\varphi }:
		\begin{array}
			{c c c}
			K[x]& \longrightarrow &\overline{K}\\
			p(x) & \longmapsto& p(\beta)
		\end{array}, \ \forall \beta \in \overline{K}
	\] 
	Per tale omomorfismo, ha senso considerare il quoziente $K(\alpha ) \cong K[x] / \langle \mu _\alpha (x) \rangle$ se e solo se $\langle \mu _\alpha (x) \rangle\subseteq \operatorname{Ker} \widetilde{\varphi }$, cio\`e se e solo se $\mu _\alpha (x) \in \operatorname{Ker} \widetilde{\varphi }$, quindi se e solo se $\mu _\alpha (\beta ) = 0$.
	Allora, per ogni radice di $\mu _\alpha (x)$ in $\overline{K}$, si ha un omomorfismo iniettivo $\varphi $ per il primo teorema di omomorfismo:
	\[
		\begin{tikzcd}
		K[x] & &  \overline{K}\\
		\\
		K(\alpha) \cong \displaystyle \frac{K[x]}{\langle \mu _\alpha (x) \rangle}
		\arrow[from=1-1, to=1-3, "\displaystyle \widetilde{\varphi }"]
		\arrow[from=1-1, to=3-1, two heads, "\displaystyle \pi_\alpha "']
		\arrow[from=3-1, to=1-3, hook, "\displaystyle \varphi "']
		\end{tikzcd}
	\] 
	giustificato dal fatto che, se $\mu _\alpha (x) \in \operatorname{Ker} \widetilde{\varphi }$, allora $\operatorname{Ker} \widetilde{\varphi }=\langle \mu _\alpha (x) \rangle$.
	Infine, ogni omomorfismo del genere \`e diverso da un altro perch\'e restituisce valori diversi quando calcolato, per esempio, su $x$, essendo relativo a radici distinte di $\mu _\alpha (x)$.
\end{proof}
\noindent Ora ci si pone un ulteriore problema: determinare il numero di radici di $\mu _\alpha (x)$ in $\overline{K}$.
Essendo $\overline{K}$ algebricamente chiuso, il numero di radici del polinomio, ciascuna contata con la relativa molteplicit\`a, deve essere pari a $\deg \mu _\alpha (x)$.
Il numero di radici multiple di $\mu _\alpha (x)$ si pu\`o ottenere tramite il seguente criterio.
\begin{teorema}
	[Criterio della derivata]
	Sia $f(x) \in K[x]$; allora $f(x)$ ha radici multiple in $\overline{K}$ se e solo se $\operatorname{gcd}(f(x),f'(x)) \neq 1$. 
	Inoltre, se $f(x)$ \`e irriducibile in $K[x]$, allora $f(x)$ ha radici multiple se e solo se $f'(x) = 0$.
\end{teorema}
\begin{proof}
	Si divide la dimostrazione per le due affermazioni.
	\begin{itemize}
		\item Sia $f(x)$ un polinomio con radici multiple $\alpha $ in $\overline{K}$; si mostra che $\operatorname{gcd}(f(x),f'(x)) \neq 1$.
			Se $f(x)$ ha radici multiple sulla chiusura algebrica, allora si scrive come $f(x) = (x-\alpha )^m g(x)$, con $g(\alpha ) \neq 0$ e $m\ge 2$.
			Allora 
			\[
				f'(x) = m (x-\alpha )^{m-1} g(x) + (x-\alpha )^m g'(x) = (x-\alpha )^{m-1} \big[m g(x) + (x-\alpha ) g'(x)\big]
			\] 
			\`e tale che $f'(\alpha ) = 0$ perch\'e $m-1\ge 1$.
			Allora il massimo comune divisore di $f$ e $f'$ \`e almeno $x-\alpha \neq 1$.

			Ora si assume che $d(x) = \operatorname{gcd}(f,f')\neq 1 $, cio\`e non \`e una costante; allora ammette almeno una radice $\alpha \in \overline{K}$, per cui $d(\alpha ) = 0 =\operatorname{gcd}(f(\alpha ),f'(\alpha )) \iff f(\alpha ) = f'(\alpha ) = 0$.
			Ma allora, per gli stessi passaggi al punto sopra, $f(x)$ deve avere almeno un fattore $(x-\alpha )^m$, con $m\ge 2$, altrimenti non si potrebbe avere anche $f'(\alpha ) = 0$, da cui la tesi.
		\item Per questo caso, si nota che $f(x) \in K[x]\Rightarrow f'(x) \in K[x]$, quindi $\operatorname{gcd}(f,f') \in K[x]$ (si ottiene tramite l'algoritmo di Euclide); se $f$ \`e irriducibile in $K[x]$, il gcd \`e $1$, oppure $f(x)$, ma quest'ultimo caso \`e verificato se e solo se $f'(x) = 0$, dovendo essere $\deg f' < \deg f$.
	\end{itemize}
\end{proof}
\begin{osservazione}
	Questo risultato permette di concludere che i polinomi irriducibili di $K[x]$ con derivata non-nulla hanno tante radici distinte, quanto vale il loro grado.
\end{osservazione}
\begin{definizione}
	[Campo perfetto]
	Un campo $K$ tale che ogni polinomio irriducibile di $K[x]$ ha derivata non-nulla \`e detto \textit{campo perfetto}.
\end{definizione}
\begin{osservazione}
Se $\operatorname{char}  K = 0$, allora $K$ \`e un campo perfetto.
Infatti, dato
\[
f(x) = \sum_{i=0}^{n} a_ix^i \implies f'(x) = \sum_{i=1}^{n} ia_ix^{i-1}
\] 
Allora $f'(x) = 0$ se e solo se ogni suo coefficiente \`e nullo: se $a_i\neq 0$, il termine $i a_i$ non pu\`o mai essere nullo perch\'e la caratteristica \`e zero.
\end{osservazione}
\begin{boxenv}[]
Per il seguito, si assumer\`a di avere a che fare esclusivamente con campi perfetti.
\end{boxenv}
\noindent Ora si estende il risultato ottenuto dalla prop. \ref{imext}, considerando un'immersione $K \longhookrightarrow \overline{K}$ generica e con l'assunzione di campo perfetto.
Questo nuovo risultato permetter\`a di contare il numero di estensioni di un omomorfismo qualsiasi da $K$ in $\overline{K}$ ad uno da $K(\alpha )$ in $\overline{K}$.
Per esempio, dato $K = \mathbb{Q}(\sqrt[3]{2}) $ e l'omomorfismo 
\[
	\varphi :K\longhookrightarrow \overline{K} : \sqrt[3]{2} \longmapsto \sqrt[3]{2} \zeta_3
\] 
si potr\`a dire esattamente quanti omomorfismi 
\[
\varphi _i : K(\zeta_3) \longhookrightarrow \overline{K}
\] 
con $\varphi_i |_K = \varphi $ esistono.
\begin{prop}
	[Estensioni di immersioni]
\label{imext2}
	Sia $\alpha  \in \overline{K}$, con $[K(\alpha ) : K] = n$; allora, $\forall \varphi : K \longhookrightarrow \overline{K}$, si hanno esattamente $n$ estensioni di $\varphi $ ad un'immersione da $K(\alpha )$ a $\overline{K}$, cio\`e:
	\[
	\exists \varphi _1,\ldots,\varphi _n : K(\alpha ) \longhookrightarrow  \overline{K}
	\] 
	tali che $\varphi _i|_K = \varphi ,\ \forall i = 1,\ldots,n$.
\end{prop}
\begin{proof}
	Similmente a quanto fatto per la prop. \ref{imext}, data $\varphi :K\longhookrightarrow \overline{K}$, si considera la sua estensione ad una mappa\footnote{Qui la notazione $\varphi (p(x))$ si riferisce all'applicare $\varphi $ ai coefficienti di $p(x)$, quindi $\big(\varphi (p(x))\big)\big(\beta \big)$ significa applicare $\varphi $ ai coefficienti di $p(x)$ e valutare il polinomio risultante in $\beta $.}
	\[
		\widetilde{\varphi }:
		\begin{array}
			{c c c}
			K[x]& \longrightarrow & \overline{K}\\
			x & \longmapsto & \beta \\
			p(x) & \longmapsto & \big(\varphi (p(x))\big)\big(\beta \big)
		\end{array}
	\] 
	Come fatto precedentemente, si richiede che $\langle \mu _\alpha (x) \rangle \subseteq \operatorname{Ker} \widetilde{\varphi }$ cos\`i da poter usare $K(\alpha ) \cong K[x]  / \langle \mu _\alpha (x) \rangle$ e avere l'immersione richiesta\footnote{Si nota che, una volta verificato tale contenimento, si ha, in realt\`a, l'uguaglianza perch\'e $\langle \mu _\alpha (x) \rangle$ \`e un ideale massimale, essendo in un PID, con $\mu _\alpha (x)$ irriducibile, quindi primo, e con un omomorfismo non nullo.}.
	Questa inclusione \`e equivalente a richiedere $\widetilde{\varphi }(\mu _\alpha (x)) = 0 \iff \varphi (\mu _\alpha (x))(\beta ) = 0$, cio\`e $\beta $ deve essere radice del polinomio $\varphi (\mu _\alpha (x))$, quindi ci saranno tante estensioni, quante sono le radici distinte di $\varphi (\mu _\alpha (x))$ in $\overline{K}$.
	Essendo $\mu _\alpha (x)$ irriducibile per definizione, allora $\varphi (\mu _\alpha (x))$ \`e ancora irriducibile e $\deg \mu _\alpha (x) = \deg \varphi (\mu _\alpha (x))$ (essendo l'omomorfismo iniettivo).
	Inoltre, il campo \`e perfetto, quindi il numero di radici distinte di $\varphi (\mu _\alpha (x))$ \`e pari al suo grado, quindi pari a quello di $\mu _\alpha (x)$.
\end{proof}
\noindent Tramite questa proposizione, si pu\`o dimostrare un risultato pi\`u generale in cui l'estensione dell'immersione parte da un'estensione di campo generica.
\begin{corollario}\label{imext3}
	Sia $E / K$ un'estensione, con $[E:K] = n$; allora $\forall \varphi  : K \longhookrightarrow \overline{K}$ immersione, esistono esattamente $n$ immersioni
	\[
	\varphi _1,\ldots,\varphi _n : E \longhookrightarrow \overline{K}
	\] 
	con $\varphi _i|_K = \varphi $.
\end{corollario}
\begin{proof}
	La dimostrazione segue per induzione.
	Per $n=2$, infatti, l'estensione \`e semplice e deriva dalla proposizione \ref{imext2} appena dimostrata.
	Per $n>2$, invece, si considera $\alpha  \in  E \setminus K$, per cui si ha la torre $K \subset K(\alpha ) \subset E$; se $[K(\alpha ):K] = m$ e $[E:K(\alpha )] = d$, allora $n = md$.
	\[
	\begin{tikzcd}
		 E \\
		 K(\alpha )\\
		 K
		 \arrow[from=3-1, to=2-1, no head, "m"']
		 \arrow[from=2-1, to=1-1, no head, "d"']
		 \arrow[from=3-1, to=1-1, no head, bend left=50, "n = md"]
	\end{tikzcd}
	\] 
	Se $m=n$, allora $E = K(\alpha )$ e si \`e ancora nel caso precedente con $n=2$ perch\'e l'estensione sarebbe semplice.
	Considerando $1<m<n$, allora $d < n$ e si pu\`o applicare la proposizione \ref{imext2} due volte di fila per dire che $\varphi $ si estende in $m$ modi a $K(\alpha )$:
	\[
	\exists \varphi _1,\ldots,\varphi _m : K(\alpha )\longhookrightarrow \overline{K} \hspace{.5cm} \text{ con } \hspace{.5cm} \varphi _i|_K = \varphi 
	\] 
	e, successivamente, ciascuna di queste $\varphi _i:K(\alpha ) \longhookrightarrow \overline{K}$ si estende, per ipotesi induttiva (visto che $[E:K(\alpha )] = d < n$):
	\[
	\exists \varphi _{i1} ,\ldots, \varphi _{i d } : E \longhookrightarrow \overline{K} \hspace{.5cm} \text{ con } \hspace{.5cm} \varphi _{ij} |_{K(\alpha )} = \varphi _i
	\] 
	quindi $\varphi _{ij} |_K = \varphi _i|_K = \varphi $ e si ha la tesi.
\end{proof}
\begin{definizione}
	[Elementi coniugati]
	Dato $\alpha  \in \overline{K}$, si definiscono i \textit{coniugati} di $\alpha $ su $K$ come le radici del polinomio minimo di $\alpha $ su $K$.
\end{definizione}
\begin{definizione}
	[Estensione separabile]
	Sia $K \subset L$ un'estensione algebrica; questa si dice \textit{separabile} se il polinomio minimo di ogni suo elemento \`e un \textit{polinomio separabile}, cio\`e se ha radici tutte distinte in un suo campo di spezzamento.
\end{definizione}
\begin{esempio}
	Sia $f(x) = x^3 - 2$; questo coincide con il polinomio minimo di $\alpha = \sqrt[3]{2} $ su $\mathbb{Q}$, $\mu _{\sqrt[3]{2}/\mathbb{Q} } (x)$.
	Si vogliono studiare le immersioni di $\mathbb{Q}(\alpha )$ in $\overline{\mathbb{Q}}$.
\end{esempio}
\begin{svolgimento}
	I coniugati di $\alpha $ sono $\alpha$, $\alpha \zeta_3$ e $\alpha \zeta_3^2$.
	Allora l'omomorfismo $\varphi :Q(\alpha ) \longhookrightarrow \overline{\mathbb{Q}}$, con $\varphi |_{\mathbb{Q}}= \mathrm{Id} _{\mathbb{Q}}$, ha le seguenti possibilit\`a:
	\[
...
\[
\varphi \big(\mathbb{Q}(\alpha )\big) = \mathbb{Q}\big(\varphi (\alpha )\big) = \begin{cases}
	\mathbb{Q}(\alpha )\\
	\mathbb{Q}(\alpha \zeta_3)\\
	\mathbb{Q}(\alpha \zeta_3^2)
\end{cases}
\] 
Allora si dice che $\mathbb{Q}(\alpha )$ \`e isomorfo \textit{su} $\mathbb{Q}$\footnote{Nel senso che \`e isomorfo tramite mappe che fissano $\mathbb{Q}$.} a questi tre distinti campi, in accordo con la proposizione \ref{imext2}.
\end{svolgimento}
\begin{esempio}
	[Polinomio ciclotomico p-esimo]
	Sia $p$ un primo; si considera il campo ciclotomico $p$-esimo $\mathbb{Q}(\zeta_p)$; si studiano le immersioni $\mathbb{Q}(\zeta_p )\hookrightarrow \overline{\mathbb{Q}}$. 
\end{esempio}
\begin{svolgimento}
	Si ha:
	\[
	\mu _{\zeta_p / \mathbb{Q}} (x) = \frac{x^p - 1}{x-1} = x^{p-1}+\ldots+ x+1
	\] 
	Questo \`e irriducibile perch\'e \`e il traslato di un $p$-Eisenstein, quindi i coniugati di $\zeta_p$ sono dati da $\zeta_p^i$, per $1\le i < p$.
	Allora $[\mathbb{Q}(\zeta_p) :\mathbb{Q}] = \varphi (p) = p-1$ e le immersioni sono della forma
	\[
	\varphi _i :\mathbb{Q}(\zeta_p) \longhookrightarrow \overline{\mathbb{Q}} : \zeta_p \longmapsto \zeta_p^i
	\] 
	con $\varphi _i|_{\mathbb{Q}} = \mathrm{Id} _{\mathbb{Q}}$.
Nuovamente, in accordo con la proposizione \ref{imext2}, si hanno $p-1$ possibili immersioni:
\[
\varphi _i\big(\mathbb{Q}(\zeta_p)\big) = \mathbb{Q}\big(\varphi _i(\zeta_p)\big) = \mathbb{Q}(\zeta_p^i) = \mathbb{Q}(\zeta_p), \ \forall i \in \left\{ 1,\ldots,p-1 \right\} 
\] 
\end{svolgimento}
\subsection{Estensioni normali}
\begin{definizione}
	[Estensione normale]
Sia $F / K$ un'estensione algebrica; questa si dice \textit{normale} se 
\[
\forall \varphi : F \longhookrightarrow \overline{K} \hspace{.5cm} \text{ con }\hspace{.5cm} \varphi |_K = \mathrm{Id} _K
\] 
si ha $\varphi (F) = F$, cio\`e l'estensione viene fissata da ogni immersione del campo di partenza nella sua chiusura algebrica.
\end{definizione}
\noindent Alcuni esempi di estensioni normali sono:
\begin{itemize}
	\item $\mathbb{Q}(\zeta_n) / \mathbb{Q}$, con $n$ generico;
	\item dato $F = \mathbb{Q}(\sqrt[3]{2},\zeta_3) $, $F / \mathbb{Q}$ \`e normale perch\'e, per $\varphi : F \longhookrightarrow \overline{\mathbb{Q}}$ con $\varphi |_{\mathbb{Q}}=\mathrm{Id} _{\mathbb{Q}}  $, si ha 
		\[
			\varphi \big(\mathbb{Q}(\sqrt[3]{2} , \zeta_3)\big) = \mathbb{Q}\big(\varphi (\sqrt[3]{2} , \varphi (\zeta_3)\big) = \mathbb{Q}(\sqrt[3]{2}\zeta_3^i, \zeta_3^j) = \mathbb{Q}(\sqrt[3]{2} , \zeta_3)
		\] 
		con $i \in \left\{ 0,1,2 \right\} $ e $j \in \left\{ 1,2 \right\} $, che sono $6$ immersioni (in accordo con il cor. \ref{imext3}) rispetto a cui $F$ \`e invariante.
\end{itemize}
\begin{prop}[Caratterizzazione delle estensioni normali]\label{nextcar}
	Sia $F / K$ un'estensione algebrica finita; allora le seguenti affermazioni sono equivalenti.
	\begin{enumerate}[(a).]
		\item $F / K$ \`e normale.
		\item Ogni polinomio irriducibile $f(x) \in K[x]$ che ha una radice in $F$ le contiene tutte in $F$.
		\item $F$ \`e il campo di spezzamento su $K$ di una famiglia di polinomi di $K[x]$.
	\end{enumerate}
\end{prop}
\begin{proof}
	Si verificano le varie equivalenze nei seguenti punti.
	\begin{itemize}
		\item (a)$\Rightarrow $(b). Sia $f(x) \in K[x]$ e sino $\alpha _1,\ldots, \alpha _n \in \overline{K}$ le sue radici.
			Per ipotesi, si sa che almeno una di queste, sia $\alpha _1$, appartiene a $F$, quindi $K(\alpha _1) \subset F$; si dimostra che, per $F / K$ normale, tutte le altre radici stanno ancora in $F$.
Si considerano le immersioni
\[
\varphi _i : K(\alpha _1) \longrightarrow K(\alpha _i) \subseteq \overline{K} : \alpha _1 \longmapsto \alpha _i
\] 
con $\varphi _i|_K = \mathrm{Id} _K$, $\forall i \in \left\{ 1,\ldots,n \right\} $, la cui esistenza \`e assicurata dalla proposizione \ref{imext}.
Ora, ciascuna di queste $\varphi _i$ si estendono ad un'immersione $F \longhookrightarrow \overline{K}$ in tanti modi quanto vale $[F : K(\alpha _1)]$, per il corollario \ref{imext3}; sia $\widetilde{\varphi }_i : F \longhookrightarrow \overline{K}$ tale che $\widetilde{\varphi }_i|_{K(\alpha _1)} = \varphi _i$ una di queste, relativa a $\varphi _i$.
Si nota che $\widetilde{\varphi }_i|_{K} = \varphi _i|_K = \mathrm{Id} _K$, visto che $K \subset K(\alpha _1)$, ma, per assunzione, $F / K$ \`e normale, quindi $\widetilde{\varphi }_i (F) = F$ e questo vale per ogni estensione di ogni $\varphi _i$.
Questo, per\`o, implica che ogni coniugato di $\alpha _1$ deve stare ancora in $F$, da cui si ha la tesi.
\item (b)$\Rightarrow $(c). Sia $F_0$ il campo di spezzamento, su $K$, della famiglia di polinomi
	\[
		\mathcal{F} = \left\{ \mu _\alpha (x) \in K[x]  \mid \alpha \in F \text{ e } \mu _\alpha (x) \text{ pol. minimo di } \alpha \text{ su } K \right\} 
	\] 
	Si ha, intanto, $F \subseteq F_0$ perch\'e $F_0$ contiene tutte le radici di tutti i polinomi minimi di tutti gli elementi di $F$; d'altra parte $F_0= K(\beta  \mid \beta \text{ radice di }\mu _\alpha (x) \in \mathcal{F} )$, con ogni $\mu _\alpha (x)$ irriducibile su $K[x]$ e almeno una radice di ognuno di questi \`e in $F$.
	Visto che si ha il punto (b) come ipotesi, $F$ contiene tutte le radici di ogni polinomio di $\mathcal{F} $, quindi si ha anche $F_0 \subseteq F$, per cui $F =F_0$.
\item (c)$\Rightarrow $(a). Sia $F$ il campo di spezzamento di una qualche famiglia di polinomi $\mathcal{F} =\left\{ f_1(x),\ldots,f_k(x) \right\} $; si dimostra che se $\varphi :F\longhookrightarrow \overline{K}$ soddisfa $\varphi |_K = \mathrm{Id} _K$, allora $\varphi (F) = F$.
	Indicando con $\left\{ \alpha _{ij}  \right\} _{j=1,\ldots,n_i} $, per $n_i = \deg f_i(x)$, le radici di $f_i(x)$, allora si pu\`o scrivere 
	\[
	F = K\big(\left\{ \alpha _{ij}  \right\}  \mid i=1,\ldots, k, \ j = 1,\ldots, n_i\big)
	\] 
	Si sa che, per ogni $i$, tale immersione deve soddisfare $\varphi (\alpha _{ij} ) = \alpha _{ij'} $, con $j' \in \left\{ 1,\ldots,n_i \right\} $, perci\`o
	\[
		\begin{split}
			\varphi (F) &= \varphi \Big( K\big(\left\{ \alpha _{ij}  \right\}  \mid i=1,\ldots, k, \ j = 1,\ldots, n_i\big) \Big)\\
				    &= K\big(\varphi (\alpha _{ij} )  \mid i=1,\ldots,k, \ j=1,\ldots,n_i\big) = F
		\end{split}
	\] 
	da cui la tesi.
	\end{itemize}
...
\end{prop}
\begin{proof}
	Sia $F/K$ un'estensione di $K$, con $\operatorname{char} K \neq 2$ e $[F:K] = 2$.
	Dato $\alpha  \in F \setminus K$, allora $F = K(\alpha )$, con polinomio minimo della forma
	\[
		\mu _\alpha (x) = x^2 + bx + c \in K[x]
	\] 
	e
	\[
	\alpha _{1,2} = \frac{-b \pm \sqrt{\Delta } }{2} \hspace{1cm} \text{ con } \hspace{1cm} \alpha = \frac{-b + \sqrt{\Delta } }{2}
	\] 
	quindi $F = K(\alpha ) = K(\sqrt{\Delta } )$. 
	Ma, allora, $\alpha _1, \ \alpha _2 \in K(\sqrt{\Delta } )$ e, dunque, $F$ \`e il campo di spezzamento di $\mu _{\alpha  / K} (x)$; per il punto (c) della proposizione \ref{nextcar}, $F / K$ \`e normale.
\end{proof}
\noindent \`E facile osservare, alla luce della proposizione \ref{nextcar}, che $\mathbb{Q}(\sqrt[3]{2} ) / \mathbb{Q}$ non \`e normale perch\'e, del polinomio minimo $f(x) = x^3 - 2$ (irriducibile su $\mathbb{Q}$), contiene solo una radice.
\begin{prop}
	Siano $F / K$ e $L / K$ due estensioni normali di $K$ nella chiusura $\overline{K}$; allora anche $FL / K$ e $F\cap L / K$ sono estensioni normali.
\end{prop}
\begin{proof}
	Si considera il diagramma 
	\[
	\begin{tikzcd}
		& & FL & &\\
		F & & & & L\\
		  & & F\cap L & &\\
		  & & K & &
		  \arrow[from=1-3, to=3-3, no head]
		  \arrow[from=1-3, to=2-1, no head]
		  \arrow[from=1-3, to=2-5, no head]
		  \arrow[from=3-3, to=2-1, no head]
		  \arrow[from=3-3, to=2-5, no head]
		  \arrow[from=3-3, to=4-3, no head]
		  \arrow[from=4-3, to=2-1, no head]
		  \arrow[from=4-3, to=2-5, no head]
	\end{tikzcd}
	\] 
	Il fatto che $FL / K$ sia normale segue direttamente dal fatto che l'immersione 
	\[
	\varphi : FL\longhookrightarrow \overline{K} \hspace{1cm} \text{ con }\hspace{1cm}\varphi |_K = \mathrm{Id} _K
	\] 
	deve essere un omomorfismo e, quindi
	\[
	\varphi (FL) = \varphi (F) \varphi (L) = FL
	\] 
	visto che gli elementi di $FL$ sono combinazioni polinomiali degli elementi di $F$ e di $L$, che sono estensioni normali per assunzione.
	Per l'intersezione, invece, si pu\`o vedere facilmente che ogni immersione deve rispettare
	\[
	\varphi (F\cap L) = \varphi (F) \cap \varphi (L) = F\cap L
	\] 
	perch\'e se mappasse un elemento dell'intersezione in un elemento che non sta nell'intersezione, ma solo in uno dei due campi, invaliderebbe immediatamente l'assunzione di normalit\`a di $F / K$ e $L / K$ perch\'e esisterebbe un'immersione che non fissa almeno uno dei due.
\end{proof}
\begin{prop}\label{normtor}
	Data una torre $K \subset F \subset L$ in una chiusura algebrica $\overline{K}$, se $L / K$ \`e normale, allora $L / F$ \`e normale.
\end{prop}
\begin{proof}
	Data $L / K$ estensione normale, si mostra che $L / F$ \`e normale, cio\`e 
	\[
	\forall \varphi : L \longhookrightarrow \overline{K} \hspace{1cm}\text{ con }\hspace{1cm} \varphi |_F = \mathrm{Id} _F
	\] 
	vale $\varphi (L) = L$.
	Ma $\varphi |_F = \mathrm{Id} _F \Rightarrow \varphi |_K = \mathrm{Id} _K$ e, per assunzione, si sa che ogni immersione $L \longhookrightarrow \overline{K}$ che fissa $K$, fissa anche $L$, quindi $\varphi (L) = L$ e si ha la tesi.
\end{proof}
\begin{osservazione}
	In generale, nella proposizione precedente (\ref{normtor}), $F / K$ non \`e normale.
	Siano, infatti, $K = \mathbb{Q}$, $F = \mathbb{Q}(\sqrt[3]{2} )$ e $L = \mathbb{Q}(\sqrt[3]{2}, \zeta_3) $; allora $L / K$ \`e normale perch\'e contiene tutte le radici del polinomio minimo dei due elementi, mentre $F / K$ ovviamente no.
\end{osservazione}
\begin{osservazione}
	Il viceversa della proposizione precedente (\ref{normtor}) \`e falso in generale; presa, infatti, la torre $\mathbb{Q} \subset  \mathbb{Q}(\sqrt{2} ) \subset \mathbb{Q}(\sqrt[4]{2}) $, entrambe le estensioni sono normali perch\'e di grado $2$, mentre la torre non lo \`e perch\'e $\mathbb{Q}(\sqrt[4]{2}) / \mathbb{Q} $ contiene solo due radici di $x^4 - 2$.
\end{osservazione}
\subsection{Teoria di Galois}
\begin{definizione}
	[Estensione di Galois]
	Un'estensione $E / K$ si dice \textit{di Galois} e \`e normale e separabile.
\end{definizione}
\begin{boxenv}[]
Avendo assunto di avere a che fare esclusivamente con campi perfetti, allora ogni estensione che si tratta \`e automaticamente separabile. 
Quindi, in questa assunzione, ci si pu\`o ridurre ad affermare che un'estensione normale \`e di Galois.
Inoltre, si considereranno unicamente estensioni di Galois finite.
\end{boxenv}
\noindent Visto che un'estensione di Galois $E / K$ deve essere normale, considerando l'insieme $\left\{ \varphi : E \longhookrightarrow \overline{K} \mid \varphi |_K = \mathrm{Id} _K \right\} $, si ha $\varphi (E) = E, \ \forall \varphi $; per questo motivo, si pu\`o restringere ciascuna di queste mappe $\varphi $ in modo da ottenere degli automorfismi e si pu\`o definire:
\[
	\operatorname{Aut} _K E = \left\{ \varphi  : E \stackrel{\sim}{\longrightarrow} E  \mid \varphi |_K = \mathrm{Id} _K \right\} 
\] 
Questi sono i $K$-automorfismi di $E$, cio\`e automorfismi di $E$ che fissano puntualmente $K$; equipaggiando questo insieme con l'operazione di composizione, si ottiene il \textit{gruppo di Galois} dell'estensione:
\begin{equation}
	\operatorname{Gal} (E / K ) : = \big(\operatorname{Aut} _K E , \circ\big)
\end{equation}
Per costruzione, alla luce del corollario \ref{imext3}, si ha:
\begin{equation}
	\lvert \operatorname{Gal} E / K \rvert = [E : K]
\end{equation}
Ora si dimostra che \`e effettivamente un gruppo.
\begin{proof}
	Per costruzione $\operatorname{Gal} E / K \subset \operatorname{Aut} E$, che \`e il gruppo degli automorfismi di $E$ rispetto alla composizione. 
	Allora basta mostrare che $\operatorname{Gal} E / K$ \`e un sottogruppo.
	Date $\varphi , \psi \in \operatorname{Gal} E / K$, la loro composizione \`e ancora un automorfismo e fissa ancora $K$, quindi rimane da mostrare l'esistenza dell'inverso.
	Sia, quindi, $\varphi \in \operatorname{Gal} E / K$; la mappa inversa $\varphi ^{-1}$ \`e ancora un automorfismo per definizione e deve fissare $K$ perch\'e 
	\[
	K=\mathrm{Id}(K) = \varphi ^{-1}\circ \varphi (K) = \varphi ^{-1}(K)
	\] 
	Quindi $\operatorname{Gal} E / K < \operatorname{Aut} E$.
\end{proof}
\begin{prop}\label{pg1}
	Sia $f(x) \in K[x]$ irriducibile e di grado $n$; se $F$ \`e il suo campo di spezzamento su $K$, allora
	\[
		n  \mid [F:K]  \mid n!
	\] 
	e $\operatorname{Gal} E / K \longhookrightarrow S_n$.
\end{prop}
\begin{proof}
	Siano $\alpha _1,\ldots, \alpha _n$ le radici di $f(x)$ in $\overline{K}$; allora $F = K(\alpha _1,\ldots, \alpha _n)$, perci\`o si ha la torre
	\[
		K \subseteq K(\alpha _1) \subseteq F \implies n = [K(\alpha _1):K] \mid [F:K] = \lvert \operatorname{Gal} F / K \rvert 
	\] 
	Ora si considera l'azione che restringe gli automorfismi di $\operatorname{Gal} F / K$ alle sole radici di $f(x)$:
	\[
	\phi :
		\begin{array}
			{c c c}
\operatorname{Gal} F / K &\longrightarrow &S\big(\left\{ \alpha _1,\ldots,\alpha _n \right\} \big)\cong S_n\\
\varphi &\longmapsto & \varphi |_{\left\{ \alpha _1,\ldots,\alpha _n \right\} } 
		\end{array}
	\] 
	Si dimostra che \`e un'azione ben definita, che \`e un omomorfismo e che \`e iniettiva.
	\begin{itemize}
		\item $\phi $ \`e ben definita perch\'e ogni $\varphi  \in \operatorname{Gal} F / K$ manda una radice in un suo coniugato, quindi \`e una permutazione; inoltre, \`e presente anche l'automorfismo identit\`a che manda ciascuna radice in s\'e.
		\item $\phi$ \`e un omomorfismo perch\'e
			\[
				\phi (\varphi \circ \psi )=(\varphi \circ \psi )_{\left\{ \alpha _1,\ldots,\alpha _n \right\} }  = \varphi _{\left\{ \alpha _1,\ldots,\alpha _n \right\} } \circ \psi |_{\left\{ \alpha _1,\ldots,\alpha _n \right\} } = \phi (\varphi )\circ \phi (\psi )
			\] 
			per ogni $\varphi ,\psi \in \operatorname{Gal} F / K$, dove la restrizione di $\varphi $ \`e data dal fatto che $\psi $ \`e ristretta alle radici e restituir\`a un'altra di queste radici.
		\item $\phi $ \`e iniettiva perch\'e
			\[
			\operatorname{Ker} \phi = \left\{ \varphi \in \operatorname{Gal} F / K  \mid \varphi (\alpha _i)= \alpha _i, \ \forall i \in \left\{ 1,\ldots,n \right\}  \right\} = \left\{ \operatorname{Id}  \right\} 
			\] 
			dove l'ultima uguaglianza \`e giustificata dal fatto che $\varphi $ genera tutti i generatori di $F / K$, quindi \`e univocamente determinata come l'identit\`a.
	\end{itemize}
	La tesi, allora, deriva dal fatto che $\operatorname{Gal} F / K \longhookrightarrow S_n$, quindi $\lvert \operatorname{Gal} F / K \rvert  \mid \lvert S_n \rvert = n!$.
\end{proof}
\begin{osservazione}
Con la dimostrazione precedente, si \`e anche evidenziato che il gruppo di Galois agisce fedelmente sulle radici.
Inoltre, agisce anche transitivamente perch\'e
\[
\operatorname{Orb} (\alpha _1) = \left\{ \varphi (\alpha _1)  \mid \varphi \in \operatorname{Gal} F / K \right\} = \left\{ \alpha _1,\ldots, \alpha _n \right\} 
\] 
Infatti, essendo $[K(\alpha _1) : K] = n$, allora si hanno esattamente $n$ immersioni che permutano le radici, ognuna delle quali \`e iniettiva e si pu\`o estendere a $F$, dunque tutte le radici sono raggiunte da una singola orbita.
\end{osservazione}
\noindent Sia $K$ un campo e sia $f(x) \in K[x]$ un polinomio irriducibile, con $F$ suo campo di spezzamento su $K$.
Si vuole studiare il gruppo di Galois di $F / K$ al variare del grado di $f(x)$.
\begin{itemize}
	\item Se $\deg f(x) = 2$, allora $[F:K]=2$ e, per la proposizione \ref{pg1}, si ha $\operatorname{Gal} F / K \cong \mathbb{Z}/2\mathbb{Z}$, quindi $\operatorname{Gal}F / K = \left\{ \mathrm{Id} , \varphi  \right\}  $.
		Visto che $f(x)$ ha grado due, allora $F = K(\sqrt{\Delta } )$ e le due mappe possibili sono:
		\[
		\mathrm{Id} :a + b \sqrt{\Delta } \longmapsto a + b\sqrt{\Delta }  \hspace{1cm} \varphi : a + b\sqrt{\Delta } \longmapsto a - b\sqrt{\Delta } 
		\] 
		Sostanzialmente, se le radici fossero $\alpha _1$ e $\alpha _2$, allora si avrebbe $\mathrm{Id} :\alpha _1 \longmapsto \alpha _1$ e $\varphi : \alpha _2 \longmapsto \alpha _2$.
	\item Se $\deg f(x) =3$, allora, sempre per la proposizione \ref{pg1}, si ha $3  \mid [F:K] \mid 6$ e $\operatorname{Gal} F / K \le S_3$.
		Quindi le opzioni sono:
		\[
		\operatorname{Gal} F / K \cong \begin{cases}
...
\end{itemize}
\begin{esempio}
	Sia $f(x) = x^3 -2 $, con $K = \mathbb{Q}$ e $F = \mathbb{Q}(\sqrt[3]{2} , \zeta_3)$.
	Allora $[F:K] = 6$, quindi $\operatorname{Gal} F / K \cong S_3$.
	I suoi elementi sono dati dall'estensione delle seguenti immersioni:
	\[
		\varphi (\sqrt[3]{2} ) =\sqrt[3]{2} \zeta_3^i \hspace{1cm} \varphi (\zeta_3) = \zeta_3^j
	\] 
	al variare di $i \in \left\{ 0,1,2 \right\} $ e $j \in \left\{ 1,2 \right\} $.
\end{esempio}
...
\end{prop}
\begin{proof}
	Si nota che, per ogni immersione $\varphi : \mathbb{F}_{q^d} \longhookrightarrow \overline{\mathbb{F}}_p$ tale che $\varphi |_{\mathbb{F}_q} = \mathrm{Id} _{\mathbb{F}_q} $, $\varphi (\mathbb{F}_{q^d} )$ \`e un sottocampo di $\overline{\mathbb{F}}_p$ con $q^d = p^{rd} $ elementi, visto che $\varphi $ \`e iniettiva.
	Ora, valendo l'unicit\`a dei campi con $p^n$ elementi, deve valere necessariamente $\varphi (\mathbb{F}_{q^d} ) = \mathbb{F}_{q^d} $, quindi l'estensione \`e normale.
	D'altra parte, si ricorda che le estensioni $\mathbb{F}_{p^n} $ di campi finiti $\mathbb{F}_p$ si costruiscono come campi di spezzamento dei polinomi $x^{p^n} -x \in \mathbb{F}_p[x]$ al variare di $n$, quindi sono sempre normali per costruzione.
\end{proof}
\begin{corollario}
	Tutte le estensioni di campi finiti sono normali.
\end{corollario}
\begin{proof}
	Si considera il seguente diagramma\footnote{In {\color{nred}rosso} sono evidenziate le estensioni normali.}:
	\[
		\begin{tikzcd}
	\mathbb{F}_{p^n} = \mathbb{F}_{q^d} \\
	\mathbb{F}_{p^r} = \mathbb{F}_{q} \\
	\mathbb{F}_p
	\arrow[from=3-1, to=2-1, no head]
	\arrow[from=3-1, to=1-1, bend left = 50, no head, color=nred]
	\arrow[from=2-1, to=1-1, no head, color=nred]
		\end{tikzcd}
	\] 
	Per quanto affermato nella proposizione precedente, $\mathbb{F}_{p^n} / \mathbb{F}_p$ \`e sempre normale, quindi, per la proposizione \ref{normtor}, tutte le estensioni di campi finiti sono normali.
\end{proof}
\begin{definizione}
	[Automorfismo di Frobenius]
	Viene chiamato \textit{automorfismo di Frobenius} l'automorfismo
	\[
		\phi :
		\begin{array}
			{c c c}
...
\end{definizione}
\begin{teorema}
	Il gruppo di Galois dell'estensione $\mathbb{F}_{q^d} /\mathbb{F}_q$, con $q = p^r$, \`e generato dall'automorfismo di Frobenius $\phi :\mathbb{F}_{q^d} \stackrel{\sim}{\longrightarrow} \mathbb{F}_{q^d} : x \longmapsto x^q$.
\end{teorema}
\begin{proof}
	Intanto si verifica che $\phi  \in \operatorname{Gal} \mathbb{F}_{q^d} /\mathbb{F}_q$. 
	Si vede che $\phi $ \`e un omomorfismo di anelli:
	\[
	\begin{split}
		&\phi (\alpha +\beta ) = (\alpha +\beta )^q = \alpha ^q + \beta ^q = \phi (\alpha ) + \phi (\beta ) \\
		&\phi (\alpha \beta )= (\alpha \beta )^q = \alpha ^q \beta ^q = \phi (\alpha )\phi (\beta ) 
	\end{split}\hspace{.25cm}, \hspace{.5cm}\forall \alpha ,\beta  \in \mathbb{F}_q^d
	\] 
	dove si \`e usato che si \`e in caratteristica $p$, con $q= p^r$, pertanto tutti i termini del binomio che non sono relativi a potenze massime di uno dei due addenti si eliminano perch\'e moltiplicati per un multiplo di $p$ (Lemma del Binomio Ingenuo).

	Inoltre, \`e biettivo perch\'e, dato $\phi :\alpha \longmapsto \alpha ^q$, si ha $\phi ^d (\alpha ) = \alpha ^{q^d}= \alpha$, quindi $\phi ^{-1}=\phi ^{d-1} $.
	Altrimenti, si pu\`o dimostrare che \`e iniettivo osservando che $\alpha ^q = \beta ^q\Rightarrow (\alpha -\beta )^q = 0 \iff \alpha - \beta = 0 \iff \alpha = \beta $, quindi \`e automaticamente anche suriettivo perch\'e il dominio \`e finito.

Ora, dato un generico $\alpha \in \mathbb{F}_q$, questo soddisfa $\phi (\alpha )= \alpha ^q = \alpha $, pertanto $\phi $ \`e un automorfismo di $\mathbb{F}_{q^d} $ che fissa $\mathbb{F}_q$, cio\`e tale che $\phi |_{\mathbb{F}_q}= \mathrm{Id} _{\mathbb{F}_q} $, quindi $\phi \in \operatorname{Gal} \mathbb{F}_{q^d} /\mathbb{F}_q$.
Inoltre, $\operatorname{Gal} \mathbb{F}_{q^d} / \mathbb{F}_q$ \`e un gruppo di ordine $d$, visto che l'estensione \`e di ordine $d$, quindi deve valere $\operatorname{ord}(\phi ) = k  \mid d$; d'altra parte, se $\phi ^k = \mathrm{Id} $:
\[
\phi ^k(\alpha ) = \alpha ^{q^k} = \alpha , \ \forall \alpha \in \mathbb{F}_{q^d} 
\] 
cio\`e $f(x) = x^{q^k} -x$ ha, come radici, tutti i $q^d$ elementi di $\mathbb{F}_{q^d} $, quindi 
\[
\deg f(x) = q^k \ge q^d = \lvert \mathbb{F}_{q^d}  \rvert \implies k\ge d
\] 
perci\`o deve essere $ k =d$.
Visto che $\phi $ ha proprio ordine $d$ ed \`e un elemento del gruppo, allora lo genera e si ha la tesi.
\end{proof}
\begin{esempio}
Si studia l'estensione $E = \mathbb{Q}(\sqrt{2} , \sqrt{3} ) / \mathbb{Q}$ e il suo gruppo di Galois.
\end{esempio}
\begin{svolgimento}
	Le estensioni, via identit\`a, $\varphi : E \longhookrightarrow \overline{\mathbb{Q}}$ sono determinate dalle immagini di $\sqrt{2} $ e $\sqrt{3} $, cio\`e sono:
	\begin{center}
\begin{minipage}
	{0.45\columnwidth}
	\[
...
\begin{minipage}
	...
	&  E  & \\
		\mathbb{Q}(\sqrt{3} )  & &  \mathbb{Q}(\sqrt{2} )\\
				      ...
				      \arrow[from=1-2, to=3-2, no head, "\displaystyle 4"]
				      \arrow[from=3-2, to=2-1, no head, "\displaystyle 2"]
				      \arrow[from=3-2, to=2-3, no head, "\displaystyle 2"']
	\end{tikzcd}
	\] 
\end{minipage}
	\end{center}
	Il composto e le due estensioni semplici sono normali perch\'e di grado $2$.
	Le possibili immersioni sono
	\[
\begin{array}
	{l l}
	\mathrm{Id} = \varphi _1 = \begin{cases}
		\sqrt{2} \longmapsto \sqrt{2} \\
		\sqrt{3} \longmapsto \sqrt{3} 
		\end{cases} \hspace{.5cm}&\varphi _2 = \begin{cases}
		\sqrt{2} \longmapsto - \sqrt{2} \\
		\sqrt{3} \longmapsto  \sqrt{3} 
	\end{cases} 	\\
\varphi _3 = \begin{cases}
		\sqrt{2} \longmapsto \sqrt{2} \\
		\sqrt{3} \longmapsto -\sqrt{3} 
	\end{cases} & \varphi _4 = \begin{cases}
		\sqrt{2} \longmapsto -\sqrt{2} \\
		\sqrt{3} \longmapsto - \sqrt{3} 
	\end{cases}
\end{array}
\] 
Si vede, allora, che tutti gli elementi (tranne ovviamente l'identit\`a) hanno ordine $2$ e sono $4$ in totale, quindi $\operatorname{Gal} \mathbb{Q}(\sqrt{2} , \sqrt{3} ) / \mathbb{Q} \cong \mathbb{Z}/ 2\mathbb{Z} \times \mathbb{Z}/2\mathbb{Z}$.	
Infine, si osserva, per doppio contenimento, che $\mathbb{Q}(\sqrt{2} ,\sqrt{3} ) = \mathbb{Q}(\sqrt{2} +\sqrt{3} )$.
\end{svolgimento}
\noindent Nell'ultima parte dell'esercizio precedente, si \`e visto come l'estensione $\mathbb{Q}(\sqrt{2} ,\sqrt{3} )$ si potesse esprimere tramite l'estensione semplice $\mathbb{Q}(\sqrt{2} +\sqrt{3} )$; questo si generalizza nel seguente teorema\footnote{Dimostrato a pagina 130.}.
\begin{teorema}
	[Teorema dell'elemento primitivo]
	Sia $K$ un campo e $E / K$ una sua estensione finita e separabile; allora $E / K$ \`e semplice, cio\`e $\exists \gamma\in E : E = K(\gamma)$.
\end{teorema}
\begin{proof}
	Si distinguono i casi di campo $K$ finito e infinito.
	\begin{itemize}
		\item $K$ campo finito.

			In questo caso, avendo per ipotesi $E / K$ \`e finita, allora $E$ \`e finito, per cui il sottogruppo moltiplicativo di $E$, indicato con $E^*$, \`e, a sua volta, finito e, quindi, ciclico con $E^* = \langle \gamma \rangle$.
			Pertanto, $E = K(\gamma)$.
		\item $K$ campo infinito.

			Per ipotesi, $E / K$ \`e finita, quindi, per la prop. \ref{efgag}, $E = K(\alpha _1,\ldots,\alpha _n)$.
			Si mostra, per induzione, che $E / K$ \`e semplice.
			Si considera solo il caso di $n=2$ ($n=1$ \`e banale) perch\'e, per i casi successivi, sar\`a sufficiente aggiungere un elemento alla volta e usare il caso precedente.
			Allora, per $n=2$, si ha $E = K(\alpha ,\beta )$ e sia $[E : K] = m$; allora, per il corollario \ref{imext3}:
			\[
			\exists \varphi _1,\ldots,\varphi _m : E \longhookrightarrow \overline{K} \hspace{1cm} \text{ con }\hspace{1cm} \varphi _i|_K = \mathrm{Id} _K
			\] 
			Si definisce il polinomio 
			\[
				F(x) = \prod_{i<j} \big[\varphi _i(\alpha ) + x\varphi _i(\beta ) - \varphi _j(\alpha ) - x \varphi _j(\beta )\big]\in \overline{K}[x]
			\] 
			che \`e come applicare $\varphi _i$ al polinomio $\alpha + \beta x$ e sottrargli $\varphi _j$ applicata allo stesso.
			Il polinomio $F(x)$ consta del prodotto di polinomi di grado al pi\`u $1$, pertanto si potr\`a affermare che 
			\[
				\deg F(x) \le \binom{m}{2}
			\] 
			cio\`e ha grado che vale al massimo quanto il numero dei fattori, dato proprio dal binomiale.
			Inoltre, $F(x) \neq 0$ perch\'e, altrimenti, si avrebbe $\varphi _i(\alpha ) + x \varphi _i(\beta ) = \varphi _j(\alpha ) + x \varphi _j (\beta )$, da cui
			\[
				x \big[\varphi _i(\beta ) - \varphi _j(\beta )\big] + \big[\varphi _i(\alpha ) - \varphi _j(\alpha )\big] = 0
			\] 
			perci\`o si avrebbe $\varphi _i(\beta ) = \varphi _j(\beta )$ e $\varphi _i(\alpha ) = \varphi _j(\alpha )$, da cui $\varphi _i = \varphi _j$ (cio\`e $i=j$) perch\'e $E = K(\alpha ,\beta )$ e le immersioni coincidono sui generatori. 
			Questo, per\`o, \`e assurdo perch\'e si stanno moltiplicando fattori con $i<j$.
			Per questo discorso, $F(x)$ ha esattamente $\deg F(x)$ radici in $\overline{K}$, visto che non \`e identicamente nullo per quanto appena detto; allo stesso tempo, $K$ \`e infinito, quindi $\exists  t \in K$ tale che $F(t)\neq 0$, visto che c'\`e un numero finito di radici.
			Dunque:
			\[
				F(t) = \prod_{i<j} \big[\varphi _i(\alpha ) + t\varphi _i(\beta ) - \varphi _j(\alpha ) - t \varphi _j(\beta )\big]\neq 0
			\] 
			il che vuol dire che $\varphi _i(\alpha +t\beta ) \neq \varphi _j(\alpha +t\beta ), \ \forall i\neq j$\footnote{Si \`e rinominato $\varphi _i(\alpha ) + x \varphi _i(\beta ) = \varphi _i(\alpha  + x \beta )$.}.
			Questo indica che $\gamma = \alpha  + t \beta $ ha $m$ coniugati, visto che si hanno $m$ immersioni distinte, perci\`o $[K(\gamma) : K] = m$ e, quindi, $E = K(\gamma)$, visto che le due estensioni hanno lo stesso grado.
	\end{itemize}
\end{proof}
...
\begin{definizione}
	...
	\] 
	come il sottocampo di $L$ i cui elementi vengono lasciati fissi rispetto ad ogni $K$-automorfismo di $H$.
\end{definizione}
\noindent Ora si verifica che $L^H$ della definizione precedente \`e un sottocampo.
\begin{proof}
	Intanto, si mostra che \`e un sottoanello. 
	Sicuramente $0,1 \in L^H$ per definizione di automorfismo; inoltre \`e chiuso rispetto alla somma e al prodotto perch\'e ogni $\varphi \in H$ \`e, in particolare, un omomorfismo.
	Per lo stesso motivo, anche gli inversi rispetto al prodotto e alla moltiplicazione sono lasciati fissati dagli elementi di $H$.
	Visto che \`e un sottoinsieme di un campo, come anello, \`e un dominio e la presenza degli inversi moltiplicativi garantisce che \`e un campo.
\end{proof}
\noindent Si nota che, per definizione:
\begin{equation}
	K\subseteq L^H \subseteq L
\end{equation}
dove la seconda inclusione deriva per costruzione, mentre la prima deriva dal fatto che ogni elemento di $H$ fissa, per definizione, gli elementi di $K$.
\begin{lemma}\label{lcg1}
	Sia $L / M$ un'estensione di Galois e $H \le \operatorname{Gal} L / M$; allora
	\[
...
\begin{proof}
	...
	\begin{itemize}
		\item ($\Leftarrow$) Si prende $G = \operatorname{Gal} L / M$ e si fa vedere che $L^G = M$.
			Se $M \subsetneq L^G$, allora $[L^G:M] > 1$ ed esiste almeno un'immersione non banale (diversa dall'identit\`a)
			\[
			\varphi :L^G \longhookrightarrow \overline{M} \hspace{1cm} \text{ con }\hspace{1cm} \varphi |_M = \mathrm{Id} _M
			\] 
			Per il corollario \ref{imext3}, si trova
			\[
			\widetilde{\varphi }:L\longhookrightarrow \overline{M}\hspace{1cm} \text{ con }\hspace{1cm} \widetilde{\varphi }|_{L^G} =\varphi 
			\] 
			con $\widetilde{\varphi }|_M = \varphi |_M = \mathrm{Id} _M$.
			Allo stesso tempo, $L / M$ \`e normale, quindi $\widetilde{\varphi} (L) = L$, quindi $\widetilde{\varphi }\in \operatorname{Gal} L / M$ e, dunque, $\widetilde{\varphi }(L^G) = L^G$, che \`e assurdo perch\'e si era assunto che il grado dell'estensione $L^G / M$ fosse maggiore di $1$, quindi con l'esistenza di almeno un elemento non fissato da $\widetilde{\varphi }$.
		\item ($\Rightarrow $) Si mostra che il campo base \`e proprio quello fissato da tutto il gruppo di Galois.
			Visto che $L / M$ \`e un'estensione finita, per il teorema dell'elemento primitivo, si ha $L = M(\alpha )$; dato $H\le G$ un campo che fissa solo il campo base, cio\`e $L^H = M$, si considera, allora:
			\[
			f(x) = \prod_{\sigma \in H} \big(x - \sigma (\alpha )\big) \hspace{1cm}\text{ con } \hspace{1cm} \deg f(x) = \lvert H \rvert 
			\] 
			Questo si annulla in $\alpha $ e sta in $L^H[x]$; infatti, dato $\rho \in H$:
			\[
			\rho \big(f(x)\big) = \prod _{\sigma \in H}  \big(x - \rho (\sigma (\alpha ))\big) = \prod_{\sigma \in H} \big(x - \sigma (\alpha )\big) = f(x)
			\] 
			dove la penultima uguaglianza deriva dal fatto che $\rho \in H$, quindi permuta gli elementi dell'insieme senza alterarne il prodotto.
			Allora si nota che:
			\[
				\lvert G \rvert =[L:M] = [M(\alpha ):M] = \deg \mu_{ \alpha  / M} (x) \le \deg f(x) = \lvert H \rvert 
			\] 
			visto che $f(x)\in L^H[x] =M[x]$ (perch\'e $L^H = M$ per ipotesi) e $f(\alpha ) = 0 \Rightarrow  \mu _\alpha (x)  \mid f(x)$, per cui $\lvert G  \rvert \le H$ e, quindi, $H = G$.
	\end{itemize}
\end{proof}
\begin{lemma}\label{lcg2}
	Sia $L / K$ un'estensione di Galois e $H < \operatorname{Gal} L / K$; data $\sigma  \in \operatorname{Gal} L / K$, allora
	\[
	L^{\sigma H\sigma ^{-1}}= \sigma (L^H)
	\] 
\end{lemma}
\begin{proof}
	Visto che $L^H = \left\{ \alpha \in L  \mid \varphi (\alpha ) = \alpha , \ \forall \varphi \in H \right\} $, si ha:
	\[
		\begin{split}
			\sigma (L^H) &= \left\{ (\sigma (\alpha )  \mid \alpha \in L^H \right\} = \big\{ \underbracket{\sigma (\alpha )}_{=\beta }   \mid \varphi (\alpha ) = \alpha , \ \forall \varphi \in H \big\} \\
				     &= \left\{ \beta \in L  \mid (\varphi \circ \sigma ^{-1})(\beta ) = \sigma ^{-1}(\beta ) , \ \forall \varphi  \in H \right\}\\
				     &= \left\{ \beta \in L  \mid (\sigma \circ \varphi \circ \sigma ^{-1}) (\beta ) = \beta , \ \forall \varphi  \in H \right\} = L^{\sigma H \sigma ^{-1}} 
		\end{split}
	\] 
\end{proof}
\begin{osservazione}
In generale, $\sigma (L^H) \neq L^H$, ma \`e sempre una sottoestensione di $L / K$, visto che $L$ \`e sempre fissato.
In generale, si ha il seguente diagramma
\[
	\begin{tikzcd}
&L&\\
	L^H&&\sigma (L^H)\\
	   ...
	   \arrow[from=1-2, to=2-1, no head]
	   \arrow[from=1-2, to=2-3, no head]
	   \arrow[from=1-2, to=3-2, no head]
	   \arrow[from=3-2, to=2-1, no head]
	   \arrow[from=3-2, to=2-3, no head]
	\end{tikzcd}
\] 
Se valesse $H\lhd G$, allora si avrebbe proprio $L^H = L^{\sigma H\sigma ^{-1}} = \sigma (L^H), \ \forall \sigma \in \operatorname{Gal} L / K$.
\end{osservazione}
\begin{teorema}
	[Teorema di corrispondenza di Galois]
Sia data $L / K$ un'estensione di Galois finita; allora l'insieme delle sottoestensioni di $L / K$ e l'insieme dei sottogruppi di $\operatorname{Gal} L / K$ sono in corrispondenza biunivoca.
Inoltre, $H \lhd G\iff L^H / K$ \`e normale e, in tal caso:
\[
\operatorname{Gal} L^H / K \cong \frac{\operatorname{Gal} L / K}{\operatorname{Gal} L / L^H} = G / H
\] 
\end{teorema}
\begin{proof}
	Si definiscono $\mathcal{E} _{L / K} = \left\{ F  \mid  K\subseteq F \subseteq L \right\} $ l'insieme delle sottoestensioni di $L / K$ e $\mathcal{G} _{L / K} = \left\{ H < \operatorname{Gal} L / K \right\} $ l'insieme dei sottogruppi del gruppo di Galois di $L / K$.
	La biezione tra questi due \`e ottenuta tramite le applicazioni
	\[
	\begin{split}
		&\alpha : \mathcal{E} _{L / K} \longrightarrow \mathcal{G} _{L / K}  : F \longmapsto \operatorname{Gal} L / F\\
		& \beta : \mathcal{G} _{L / K} \longrightarrow \mathcal{E} _{L / K} : H \longmapsto L^H
	\end{split}
	\] 
	dove $L / F$ \`e un'estensione di Galois (cio\`e un'estensione normale, visto che si considerano campi perfetti) per la proposizione \ref{normtor}, quindi $\alpha $ \`e ben definita; si nota, inoltre, che $\operatorname{Gal} L / F < \operatorname{Gal}  L / K$ perch\'e un $K$-automorfismo di $F$ che lo fissa, fissa puntualmente anche $K$, quindi tutti gli automorfismi di $\operatorname{Gal} L / F$ stanno anche in $\operatorname{Gal} L / K$.
Poi, anche $\beta $ \`e ben definita perch\'e si \`e dimostrato che $L^H$ \`e un campo e vale $K \subseteq L^H \subseteq L$.
Ora si verifica che le due mappe, effettivamente, definiscano una biezione; intanto si vede che:
\[
\beta \circ \alpha (F) = \beta \left(\operatorname{Gal} L / F\right) = L^{\operatorname{Gal} L / F} = F, \ \forall F \in \mathcal{E} _{L / K} 
\] 
dove, per l'ultima uguaglianza, si \`e usato il lemma \ref{lcg1}.
Viceversa:
\[
\alpha \circ \beta (H) = \alpha (L^H) = \operatorname{Gal} L / L^H = H , \ \forall H \in \mathcal{G} _{L / K} 
\] 
Infatti, tutti gli elementi di $L^H$ sono fissati dagli elementi di $H$ (per definizione), quindi $H \le \operatorname{Gal} L / L^H$; allo stesso tempo, avendo $H \le \operatorname{Gal} L / L^H$, per il lemma \ref{lcg1}, essendo il campo fissato da $H$ quello base, deve valere proprio $H = \operatorname{Gal} L / L^H$.

Ora si dimostra la seconda parte del teorema.
Si sa che
\[
H \lhd \operatorname{Gal} L / K \iff \sigma H \sigma ^{-1} = H, \ \forall \sigma \in \operatorname{Gal} L / K
\] 
quindi, per il lemma \ref{lcg2}, $H \lhd \operatorname{Gal} L / K$ \`e equivalente a richiedere che
\[
\sigma (L^H) = L^{\sigma H \sigma ^{-1}} = L^H, \ \forall \sigma \in \operatorname{Gal} L / K
\] 
Questo significa che $L^H / K$ \`e normale; infatti, la condizione di normalit\`a corrisponde al fatto che $\forall \psi : L^H \longhookrightarrow \overline{K}$, con $\psi |_K = \mathrm{Id} _K$, si ha anche $\psi (L^H) = L^H$, per\`o ogni $\psi $ si estende ad un'immersione $L \longhookrightarrow \overline{K}$ (per il corollario \ref{imext3}, quindi
\[
\forall \psi \in \operatorname{Gal} L^H / K , \ \exists \sigma \in \operatorname{Gal} L / K : \sigma |_{L^H} =\psi 
\] 
Per finire, si verifica che $\operatorname{Gal} L^H / K \cong G / H$.
Si considera l'omomorfismo 
\[
\phi :\operatorname{Gal} L/K \longrightarrow \operatorname{Gal} L^H / K : \varphi \longmapsto \varphi |_{L^H} 
\] 
Questo \`e suriettivo perch\'e ogni $\psi \in \operatorname{Gal} L^H / K$ si estende ad $L$; inoltre:
\[
	\begin{split}
		\operatorname{Ker} \phi &= \left\{ \varphi \in \operatorname{Gal} L / K  \mid  \varphi |_{L^H} = \mathrm{Id} _{L^H}  \right\} = \left\{ \varphi \in \operatorname{Gal} L / K  \mid \varphi (\alpha ) = \alpha , \ \forall \alpha \in L^H \right\} \\
					& = \operatorname{Gal} L / L^H = H
	\end{split}
\] 
Quindi si ha la tesi per il primo teorema di omomorfismo.
\end{proof}
\noindent Il teorema di corrispondenza si pu\`o, pi\`u o meno, riassumere nel considerare la seguente torre:
\[
\begin{tikzcd}
	L\\
	L^H \\
	K
	\arrow[from=1-1,to=2-1, no head, "H", color=nred]
	\arrow[from=3-1,to=2-1, no head, "G / H"', color=nred]
	\arrow[from=3-1,to=1-1, no head, "G", color=nred, bend left = 50]
\end{tikzcd}
\] 
Nel diagramma, $G$ \`e il gruppo di Galois di $L / K$, mentre $H < G$ \`e il sottogruppo di Galois dell'estensione $L / L^H$, anch'essa di Galois\footnote{Visto che $L / K$ normale per assunzione e, quindi, si pu\`o applicare la proposizione \ref{normtor}}.
Inoltre, se $H \lhd G$, allora anche $L^H / K$ \`e di Galois, con gruppo di Galois dato da $G / H$.
\begin{prop}
	[Propriet\`a della corrispondenza]
	Siano $H , S \le \operatorname{Gal} L / K$; allora valgono i seguenti punti:
	\begin{enumerate}[(a).]
...
\begin{proof}
	Si divide la dimostrazione nei vari punti.
	\begin{enumerate}[(a).]
		\item Sia $\sigma \in H$, con $L^H \supseteq L^S$; allora $\sigma $ fissa anche tutti gli elementi di $L^S$, per cui $\sigma \in S, \ \forall \sigma \in H$ e, quindi $H\le S$.
			Viceversa, visto che ogni elemento di $H$ \`e anche di $S$, allora tutti gli elementi di $H$ fissano anche quelli di $L^S$.
		\item Si nota che $H\cap S \subseteq H,S$, quindi $L^{H\cap S} \supseteq L^H,L^S$ per il punto precedente.
			Visto che $L^{H\cap S} $ contiene sia $L^H$, che $L^S$, allora deve contenerne anche il composto, quindi $L^{H\cap S} \supseteq L^{H} L^S$.
			D'altra parte, $L^H L^S \subseteq L$, quindi $\exists N\le \operatorname{Gal} L / K$ tale che $L^H L^S = L^N$; essendo il composto dei due campi pi\`u grande dei singoli, per il teorema di corrispondenza:
			\[
				\operatorname{Gal} \faktor{L}{L^H L^S}= \operatorname{Gal} L / L^N = N \subseteq \Big(\operatorname{Gal} L / L^H \cap \operatorname{Gal} L / L^S\Big) = H \cap S
			\] 
			da cui $N \subseteq H\cap S$, che \`e equivalente a chiedere $L^HL^S \supseteq L^{H\cap S} $, quindi si ha la tesi.
		\item Visto che $H , S \subseteq \langle H,S \rangle$, allora $L^S , L^H \supseteq L^{\langle H,S \rangle} $, quindi $L^{\langle H,S \rangle} \subseteq L^H \cap L^S$, essendo contenuto in entrambi.
			Viceversa, dato $\alpha \in L^H \cap L^S$, si ha $\varphi (\alpha ) = \alpha, \ \forall \varphi \in H, S$, quindi $\alpha $ \`e fissato da tutti gli elementi di $\langle H,S \rangle$ e $L^H \cap L^S \subseteq L^{\langle H,S \rangle} $.
	\end{enumerate}
\end{proof}
\subsection{Discriminante polinomiale}

\begin{definizione}
	[Discriminante]
	Sia $p(x) \in K[x]$.
	Se $\deg p(x)=n$ e $\alpha _1,\ldots,\alpha _n\in \overline{K}$ sono le sue radici, si definisce il \textit{discriminante polinomiale} di $p(x)$ come:
	\[
		\operatorname{disc} p(x) = \prod_{i<j} (\alpha _i-\alpha _j)^2 \in K[\alpha _1,\ldots,\alpha _n]
	\] 
\end{definizione}
\begin{prop}
	Dato $p(x) \in K[x]$, questo ha radici multiple se e solo se $\operatorname{disc} p(x) = 0$.
\end{prop}
\noindent Si nota che l'annullamento di $\operatorname{disc} p$ \`e indipendente dal coefficiente di testa $a_n$ del polinomio; infatti, polinomi associati condividono le stesse radici.

Nell'esprimere il discriminante in termini dei coefficienti di $p(x)$, per\`o, qualora questo non fosse monico, comparirebbe un termine $a_n^{2n-2} $ al denominatore del discriminante.
Per esempio, se $p(x) = ax^2 + bx + c$, il discriminante \`e dato da $\operatorname{disc} p(x) = \frac{b^2 - 4ac}{a^2}$ (es. \ref{e36}), pertanto risulta naturale ridefinire il discriminante come:
\begin{equation}
	\operatorname{Disc} p(x) = a^{2n-2} \operatorname{disc} p(x) = a^{2n-2} \prod_{i<j} (\alpha _i-\alpha _j)^2
\end{equation}
In questo modo, $\operatorname{Disc} (ax^2 + bx+c) = b^2 - 4ac = \Delta $.
\begin{prop}
Il discriminante \`e invariante per traslazioni.
\end{prop}
\begin{proof}
	Sia $p(x) \in K[x]$. 
	Dato $a \in K$, le radici di $p_a(x) := p(x+a)$ sono $\alpha _1 - a, \ldots, \alpha _n-a$, quindi:
	\[
	\operatorname{disc} p_a = \prod_{i<j} (\alpha _i - a - \alpha _j + a)^2 = \prod_{i<j} (\alpha _i-\alpha _j)^2 = \operatorname{disc} p(x)
	\] 
\end{proof}
\begin{esempio}
	[Discriminante per polinomi di grado 2]\label{e36}
	Sia $p(x) =ax^2 + bx + c$, con $a\neq 0$.
	Date $\alpha _1, \alpha _2$ le sue radici in $\overline{K}$, allora:
	\[
	\operatorname{disc} p(x) = (\alpha _1-\alpha _2)^2 = \alpha _1^2 + \alpha _2^2 - 2 \alpha _1 \alpha _2 = (\alpha _1 + \alpha _2)^2 - 4 \alpha _1 \alpha _2 = \frac{b^2}{a^2}- 4 \frac{c}{a}=\frac{\Delta }{a^2}
	\] 
	dove si \`e usato che $\alpha _1 + \alpha _2 = -b /  a$ e $\alpha _1 \alpha _2 = c / a$.
\end{esempio}
\noindent Prima di enunciare il teorema che permette di collegare il discriminante alla teoria di Galois, si nota il fatto.
Se $\alpha _1,\ldots,\alpha _n$ sono le radici di un polinomio $p(x) \in K[x]$ di grado $n$ e se $\sigma \in S(\left\{ \alpha _1,\ldots,\alpha _n \right\} )$, allora:
\[
\prod_{i<j} \big(\sigma (\alpha _i)-\sigma (\alpha _j)\big) = \prod_{i<j} \frac{\sigma (\alpha _i) - \sigma (\alpha _j)}{\alpha _i-\alpha _j} \prod_{i<j} (\alpha _i-\alpha _j) = \operatorname{sgn} (\sigma ) \prod_{i<j} (\alpha _i-\alpha _j)
\] 
Se $L=K(\alpha _1,\ldots,\alpha _n)$ \`e il campo di spezzamento di $p$ su $K$ e ogni fattore irriducibile di $p$ \`e separabile, si pu\`o elevare al quadrato l'espressione sopra per ottenere che
\[
\sigma (\operatorname{disc} p(x)) = \operatorname{disc} p, \ \forall \sigma \in \operatorname{Gal}  (L / K)
\] 
quindi $\operatorname{disc} p(x) \in L^{\operatorname{Gal}( L / K)}  = K$.
\begin{teorema}
	Sia $p(x) \in K[x]$ un polinomio irriducibile e separabile di grado $n$; allora, se $L$ \`e il suo campo di spezzamento su $K$, si ha $\operatorname{Gal} (L/K) \longhookrightarrow A_n \iff\operatorname{disc} p $ \`e un quadrato in $K$.
\end{teorema}
\begin{proof}
Sia $G = \operatorname{Gal}( L / K)$; allora $G \longhookrightarrow A_n\iff \operatorname{sgn} (\sigma )=1, \ \forall \sigma \in G$.
Siano $\alpha _1,\ldots,\alpha _n$ le radici di $p(x)$ in $\overline{K}$; allora si \`e visto che 
\[
\sigma \left(\prod_{i<j} (\alpha _i-\alpha _j)\right) =\prod_{i<j} (\sigma (\alpha _i) - \sigma (\alpha _j)) = \operatorname{sgn} (\sigma ) \prod_{i<j} (\alpha _i -\alpha _j)
\] 
Visto che gli elementi fissati da ogni $\sigma \in G$ sono tutti e soli gli elementi di $K$.
\begin{itemize}
	\item[$(\Rightarrow)$] Se $G \longhookrightarrow A_n$, allora $\operatorname{sgn} (\sigma ) = 1, \ \forall \sigma \in G$, quindi $\prod_{i<j} (\alpha _i-\alpha _j)$ \`e fissato da ogni elemento di $G$, quindi $\prod_{i<j} (\alpha _i-\alpha _j) \in K$.
		Ne segue subito che $\operatorname{disc} p(x)$ \`e un quadrato in $K$ perch\'e una sua radice \`e in $K$.
	\item[$(\Leftarrow)$] Se $\operatorname{disc} p(x)$ \`e un quadrato in $K$, allora $x^2 - \operatorname{disc} p(x)$ ammette una soluzione in $K$, pertanto deve scomporsi in fattori lineari.
		Questo implica che $\prod_{i<j} (\alpha _i-\alpha _j)\in K$, quindi ogni $\sigma \in G$ deve fissarlo. 
		Per la formula riportata sopra, affinch\'e $\sigma $ lo fissi, si deve avere $\operatorname{sgn} (\sigma )=1$, quindi $G \longhookrightarrow A_n$.
\end{itemize}
\end{proof}
\end{document}
