...
Intanto si nota che, grazie alla compattezza di $F$, si pu\`o usare il lemma \ref{l2t1}:
\[
G = G\setminus F \cup F \implies \lvert G \rvert = \lvert F \rvert _e  + \lvert G \setminus F \rvert _e \implies \lvert G \setminus F \rvert _e = \lvert G \rvert - \lvert F \rvert _e
\] 
...
\begin{lemma}[Da capire dove inserire]
	Siano $\left\{ I_k \right\} _{k=1} ^N$ degli intervalli non sovrapposti, cio\`e per cui vale $\mathring{I}_k \cap \mathring{I}_j = \varnothing$ per $k\neq j$; allora $\bigcup_{k=1} ^N I_k$ \`e misurabile e 
	\[
	\left\lvert \bigcup_{k=1} ^N I_k\right\rvert  = \sum_{k=1}^{N} \lvert I_k \rvert 
	\] 
\end{lemma}
\begin{boxenv}[]
	\centering \color{mastercolor} Riprendere da 16:38, lezione 29/09
\end{boxenv}
