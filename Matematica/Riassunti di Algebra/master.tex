...
\subsection{Divisibilit\`a tra interi}
\begin{prop}
	[Algoritmo di Euclide]
	Siano $a,b$ due interi non negativi, con $a \ge b$.
	Si prendono $r_0 =a$ e $r_1 = b$; se, per $k\ge 1$, $r_k >0$, allora si definisce ricorsivamente $r_{k+1} $ come il resto della divisione di $r_{k-1} $ per $r_k$.
	Essendo $r_0>r_1>\ldots\ge 0$, in un numero finito di passi, siano $n$ per esempio, si ottiene $r_n = 0$.
	Allora $(a,b) = r_{n-1} $.
\end{prop}
\newpage
...
\end{lemma}
\subsubsection{Azioni per coniugio}
Per $X = G$ e $\gamma : G \longrightarrow \operatorname{Int} G \subset S(G) $ si ha l'azione per coniugio.
