...
\begin{boxenv}[]
\begin{equation}
	\left\langle \frac{1}{\rho }\frac{d E}{d x}  \right\rangle = \frac{4\pi \mathcal{N}_A Z z^2 m_e c^2 r_e^2}{A \beta ^2} \ln \frac{\beta ^3 \gamma^2 c}{zr_e \omega_e} =z^2 \frac{Z}{A} \frac{K}{\beta ^2}\ln \frac{\beta ^2 \gamma^2}{zr_e \omega_e / c}
\end{equation}
\end{boxenv}
...
\begin{boxenv}[]
\centering {\color{red} Inserire immagine di varie particelle che attraversano spessore $\Delta x$.} 
...
\section{Lezione 10/12/2020}
\subsection{Energia residua dopo attraversamento di un materiale}
L'ottenimento dell'energia residua di una particella che ha attraversato spessore $\Delta x$ di materiale si ottiene facilmente se \`e nota la forma grafica o tabellare della funzione $R(E)$. 
...
Si usa una camera a bolle per distinguere la carica della particella che si vuole individuare. La camera a nebbia \`e riempita di un materiale vicino ad una temperatura sopra cui produce bolle e una minima quantit\`a di energia persa dalla particella finisce in bolle. 
...
Il problema \`e che perch\'e questo metodo funzioni, bisognerebbe sapere da dove entra la particella: potrebbe essere di carica positiva che entra dal basso, o di carica negativa che entra dall'altro. Per ottenere questa informazione, si applica uno spessore di piombo pari a una lunghezza di radiazione ($\sim 6$ mm) cosicch\'e percorra una traiettoria circolare con raggio di curvatura inferiore dopo aver attraversato spessore di piombo a seguito della perdita di energia.


Nell'esperimento si \`e misurato $q = e$ con $R_A B = 0.21$ Tm e $R_B B = 0.075$ Tm, che risultano in $\lvert \vec{p} _\text{in}\rvert = 63 \text{ MeV} / c$ e $\lvert \vec{p}_\text{out} \rvert = 22.5 \text{ MeV}/c$.
...
Per verificare che si pu\`o trattare di positrone, questo sarebbe comunque soggetto a perdita di energia nel piombo per collisione pari a $7 / 8$ MeV, in aggiunta alla perdita per Bremsstrahlung data da
\[
...
\begin{boxenv}[]
	\centering \color{red} Inserire schema scattering 
\end{boxenv}
\noindent Si considera $\theta  \ll 1$; in questo caso si pu\`o approssimare la distribuzione angolare come distribuzione Gaussiana e si ha $\hat{n} = (\theta  \cos \varphi , \theta  \sin \varphi  , 1)\equiv (\theta _x, \theta _y, 1)$.
...
\end{split}
\] 
dove si \`e usato $\theta \sim \ell  / b$ ({\color{red}Da ricontrollare dagli appunti}). Usando $b_\text{max} = R_\text{atomo} $ e $b_\text{min} = R_\text{nucleo}$ e visto che $A^{1 / 3} Z^{1 / 3} \propto Z^{2 / 3} $:
\[
	\begin{split}
	...
	\end{split}
\] 
Visto che $N_\text{urti} = nL \sigma _{ms} $, usando la formula di Tsai approssimata ({\color{red}Aggiungere formula di Tsai}):
\[
	\langle \theta ^2 \rangle = nL\sigma _{ms} \langle \theta _{\text{urto}} ^2 \rangle = \frac{X_0}{X_0}n L 4 \pi \ell ^2 \ln \frac{205}{Z^{1 / 3} } \simeq \frac{nL}{X_0} \frac{4\pi \ell ^2 \ln \frac{205}{Z^{1 / 3} }}{4Z^2 n \alpha  r_e^2 \ln \frac{184}{Z^{1 / 3} }}\simeq \frac{\pi \ell ^2 L}{X_0 \alpha r_e^2}
\] 
dove il rapporto dei due logaritmi \`e circa $1$. Si ottiene, quindi, andando a sostituire $r_e = \frac{1}{4\pi \varepsilon _0 m_e c^2}$ ({\color{red}Vedere perch\'e non si mette $e^2 $ al numeratore}):
\[
\begin{split}
	&\sqrt{\langle \theta ^2 \rangle} = \ell  \sqrt{\frac{L}{X_0}} \sqrt{\frac{\pi}{\alpha r_e^2 Z^2}} = \sqrt{\frac{L}{X_0}} \sqrt{\frac{\pi}{\alpha r_e^2 Z^2}} \frac{2zZ \alpha  \hbar  c}{pv}= 2z \sqrt{\frac{L}{X_0}}  \sqrt{\frac{\pi}{\alpha }} \frac{m_e c^2}{pv}= z \sqrt{\frac{2L}{X_0}} = \sqrt{2} \theta _0\\
