...
Da osservazioni sperimentali, si hanno i due seguenti punti:
\begin{enumerate}[(I).]
...
\end{enumerate}
L'argomento di Widom si basa sul combinare queste due osservazioni sperimentali per determinare una forma del parametro d'ordine.
Si ipotizza che
...
\end{itemize}
Si fissa il riscalamento in in modo che $\lvert t' \rvert =1$, cio\`e $b = \lvert t \rvert ^{-1 / D_t}$.
Cos\`i facendo, essendo $f(h,t) = b^{-d}f(h',t')$, si trova che
\[
f(h,t) = \left(\lvert t \rvert ^{-1 / D_t}\right) ^{-d} f(h',1) \implies f(h,t) = \lvert t \rvert ^{d / D_t} \widetilde{f} \left(\frac{h}{\lvert t \rvert ^{D_h / D_t}}\right) 
\] 
dove si \`e usato che $h ' = b^{D_h}h = \left(\lvert t \rvert ^{-1 / D_t}\right) ^{D_h}h$ e la $\widetilde{f}$ \`e ottenuta eliminando la dipendenza da $t'$ (fissato a $1$) nella $f(h',t')$.
...
\begin{enumerate}[(1).]
	...
	\item $\psi \sim h^{1 / \delta }$ implica che 
		\[
		\psi  = - \frac{\partial f}{\partial h}  = - \lvert t \rvert ^{\frac{d-D_h}{D_t}} \widetilde{f}\left(\frac{h}{\lvert t \rvert ^{D_h / D_t}}\right)  \sim h^{1 / \delta } \sim h ^{1 / \delta }
		\] 
		Questo andamento \`e vero se 
		\[
		\widetilde{f} \left(\frac{h}{\lvert t \rvert ^{D_h / D_t}}\right) \sim \left(\frac{h}{\lvert t \rvert ^{D_h / D_t}}\right) ^{1 / \delta } \implies \lvert t \rvert ^{\frac{d - D_h}{D_t} - \frac{D_h}{\delta D_t}} h^{1 / \delta } \sim h ^{1 / \delta } \iff \delta = \frac{D_h}{d-D_h}
		\] 
\end{enumerate}
...
\[
H = K_1 \sum_{i}^{} S_i + K_2 \sum_{\langle i,j \rangle}^{} S_iS_j + \widetilde{K}_2 \sum_{\langle \langle i,j \rangle \rangle}^{} S_iS_j + \ldots + K_3 \sum_{\langle i,j,k \rangle}^{} S_iS_jS_k +\ldots
\] 
In ogni caso, vale
\[
...
\[
\overline{F} (\Delta x , \Delta y) = \Delta x  ^{1 / 2} \overline{F}\left(1 ,  \frac{\Delta y}{\Delta x ^{-1 / 2}}\right) = \Delta x ^{1 / 2} \widetilde{F}  \left(\frac{\Delta y}{\Delta x ^{1 / 2}}\right) 
\] 
avendo definito $\widetilde{F}$ in modo da togliere la dipendenza da $1$.
Visto che $\Delta x = x-x^* = x - 0 = x = e^{-4\beta J}$ e $\Delta y = y-1 = e^{-2\beta  h}-1 \simeq -2\beta  h$ (per $\Delta y$ piccolo), si ha
\[
\overline{F} (\Delta x,\Delta y) = e^{-2 \beta J} \widetilde{F} \left(2 \beta h e^{2\beta J}\right) 
\] 
dove si pu\`o trascurare il fattore $2$ nell'argomento della $\widetilde{F}$ perch\'e si \`e interessati ad andamenti asintotici.
Visto che si \`e in 1D, ci si aspetta che $[\overline{F}] = 1 / L \Rightarrow \overline{F}\sim\xi^{-1} $.
Immaginando che $\xi \sim e^{2 \beta J}$, cio\`e la sua divergenza sia catturata dal prefattore $e^{-2 \beta J}$, si ha $\overline{F} \sim \xi ^{-1}\widetilde{F}$.
...
\[
O = \operatorname{Tr} \left(\hat{O}\rho \right) 
\] 
...
\begin{equation}
i\hbar  \frac{d \rho }{d t} = \left[ \hat{H},\rho  \right] 
\end{equation}
\begin{osservazione}
In questa trattazione, si \`e interessati esclusivamente a stati di equilibrio, per i quali $\left[ \hat{H}, \rho   \right]  = 0$.
\end{osservazione}
\noindent Si diagonalizza l'hamiltoniano: $\hat{H } \ket{\phi _n}  = E_n \ket{\phi _n} $, per cui si ha che $\ket{\psi (t)} = \sum_{n}^{} c_n(t) \ket{\phi _n} $.
...
\end{osservazione}
\subsubsection{Gas perfetto con ensemble gran canonico}
Si considera preliminarmente\footnote{La traccia, per sistemi non interagenti, \`e banale.}
\[
...
La funzione $g\left(\left\{ n_\mathbf{p}  \right\} \right) $ stabilisce l'ammissibilit\`a della frequenza e si comporta differentemente a seconda della statistica:
\[
g\left(\left\{ n_\mathbf{p}  \right\} \right)  = \begin{cases}
	...
	\left(\frac{\cancel{N!}}{\prod_\mathbf{p} n_\mathbf{p} !}\right) \frac{1}{\cancel{N!}}& \text{ per } n_\mathbf{p} \text{ arbitrario } \hspace{.3cm}\text{(Boltzmann)}
\end{cases} 
\] 
\begin{itemize}
	\item Caso della statistica di Boltzmann.
...
\item Caso di bosoni e fermioni.

	Questi due casi sono pi\`u complicati a causa del vincolo $N = \sum_{\mathbf{p} }^{} n_\mathbf{p} $, motivo per cui conviene passare alla funzione di gran partizione, che permette di sommare via $N$.
	\[
		\begin{split}
...
Nel caso particolare dei fermioni, $n_\mathbf{p} = 0$, oppure $1$, quindi
\[
...
\] 
dove si \`e usato che il logaritmo si sviluppa come
\[
\sum_{n=1}^{+\infty} (-1)^{n+1} \frac{\varepsilon ^n}{n} \ \text{ con } \ \ \varepsilon  = z e^{-\beta p^2 / 2m}
\] 
Definendo 
\[
f_\alpha (z) = \sum_{n=1}^{+\infty} (-1)^{n+1}\frac{z^n}{n^\alpha }
\] 
si pu\`o scrivere che
\[
\begin{cases}
	\displaystyle \log\mathcal{Z} = \frac{pV}{\kappa _BT}= \frac{v}{\lambda _T^3} f_{5 / 2} (z)\\
	\\
	\displaystyle \langle N \rangle= z \frac{\partial }{\partial z} \log \mathcal{Z}  \implies \frac{1}{v} = \frac{\langle N \rangle}{v}=\frac{1}{\lambda _T^3} f_{3/2} (z)
\end{cases}
\] 
\end{itemize}
\begin{boxenv}[]
\centering Riprendere da lezione 19
\end{boxenv}
