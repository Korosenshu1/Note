...
\[
\widetilde{\psi }_{\mathbf{p} _1,\ldots,\mathbf{p} _N} = \frac{1}{\sqrt{\prod_\mathbf{p} (n_\mathbf{p} )!} }\psi _{\mathbf{p} _1,...,\mathbf{p} _N} 
\] 
...
\[
	\begin{split}
		\operatorname{Tr} [ \hat{O} ]  &= \sum_{\left\{ \mathbf{p} _1,\ldots,\mathbf{p} _N \right\} }^{*} \Braket{\widetilde{\psi }_{\mathbf{p} _1,\ldots,\mathbf{p} _N} |\hat{O}|\widetilde{\psi } _{\mathbf{p} _1,\ldots,\mathbf{p} _N} } = \sum_{\mathbf{p} _1,\ldots,\mathbf{p} _N}^{} \frac{\prod_\mathbf{p} (n_\mathbf{p} !)}{N!} \braket{\widetilde{\psi }|\hat{O}|\widetilde{\psi }} \\
...
dove
\[
\widetilde{v}_{ij} : = -\frac{1}{\beta }\log \left(1\pm e^{\frac{-2\pi\lvert \mathbf{r} _i - \mathbf{r} _j \rvert^2 }{\lambda _T^2}}\right) 
\] 
...
Si vede che \`e attrattivo per i bosoni, essendo $\widetilde{v}_{ij} <0$, mentre \`e repulsivo per i fermioni.
...
Visto che, nel limite termodinamico (cio\`e $N,V \to +\infty$ con $v$ costante), il contributo di $\frac{1}{V} \frac{z}{1-z}$ \`e rilevante solo se $z=1$, si conclude che:
\begin{itemize}
	\item per $T<T_c$, si deve necessariamente avere $z\to 1$, altrimenti l'equazione \ref{dbec} non ha soluzione;
	\item per $T>T_c$, il termine additivo non \`e pi\`u rilevante perch\'e si pu\`o ottenere una soluzione $z<1$ da $\lambda _T^3 / v = g_{3 / 2} (z)$.
\end{itemize}
...
\end{osservazione}
\begin{boxenv}[]
\centering Riprendere lezione 21 da 42:00
\end{boxenv}
