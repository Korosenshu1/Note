...
\[
	\begin{split}
		H &\simeq -\frac{1}{2} J \sum_{i=1}^{N} \sum_{j_\text{p.v.}}^{} (-m^2+2mS_i) - h \sum_{i=1}^{N} S_i\\
		  &= - \frac{1}{2}J y  \sum_{i=1}^{N} (-m^2+2mS_i) - h \sum_{i=1}^{N} S_i\\
...
\[
F_0=-N\kappa _B T \log 2 \hspace{.5cm} a = N \frac{Jy}{\kappa _B T } \left(\kappa _B T - Jy\right)  \hspace{.5cm} b = 2N \kappa _B T \left(\frac{Jy}{\kappa _B T}\right) ^4 \hspace{.5cm}\lambda = N \frac{Jy}{\kappa _B T}
\] 
...
\end{enumerate}
L'argomento di Widom si basa sul combinare queste osservazioni sperimentali per determinare una forma del parametro d'ordine e si vedr\`a portare alla relazione di Widom.
Si ipotizza che
...
L'omogeneit\`a permette di codificare l'invarianza di scala che si verifica nei pressi del punto critico dovuta alla divergenza del parametro di scala; in questo modo, quindi, si generalizza quanto visto con l'argomento di Widom, partendo da un ragionamento pi\`u generale e basato unicamente sul fatto che il sistema, nel punto critico, deve essere indipendente dalla scala col quale lo si osserva (cio\`e deve valere l'ipotesi di scala).
...
La seguente trattazione \`e basata sul mostrare perch\'e l'ipotesi di scala porta naturalmente alla richiesta che la funzione $f= F / V$ sia omogenea, imponendo che la fisica del sistema sia la stessa, indipendentemente dalla scala con cui lo si studia (nei pressi del punto critico).
\begin{center}
	\begin{minipage}
		{.45\columnwidth}
...
\[
Z = e^{-N f(K)}= \sum_{\left\{ S \right\} }^{} e^{- H \left(\left\{ S \right\} \right) }= \sum_{\left\{ S' \right\} }^{} \sum_{\left\{ S \right\} }^{} P\left(\left\{ S' \right\} , \left\{ S \right\} \right)  e^{-H\left(\left\{ S \right\} \right) } \overset{\text{def}}{=} e^{-N\mu } \sum_{\left\{ S' \right\} }^{} e^{-H' \left(\left\{ S' \right\} \right) }
\] 
...
\] 
con $\mu $ legato al fatto che il numero di siti cambia e determinato da $\sum_{\left\{ S' \right\} }^{} H' \left(\left\{ S' \right\} \right) = 0$.
Si richiede anche che $H'$ abbia la stessa forma di $H$, cio\`e $H'\left(\left\{ S' \right\} \right) = \sum_{\alpha }^{} K_\alpha ' S'_\alpha $.
Quindi, si ha
\[
...
f(K) = \mu (K) + b ^{-d}f(K')
\] 
con $\mu (K)$ costante\footnote{Qui $K$ \`e il vettore di componenti $K_\alpha $.}, per cui non altera la fisica del sistema.

La teoria del gruppo di rinormalizzazione consiste, quindi, nello studiare una trasformazione, detta \textit{trasformazione RG}
\[
K \longmapsto K' = R(K)
\] 
che riscala il sistema: ogni iterazione della mappa raggruppa un sito in un blocco di \textit{passo successivo} (per esempio, nel reticolo quadrato, si incorpora un altro sito per dimensione); interando, si pu\`o arrivare al riscalamento del sistema desiderato.
...
\begin{itemize}
	\item sistemi completamente disordinati ($T=+\infty\Rightarrow \xi =0$);
...
Si vuole che questa coincida con la sua forma riscalata, ossia
\[
...
\] 
cio\`e si ritrova esattamente quanto ottenuto con la teoria del gruppo di rinormalizzazione, in prossimit\`a di $\beta \to +\infty$ e $h \to  0$.
...
Gli osservabili sono gli operatori $\hat{O}$, il cui valore medio nello stato $\ket{\psi } $ \`e dato da $O = \braket{\psi |\hat{O}|\psi } $.
...
\[
\langle O \rangle = \operatorname{Tr}  (\hat{O}\rho)
\] 
...
La differenza tra i vari tipi di sistemi elencati sopra, in questa trattazione, entra proprio nelle possibilit\`a degli $n_\mathbf{p} $: non si pu\`o avere pi\`u di un fermione per momento $\mathbf{p} $, quindi $n_\mathbf{p} = 0, 1$; per bosoni, non si hanno vincoli; per Boltzmann, $n_\mathbf{p} $ pu\`o essere arbitrario, ma una certa sequenza $\left\{ n_\mathbf{p}  \right\} $ pu\`o essere associata a pi\`u stati differenti, per un totale di $\frac{N!}{\prod_\mathbf{p} n_\mathbf{p} !}$\footnote{Il numeratore considera tutte le possibili combinazioni; il denominatore si riferisce al fatto che, scambiando due particelle con uguale momento, si ha ancora lo stesso stato}, perch\'e non vi \`e alcun discorso riguardo simmetrie per scambio di particelle.
Per la statistica di Boltzmann, inoltre, si dovr\`a anche tenere conto del fattore correttivo di Gibbs, che semplificher\`a l'$N!$ al numeratore.
\begin{osservazione}
...
\textbf{Caso della statistica di Boltzmann.} In questo caso, si ha
		\[
			Z = \sum_{\left\{ n_0,n_1,\ldots \right\} }^{*} \left[ \frac{e^{-\beta n_0 \varepsilon _0}}{n_0!} \frac{e^{-\beta n_1 \varepsilon _1}}{n_1!} \cdot \ldots \right] = \frac{1}{N!}\left[ e^{-\beta \varepsilon _0}+ e^{-\beta  \varepsilon _1}+ \ldots\right] ^N
		\] 
...
\[
	\log \mathcal{Z}  = \sum_{\mathbf{p}}^{} \log\left(1+ z e^{-\beta \varepsilon _\mathbf{p} }\right) \xrightarrow{\hspace{.4cm}} \frac{V}{h^3} \int d^3 p \ \log\left(1 + z e^{- \frac{\beta  p^2}{2m}}\right) = \frac{V}{\lambda _T^3} \sum_{n=1}^{+\infty} \frac{(-1)^{n+1}z^n}{n^{5 / 2}}
\] 
...
\begin{cases}
	...
	\\
	\displaystyle \langle N \rangle = z \frac{\partial }{\partial z}  \log\mathcal{Z} =  \frac{V}{\lambda _T^3} g_{3 / 2} (z) {\color{asdf}\ +\ \frac{z}{1-z}}
\end{cases}
\end{equation}
...
Visto che
\[
\epsilon _\mathbf{p} = \frac{1}{2m} \left(\frac{2\pi\hbar}{L}\right) ^2 \lvert \mathbf{n}  \rvert ^2
\] 
...
\[
U = \frac{3}{2}pV \hspace{1cm} F = \mu N - pV = \frac{N}{\beta } \log z- pV
\] 
Da questo, si ottiene che 
\[
\frac{S}{N} = \frac{1}{T} (U - F) = \begin{cases}
...
\end{cases} \ \ \implies \lim_{z \to 1} \frac{S}{N} = \frac{\ell}{T}
\] 
...
Questo fatto si pu\`o vedere considerando che
\[
...
\end{equation}
In questo modo, si determina $z$ e, quindi, si pu\`o sostituire in $\frac{p}{\kappa _B T } = \frac{1}{\lambda _T^3}f_{5 / 2} (z)$, utilizzando il fatto che
\[
	f_{3/2} (z) = z \frac{d }{d z} f_{5 / 2} (z) = \frac{d }{d (\log z)} f_{5 / 2} (z) \implies f_{5 / 2} (z) = \int  f_{3 / 2} (z)\ d\nu
\] 
Cos\`i facendo, $f_{5 / 2} (z) \simeq \frac{8}{15 \sqrt{\pi} } (\beta \mu  )^{5 / 2} + \frac{\pi^2}{3 \sqrt{\pi} }(\beta \mu  ) ^{1 / 2} $, quindi si arriva a 
\[
p\simeq  \frac{2}{5}\frac{\varepsilon _F}{v}\left[ 1 + \frac{5}{12}\pi^2 \left(\frac{\kappa _B T}{\varepsilon _F}\right) ^2 \right]  
\] 
\begin{osservazione}
...
\] 
\end{osservazione}
\noindent Per $T\to 0$, si possono scartare le potenze superiori all'ordine $2$ in $T$ nell'espressione di $p$, per cui si trova che 
\[
...
\[
\hat{H} = \sum_{i=1}^{N} \left\{ \frac{1}{2m} \left(\hat{\mathbf{p} }_i - \frac{q}{c}\mathbf{A} \right) ^2 - \mu _0 \hat{\pmb{\sigma }} _i \cdot \mathbf{H}  \right\} 
\] 
dove il termine $\left(\hat{\mathbf{p} }_i - \frac{q}{c}\mathbf{A} \right) ^2$ \`e legato all'avere un fermione carico in un campo magnetico, che origina il \textit{diamagnetismo di Landau}, mentre il termine $\mu _0 \hat{\pmb{\sigma }} _i \cdot \mathbf{H} $ \`e legato all'interazione tra spin e campo, che origina il \textit{paramagnetismo di Pauli}.
...
\[
\hat{H} = \sum_{i=1}^{N} \left(\frac{\hat{\mathbf{p} }_i ^2}{2m} - \mu _0 \hat{\pmb{\sigma } }_i \cdot \mathbf{H} \right) 
\] 
In opportuna base, gli autovalori di $\hat{\pmb{\sigma } }_i \cdot \mathbf{H} $ sono $S_i\mathrm{H} $, con $S_i = \pm 1$, per cui
\[
...
\[
	\begin{split}
		Z_N &= \sum_{N^+=0}^{N} \sum_{\left\{ n^+_\mathbf{p} , n^-_\mathbf{p}  \right\} }^{*_1 *_2} e^{-\beta \sum_{\mathbf{p} }^{} \varepsilon _\mathbf{p} \left(n^+_\mathbf{p}  + n^-_\mathbf{p} \right) }e^{\beta \mu _0 \mathrm{H} (2N^+ - N)}\\
		    &= \sum_{N^+ = 0}^{N} e^{\beta \mu _0\mathrm{H} (2N^+ - N)} \sum_{\left\{ n^+_\mathbf{p}  \right\} }^{*_1} e^{-\beta \sum_{\mathbf{p} }^{} \varepsilon _\mathbf{p} n^+_\mathbf{p} } \sum_{\left\{ n_\mathbf{p} ^- \right\} }^{*_2} e^{-\beta \sum_{\mathbf{p} }^{} \varepsilon _\mathbf{p} n^-_\mathbf{p} }
	\end{split}
\] 
...
\[
	\begin{split}
		Z_N(T,V,\mathrm{H} ) &= \sum_{N^+ = 0}^{N} e^{\beta \mu _0 \mathrm{H}  (2N^+ - N) - \beta \left\{ F_{N^+} ^{(0)} (T,V) + F_{N-N^+} ^{(0)}(T,V) \right\} }\\
				     & = \sum_{N^+ = 0}^{} e^{\beta f (N^+)}
	\end{split}
\] 
...
\[
Z_N \simeq e^{\beta f(\overline{N}^+)}\implies \frac{1}{\beta }\log Z_N = f(\overline{N}^+)
\] 
...
\] 
Ora, nel limite $T\to 0$ e assumendo $\lvert r \rvert \ll 1$, ossia non si hanno situazioni sbilanciate a livello di spin nel sistema\footnote{Questa \`e una situazione termodinamicamente tipica nei casi di $\mathrm{H} $ piccoli.}, si trova che 
\[
...
\[
\hat{H} = \frac{1}{2m}\sum_{i=1}^{N}  \left(\hat{\mathbf{p} }_i + \frac{e}{c} \mathbf{A} \right) ^2 = \sum_{i}^{} \hat{H}_i
\] 
...
Non si intende studiare questo in quanto complicato a livello di conti; piuttosto, ci si mette nel limite $\kappa _B T \ll \hbar \omega_c$ e si ipotizza che $p_z$ sia costante, per cui $\varepsilon _j \simeq \hbar \omega_c (j + 1 / 2)$, con $j=0,1,2,\ldots$ e $\omega_c = \frac{e\mathrm{H} }{mc}$.
...
\] 
Questa \`e l'intensit\`a del campo esterno tale per cui $g = N$, quindi, se $\mathrm{H} > \mathrm{H} _0$, tutti i fermioni si trovano nello stato fondamentale relativo a $j=0$.
\begin{itemize}
...
\] 
Questa scrittura gi\`a sottintende una scelta di ordinamento: si parte con il creare le particelle nello stato energetico pi\`u alto, scendendo mano mano.
...
\[
\hat{O}_1 = \sum_{\alpha }^{}\braket{\alpha |\hat{o}_1|\alpha } \hat{n}_\alpha =\sum_{\alpha }^{}\braket{\alpha |\hat{o}_1|\alpha } \hat{a}_\alpha ^\dagger \hat{a}_\alpha  
\] 
...
Usando questo vincolo per scrivere $\mathbf{v}  = \mathbf{p} _1 + \mathbf{p} _2 - \mathbf{u} $ e sostituendo $V(\mathbf{q} ) = \int V(\mathbf{s} ) e^{-\frac{i}{\hbar }\mathbf{q} \cdot \mathbf{s} } \ d\mathbf{s} $, si ottiene
\[
...
\] 
L'idea, ora, \`e di scomporre le somme, lasciando al pi\`u un impulso libero e diverso da zero, trascurando gli altri; tenendo conto delle varie combinazioni possibili nel secondo termine, si rimane con
\[
...
Ponendo $n = N / V$ e trascurando i termini di ordine superiore al secondo negli operatori di creazione e distruzione dalla sostituzione di $N$, si ha
\[
...
\end{cases}
\end{equation}
con $u_p ,v_p \in \mathbb{R}$ sfericamente simmetrici in $\mathbf{p} $ e $\hat{b}_\mathbf{p} , \hat{b}^\dagger _\mathbf{p} $ sono nuovi operatori che devono soddisfare la statistica bosonica $[\hat{b}_\mathbf{p} ,\hat{b}^\dagger _\mathbf{q} ] = \delta _{\mathbf{p} \mathbf{q} } $, per cui descrivono delle quasi-particelle bosoniche.
...
\] 
con $E_0 \simeq g \frac{n^2V}{2}$. I valori in questa espressione si ottengono dal processo per determinare l'$\alpha _p$ voluto; questa condizione \`e soddisfatta se 
\[
...
Moltiplicando le due espressioni in \ref{apbp} e togliendo i termini con $\langle \hat{b}_\mathbf{p} \hat{b}_{-\mathbf{p} }  \rangle$ e $\langle \hat{b}_\mathbf{p}^\dagger  \hat{b}^\dagger _{-\mathbf{p} }  \rangle$ in quanto nulli, si ottiene che
\[
...
\[
\left(\frac{p^2}{2m}\right) ^2 \sim \frac{gn}{m}p^2 \implies p \sim \sqrt{4 mgn} 
\] 
...
\end{equation}
Il secondo termine \`e relativo all'\textit{hopping} tra un sito e l'altro, mentre il primo termine coinvolge il potenziale chimico perch\'e si vede essere $\sim \mu \cdot M$, con $M$ numero di elettroni del sistema; di fatto, si definisce l'operatore numero di elettroni nel reticolo come
\[
\hat{M}=\sum_{j,\sigma }^{} \hat{c}^\dagger _{j,\sigma } \hat{c}_{j,\sigma } 
\] 
Si nota che il numero di elettroni del sistema \`e una costante del moto, in quanto $[\hat{M},\hat{H}] = 0$.
...
\begin{center}
	\begin{tikzpicture}
		...
		\foreach \i in {2,4,6,8,10}{
			\draw[thick] (\i,-0.10) -- (\i,0.10);
		}
		\draw[very thick] (0,-0.15) -- (0,0.15) node[above] {\footnotesize $0$};
		\draw[very thick] (12,-0.15) -- (12,0.15) node[above] {\footnotesize $L$};
		\node[above] at (2,0) {\footnotesize $a$};
		\node[above] at (4,0) {\footnotesize $2a$};
		\node[above] at (6,0) {\footnotesize $3a$};

		\draw[] (0,-1) -- (8,-1);
		\draw[dashed] (8,-1) -- (10,-1);
		\draw[] (10,-1) -- (12,-1);
		\foreach \i in {2,4,6,8,10}{
			\draw[fill] (\i,-1) circle (1.25pt) ;
		}
		\draw[fill] (0,-1) circle (2pt);
		\draw[fill] (12,-1) circle (2pt);
		\node[below] at (-0.5,-1) {\footnotesize $j$};
		\node[below] at (12,-1) {\footnotesize $N-1$};
		\node[below] at (0,-1) {\footnotesize $0$};
		\node[below] at (2,-1) {\footnotesize $1$};
		\node[below] at (4,-1) {\footnotesize $2$};
		\node[below] at (6,-1) {\footnotesize $3$};
	\end{tikzpicture}
\end{center}
dove $j$ \`e un indice che rappresenta il sito del reticolo considerato e $N$ indica il numero di ioni del sistema.
Si ha:
\[
 L=N a\hspace{1cm}k = \frac{2\pi}{L}n=\frac{2\pi}{Na} n \in \bigg[-\frac{\pi}{a}, \frac{\pi}{a}\bigg)\hspace{1cm} n = -\frac{N}{2}, - \frac{N}{2}+1 ,\ldots, \frac{N}{2}-1
\] 
con $k$ che individua la prima zona di Brillouin.
...
\[
	\begin{split}
		\hat{H} &= -\frac{t}{N} \sum_{j,\sigma }^{} \sum_{k,q}^{}  \left(e^{-i kx _j}e^{i q (x_j + a)} \hat{c}^\dagger _{k\sigma } \hat{c}_{q\sigma } +e^{i kx _j}e^{-i q (x_j + a)} \hat{c}^\dagger _{q\sigma } \hat{c}_{k\sigma }\right) \\
			&= - \frac{t}{N} \sum_{j,\sigma }^{} \sum_{k,q}^{} \bigg\{ e^{-i (k-q)x_j} \big(\underbracket{e^{iqa}+ e^{-iqa}}_{=2\cos qa} \big) \hat{c}^\dagger _{k\sigma } \hat{c}_{q\sigma }  \bigg\} \\
			&= \sum_{k,\sigma }^{} (-2t \cos ka) \hat{c}^\dagger _{k\sigma } \hat{c}_{k\sigma } =-2t \sum_{k,\sigma }^{} \cos (ka) \ \hat{n}_{k,\sigma } 
	\end{split}
\] 
dove si \`e usato che $\frac{1}{N}\sum_{j}^{} e^{i(k-q) x_j} = \delta _{kq} $ e si \`e definito $\hat{n}_{k,\sigma } = \hat{c}^\dagger _{k,\sigma } \hat{c}_{k,\sigma } $ operatore numero in spazio $k$.
Sfruttando la trasformata di Fourier, quindi, si \`e potuto scrivere $\hat{H}$ in forma diagonale, da cui si vede che l'energia del sistema \`e legata ad una legge di dispersione in cui compare il $\cos (ka)$.
\begin{center}
	\begin{minipage}
		{.45\columnwidth}
In realt\`a, il grafico \`e discreto, in quanto ogni punto \`e separato di un $\Delta k = \frac{2\pi}{Na}$; conseguentemente, il numero di stati possibili, tenendo conto della degenerazione legata allo spin, si ha
\[
2\cdot \frac{k_\text{max}- k_\text{min}}{\Delta k}= 2N 
\] 
Nel limite termodinamico, per\`o, il numero di stati permessi tende al continuo e viene ben rappresentato dalla curva.
	\end{minipage}
	\hfill
	\begin{minipage}
		{.5\columnwidth}
...
\begin{axis}[
    ...
    xticklabels={\footnotesize $-\pi$,\footnotesize$-\pi/2$,\footnotesize$\pi/2$,\footnotesize$\pi$},
    yticklabels={\footnotesize $-1$,\footnotesize$1$},
    ytick={-1,1},
    xlabel={\footnotesize $ka$},
    ylabel={\footnotesize $E/2t$},
]
\addplot[thick,name path=band,domain=-pi:pi,samples=200] {-cos(deg(x))};
...
\node[color=blue!60!gray] at (1.9,-0.5) {\footnotesize half filling};
\node[color=blue!60!gray,below] at (1.9,-0.5) {\footnotesize ($T=0$)};
\end{axis}
\end{tikzpicture}
	\end{minipage}
\end{center}
Considerando il caso di half filling ($N$ elettroni su $N$ siti), per $T=0$, si vede popolata tutta la zona per $E / 2t < 0$, come riportato in figura.
Essendo il sistema \textit{gapless}, cio\`e la differenza di energia tra lo stato fondamentale e il primo eccitato va a zero nel limite termodinamico, si vede che, a $T>0$, anche infinitesima, si pu\`o facilmente osservare l'eccitazione a stati energetici superiori; questo comportamento, rende il sistema un \textit{conduttore}.

In alcuni casi, si possono sviluppare, nel limite termodinamico, dei gap finiti di energia che impediscono un comportamento del sistema da conduttore puro.
Alcuni esempi di questi casi sono i seguenti:
\begin{enumerate}[(a).]
	\item si aggiungono disomogeneit\`a nel tight binding;
	\item si aggiungono interazioni tra fermioni.
\end{enumerate}
\subsubsection{Dimerizzazione}

Si considera ancora il modello di tight binding 1D, modificando, per\`o, il valore di $t$ a seconda dei siti coinvolti:
\[
t_{j,j+1} = t \Big\{ 1+ (-1)^j\Delta \Big\} 
\] 
Sostanzialmente, a seconda che il sito di partenza sia pari o dispari, si avr\`a un diverso accoppiamento per l'hopping; per esempio, si definiscono $t_1$ come l'accoppiamento tra siti con $j$ pari e $t_2$ quello con siti dispari.
Si definisce, poi, una cella relativamente alla coppia di siti tra cui si forma un certo accoppiamento, per esempio a partire da siti dispari.
\begin{center}
	\begin{tikzpicture}
		\draw[color=red!60!gray,thick] (4,0) -- (6,0);
		\draw[color=red!60!gray,thick] (1,0) -- (2,0);
		\draw[color=red!60!gray,thick] (8,0) -- (9,0);
		\node[above,color=blue!60!gray] at (3,0) {\footnotesize $t_1$};
		\node[above,color=red!60!gray] at (5,0) {\footnotesize $t_2$};
		\node[above,color=blue!60!gray] at (7,0) {\footnotesize $t_1$};
		\draw[dashed,thick] (0,0) -- (1,0);
		\draw[dashed,thick] (9,0) -- (10,0);
		\foreach \i in {2,6}{
			\draw[color=blue!60!gray,thick] (\i,0) -- (\i+2,0);
		}
		\foreach \i in {2,4,6,8}{
			\draw[fill] (\i,0) circle (1.5pt);
		}
		\draw[dashed] (3,0) ellipse (1.05 and .5);
		\draw[dashed] (7,0) ellipse (1.05 and .5);
		\node at (3,-.75) {\footnotesize cella};
	\end{tikzpicture}
\end{center}
Usando il raggruppamento in celle, il sistema diventa periodico, dove ogni cella contiene due siti: si definiscono degli operatori $\hat{c}_{j\sigma } ^\dagger $, che agisce sui siti dispari, e $\hat{d}^\dagger _{j\sigma } $, che agisce su quelli pari; in questo modo
\[
\hat{H} = - t (1-\Delta ) \sum_{j,\sigma }^{} \left\{ \hat{c}^\dagger _{j\sigma } \hat{d}_{j\sigma } + \hat{d}^\dagger _{j\sigma } \hat{c}_{j\sigma }  \right\} - t (1+\Delta ) \sum_{j,\sigma }^{} \left\{ \hat{c}^\dagger _{j+1,\sigma } \hat{d}_{j\sigma } + \hat{d}^\dagger _{j\sigma } \hat{c}_{j+1,\sigma }  \right\}
\] 
...
\[
\hat{c}_{k,\sigma }  = \frac{1}{\sqrt{N / 2} } \sum_{j \text{ dispari}}^{} e^{-i k x_j}\hat{c}_{j\sigma } \hspace{1cm} \hat{d}_{k,\sigma }  = \frac{1}{\sqrt{ N / 2} } \sum_{j \text{ pari}}^{} e^{-ikx_j}\hat{d}_{j\sigma } 
\] 
In questo modo, si trova che
\[
	\begin{split}
		\hat{H} = &  - \frac{2 t_1}{N} \sum_{j,\sigma }^{}\sum_{k,q}^{} \left\{ e^{-ikx_j}e^{iqx_j}\hat{c}^\dagger _{k\sigma } \hat{d}_{q\sigma } +e^{ikx_j}e^{-iqx_j}\hat{d}^\dagger _{q\sigma } \hat{c}_{k\sigma }  \right\} \\
			  & - \frac{2t_2}{N} \sum_{j,\sigma }^{} \sum_{k,q}^{} \left\{ e^{-ik(x_j +2a)}e^{iqx_j}\hat{c}^\dagger _{k\sigma } \hat{d}_{q\sigma } +  e^{ik(x_j +2a)}e^{-iqx_j} \hat{d}^\dagger_{q\sigma } \hat{c} _{k\sigma }\right\} 
	\end{split}
\] 
Questo si pu\`o compattare in linguaggio matriciale:
\[
	\hat{H} = \sum_{k,\sigma }^{} \begin{pmatrix} \hat{c}^\dagger _{k\sigma } & \hat{d}^\dagger _{k\sigma }  \end{pmatrix} \begin{pmatrix} 0 & - t_1 - t_2 e^{-ik 2a}\\ - t_1 - t_2 e^{ik 2a} & 0 \end{pmatrix} \begin{pmatrix} \hat{c}_{k\sigma } \\ \hat{d}_{k\sigma }  \end{pmatrix}= \sum_{k,\sigma }^{}\hat{\mathfrak{c} }_{k\sigma } ^\dagger  \mathcal{H} _k \hat{\mathfrak{c} }_{k\sigma } 
\] 
dove $\hat{\mathfrak{c} }_{k\sigma } $ \`e il vettore di operatori di distruzione, chiamato \textit{spinore}, e 
\[
	\mathcal{H} _k = \Big[\overbracket{-t_1-t_2 \cos (2ka)}^{A_k} \Big]\sigma _x + \Big[\overbracket{-t_2\sin (2ka)}^{B_k} \Big]\sigma _y = \begin{pmatrix} 0 & A_k - i B_k\\ A_k + i B_k & 0  \end{pmatrix} 
\] 
con $\sigma _x,\sigma _y$ matrici di Pauli.
Ora, diagonalizzando la matrice $\mathcal{H} _k$, si ottengono tutte le informazioni sul sistema.
Gli autovalori di $\mathcal{H} _k$ sono 
\[
\lambda _{\pm} = \pm \sqrt{A_k^2 + B_k^2}  = \pm 2t \sqrt{1 - (1-\Delta ^2) \sin^2 (ka)} 
\] 
Questi modi normali sono le energie delle quasi-particelle che diagonalizzano l'hamiltoniano:
\[
	\hat{H} = \sum_{k,\sigma }^{} \hat{\mathfrak{D}}^\dagger _{k\sigma } \begin{pmatrix} \lambda _{\pm,k} &0 \\ 0 & \lambda _{-,k}  \end{pmatrix}  \hat{\mathfrak{D} }_{k\sigma  } 
\] 
\begin{center}
	\begin{minipage}
		{.4\columnwidth}
	Avendo diagonalizzato l'hamiltoniano, si possono studiare i livelli energetici: si osserva un gap energetico tra le due bande di possibili stati energetici pari a $2\Delta $.
	Si nota, inoltre, che gli stati ammissibili nelle due bande \`e pari a $N / 2$, in quanto ogni trasformata di Fourier \`e stata fatta su $N / 2$ siti.
	\end{minipage}
	\hfill
	\begin{minipage}
		{.55\columnwidth}
...
% Assi
\draw[thick,->] (-1.8,0) -- (1.8,0) node[right] {$ka$};
\draw[thick,->] (0,-1.3) -- (0,1.3) node[above] {$E/2t$};

% Tacche asse x
\draw[very thick] (-1.57,0.03) -- (-1.57,-0.03) node[below] {$-\pi/2$};
\draw[very thick] ( 1.57,0.03) -- ( 1.57,-0.03) node[below] {$\pi/2$};

% Bande
\draw[thick,domain=-1.57:1.57,samples=200]
    plot (\x,{sqrt(1-(1-0.25)*sin(\x r)^2)});

\draw[thick,domain=-1.57:1.57,samples=200]
    plot (\x,{-sqrt(1-(1-0.25)*sin(\x r)^2)});

% Livelli ±Delta
\draw[dashed] (-1.7,0.5) -- (1.7,0.5);
\draw[dashed] (-1.7,-0.5) -- (1.7,-0.5);

\node[left] at (0,0.6) {$\Delta$};
\node[left] at (0,-0.6) {$-\Delta$};

% Indicazione gap
\draw[<->,red,thick] (0.35,-0.5) -- (0.35,0.5);
\node[red,right] at (0.35,0.2) {$2\Delta$};

\end{tikzpicture}
	\end{minipage}
\end{center}
Ad half filling per $T\to 0$, si popola esclusivamente la banda inferiore e, anche per $T>0$, \`e necessario un certo quantitativo di energia per saltare a stati di energia superiore.
Questo sistema \`e chiamato \textit{isolante di banda}.

Verificando l'energia relativa allo stato fondamentale per questo sistema, si vede che questa \`e inferiore rispetto a quella ottenuta per il sistema gapless; questo significa che, a livello fisico, la dimerizzazione \`e pi\`u stabile del reticolo omogeneo, per cui il sistema tende ad uno stato di disomogeneit\`a per abbassare l'energia\footnote{Questo \`e quello che si verifica nel caso ideale, cio\`e in assenza di interazioni tra fermioni e per $T\to 0$.}. 
Questo fenomeno si chiama \textit{instabilit\`a di Peierls}.

\subsubsection{Modello di Hubbard}
Si considera un potenziale di interazione tra elettroni della forma generale
\[
\hat{V}_e = \frac{1}{2} \sum_{\sigma ,\sigma '}^{} \int d\mathbf{r}  \int d\mathbf{r} ' \ V\left(\lvert \mathbf{r} -\mathbf{r} ' \rvert \right) \hat{\psi }^\dagger _\sigma (\mathbf{r} ) \hat{\psi }^\dagger _{\sigma '} (\mathbf{r} ') \hat{\psi }_{\sigma '} (\mathbf{r} ') \hat{\psi }_\sigma (\mathbf{r} )
\] 
Tramite procedimenti analoghi a quelli fatti prima, si ha
\[
		\hat{V}_e = \operatorname{}  \sum_{\mathbf{R} }^{} U_{\mathbf{R} ,\mathbf{R} }  \hat{n}_{\mathbf{R} ,  \uparrow} \hat{n}_{\mathbf{R} ,\downarrow}  + \sum_{\mathbf{R} '\neq \mathbf{R} }^{} U_{\mathbf{R} ,\mathbf{R} '} \hat{n}_\mathbf{R} \hat{n}_{\mathbf{R} '} + \sum_{\mathbf{R}' \neq \mathbf{R} }^{} \sum_{\sigma ,\sigma '}^{} J_{\mathbf{R} ,\mathbf{R} '} \hat{c}^\dagger _{\mathbf{R} ,\sigma } \hat{c}^\dagger _{\mathbf{R}' ,\sigma '}  \hat{c}_{\mathbf{R} ,\sigma '} \hat{c}_{\mathbf{R} ' , \sigma } 
\] 
con $\hat{n}_{\mathbf{R} ,\sigma } $ operatore numero, tale che $\hat{n}_\mathbf{R} = \sum_{\sigma }^{} \hat{n}_{\mathbf{R} ,\sigma } $.
Al di l\`a dell'espressione e come ci si arriva, \`e importante considerare che, tipicamente, \`e il primo termine (quello diagonale) a dominare sugli altri: nell'approssimazione in cui si trascurano tutti gli altri termini, si ha il \textit{modello di Hubbard}:
\[
\hat{H}=-t \sum_{\langle i,j \rangle}^{} \sum_{\sigma }^{} \left(\hat{c}^\dagger _{i,\sigma } \hat{c}_{j,\sigma } + \hat{c}^\dagger _{j,\sigma } \hat{c}_{i,\sigma } \right) + U \sum_{j}^{} \hat{n}_{j,\uparrow} \hat{n}_{j,\downarrow} 
\] 
La soluzione esatta a questo problema si riesce a trovare esattamente in 1D tramite l'ansatz di Bethe, ma non verr\`a trattato, mentre, in dimensioni superiori, si usano approssimazioni.

Quello che \`e interessante notare \`e che, in half filling, se $U \gg t$, si ha un materiale isolante, chiamato \textit{isolante di Mott}: avendo un elettrone per ogni sito ed essendo il termine di interazione dominante rispetto a quello di hopping, il sistema tender\`a a fissare un elettrone per ciascun sito ed eviter\`a qualunque effetto per cui si possano trovare pi\`u elettroni per ione.
Questo gap energetico \`e indotto esclusivamente dalle interazioni elettrone-elettrone.

Modificando il rapporto $U / t$, \`e possibile incorrere in una transizione di fase quantistica in cui il sistema non \`e pi\`u isolante, ma diventa un conduttore.
Questa transizione di fase \`e tanto pi\`u apparente quanto piccola \`e la temperatura: a temperatura nulla, la transizione presenta esattamente le propriet\`a delle transizioni di fase classiche, manifestando discontinuit\`a nei potenziali.

\subsubsection{Modello di Heisenberg}
Il modello di Hubbard tiene in considerazione i gradi di libert\`a di spin e di carica; si fa vedere che, per $U \gg t$ in half filling, il grado di libert\`a di carica non contribuisce, perch\'e si ha un elettrone per singolo sito, e si mostra che l'hamiltoniano si pu\`o scrivere come
\[
...
\] 
operatori di spin, esprimibili tramite matrici di Pauli. 
Questo hamiltoniano definisce il \textit{modello di Heisenberg} per spin $1 / 2$ e la sua forma evidenzia un'interazione di natura anti-ferromagnetica, visto il segno del termine di interazione degli spin.
\begin{osservazione}
Il problema non \`e tanto pi\`u facile da trattare a livello analitico, ma si mostra la possibilit\`a di rimuovere un grado di libert\`a.
\end{osservazione}

Per ricavare questa forma, si scrive l'hamiltoniana di Hubbard in forma perturbativa come $\hat{H}_b = \hat{H}_0 + \delta \hat{H} $, con $\hat{H}_0$ termine di interazione e $\delta \hat{H}$ termine di hopping, il quale si assume piccolo rispetto ad $\hat{H}_0$.
Ci si mette in half filling e si considera, preliminarmente, una coppia di siti, con una coppia di elettroni.
\begin{center}
	\begin{minipage}
		{.45\columnwidth}
		Si rappresenta uno stato, specificando lo spin lungo $z$ nel primo sito e spin lungo $z$ nel secondo sito.
		Il sistema \`e composto da uno stato fondamentale, ottenuto da quattro possibili combinazioni di spin: $\ket{\uparrow,\uparrow} , \ \ket{\uparrow,\downarrow} , \ \ket{\downarrow, \uparrow} ,\ \ket{\downarrow,\downarrow} $, in cui i due elettroni si trovano in siti diversi, e da due stati eccitati $\ket{\uparrow\downarrow,0} , \ \ket{0,\uparrow\downarrow} $, i cui i due elettroni condividono lo stesso sito, necessariamente con spin opposto.
	\end{minipage}
	\hfill
	\begin{minipage}
		{.5\columnwidth}
	...
	\begin{tikzpicture}[scale=1.3]
		\foreach \i in {2, 3, 4, 5}{
			\draw[thick] (\i,0) -- (\i+.5,0);
		}
		\foreach \i in {2.5, 4.5}{
			\draw[thick] (\i,3) -- (\i+.5,3);
		}
		\draw[stealth-stealth] (1.5,0) -- (1.5,3);
		\node[left] at (1.5,1.5) {\footnotesize$U$};
		\node at (3.75,-0.5) {\footnotesize $\left\{ \ket{\uparrow,\uparrow} , \ \ket{\uparrow,\downarrow} ,\ \ket{\downarrow,\uparrow} , \ \ket{\downarrow,\downarrow}  \right\} $};
		\node at (3.75, 2.5) {\footnotesize $\left\{ \ket{\uparrow\downarrow,0} , \ \ket{0, \uparrow\downarrow}  \right\} $};
		\draw[stealth-] (5.7,0) -- (6.2,0) node[right] {\footnotesize ground};
		\node at (6.5,3) { $\hat{H}_0$};
	\end{tikzpicture}
	\end{minipage}
\end{center}
Visto che $U$ pesa molto rispetto a $t$, gli stati eccitati rispetto ad $\hat{H}_0$ si trascurano, cio\`e non si tiene conto della possibilit\`a di avere due spin in un singolo sito.

Come base per il ground, si scelgono gli stati di tripletto e di singoletto, relativi a $\ket{S,\ S_z} $:
\[
\hspace{1cm}\begin{cases}
	\ket{1,1} = \ket{\uparrow, \uparrow} \\
	\ket{1,0} = \displaystyle \frac{1}{\sqrt{2} } \big(\ket{\uparrow,\downarrow} + \ket{\downarrow,\uparrow} \big) \\
	\ket{1,-1} = \ket{\downarrow,\downarrow} 
\end{cases} \hspace{1cm} \ket{0,0} = \frac{1}{\sqrt{2} } \big(\ket{\uparrow,\downarrow} - \ket{\downarrow,\uparrow} \big) 
\] 
Su due di questi stati, il termine perturbativo si annulla: l'hopping sugli stati $\ket{\uparrow,\uparrow} $ e $\ket{\downarrow,\downarrow} $ deve fare zero, altrimenti si andrebbero a inserire due stati con uguale spin nello stesso livello energetico, cosa impedita dal principio di Pauli.
\begin{boxenv}[]
Per svolgere i calcoli con gli operatori di creazione e distruzione, viste le regole di anti-commutazione, \`e necessario scegliere un ordinamento. Si seguono i due seguenti punti:
\begin{itemize}
	\item per i siti, si costruisce da quello con indice pi\`u alto, verso quello con indice pi\`u basso;
	\item per gli spin, si sceglie prima lo spin up.
\end{itemize}
\end{boxenv}

\begin{esempio}
Si svolge il calcolo per mostrare che $\delta \hat{H} \ket{\uparrow,\uparrow} =0 $. 
Per l'ordinamento scelto sopra, $\ket{\uparrow,\uparrow} = \hat{c}^\dagger _{1,\uparrow} \hat{c}^\dagger _{2,\uparrow}\ket{0,0}  $.
Inoltre, si nota che la somma sullo spin consiste di solo lo spin up perch\'e il termine con spin down contiene degli operatori di distruzione $\hat{c}_{i,\downarrow} $ che, agendo su $\ket{0} $, restituiscono il vettore nullo per definizione.
Allora, si vede che
\[
	\begin{split}
	&\hat{c}^\dagger _{1,\uparrow} \hat{c}_{2,\uparrow} \hat{c}^\dagger _{1,\uparrow} \hat{c}^\dagger _{2,\uparrow} \ket{0} =- \overbracket{(\hat{c}^\dagger _{1,\uparrow} )^2}^{=0} \hat{c}_{2,\uparrow} \hat{c}^\dagger _{2,\uparrow} \ket{0} = 0 \\
	&\hat{c}^\dagger _{2,\uparrow} \hat{c}_{1,\uparrow} \hat{c}^\dagger _{1,\uparrow} \hat{c}^\dagger _{2,\uparrow} \ket{0} = \hat{c}_{1,\uparrow} \hat{c}^\dagger _{1,\uparrow} (\hat{c}^\dagger _{2,\uparrow} )^2 \ket{0}  = 0
	\end{split}
\] 
da cui $\delta \hat{H} \ket{\uparrow,\uparrow} =0$.
\end{esempio}
\noindent Allora $\delta \hat{H} \ket{1,1} = \delta \hat{H} \ket{1,-1} =0$.
Per gli altri due termini, si calcola come agisce sugli stati $\ket{\uparrow,\downarrow} $ e $\ket{\downarrow,\uparrow} $.
Si osserva che, per $\ket{\uparrow,\downarrow} = \hat{c}^\dagger _{1,\uparrow} \hat{c}^\dagger _{2,\downarrow} \ket{0} $, si ha:
\[
	\left(\cancel{\hat{c}^\dagger _{1,\uparrow} \hat{c}_{2,\uparrow}} + \hat{c}^\dagger _{2,\uparrow}\hat{c}_{1,\uparrow} +\hat{c}^\dagger _{1,\downarrow} \hat{c}_{2,\downarrow} + \cancel{\hat{c}^\dagger _{2,\downarrow} \hat{c}_{1,\downarrow}}  \right) \hat{c}^\dagger _{1,\uparrow} \hat{c}^\dagger _{2,\downarrow} \ket{0} = \left(\hat{c}^\dagger _{2,\uparrow}\hat{c}_{1,\uparrow} +\hat{c}^\dagger _{1,\downarrow} \hat{c}_{2,\downarrow} \right) \hat{c}^\dagger _{1,\uparrow} \hat{c}^\dagger _{2,\downarrow} \ket{0}
\] 
visto che si formano $(\hat{c}^\dagger _{i,\sigma } )^2$.
Per finire, basta osservare che $\hat{c}_{i,\sigma } \hat{c}^\dagger _{i,\sigma } = \mathds{1}$, per cui 
\[
\delta \hat{H} \ket{\uparrow,\downarrow} = - t \left(\hat{c}^\dagger _{2,\uparrow} \hat{c}^\dagger _{2,\downarrow}  - \hat{c}^\dagger _{1,\downarrow} \hat{c}^\dagger _{1,\uparrow} \right) \ket{0} 
\] 
Analogamente, si trova che $\delta \hat{H} \ket{\downarrow,\uparrow}=t(\hat{c}^\dagger _{2,\uparrow} \hat{c}^\dagger _{2,\downarrow}  - \hat{c}^\dagger _{1,\downarrow} \hat{c}^\dagger _{1,\uparrow} ) \ket{0}  $.
Questo permette di concludere che:
\[
\delta \hat{H} \ket{1,0} =0 \hspace{1cm}\delta \hat{H}\ket{0,0} = -\frac{2t}{\sqrt{2} } \big(\ket{\uparrow\downarrow,0} + \ket{0,\uparrow\downarrow} \big) 
\] 
Pertanto, il secondo ordine perturbativo\footnote{Il primo \`e relativo al fondamentale, mentre $\delta \hat{H}$ porta solo stati eccitati, quindi \`e nullo.} (degenere), indicando con $\ket{s} $ il singoletto e $\ket{m} $ gli stati $\ket{\uparrow\downarrow,0} $ e $\ket{0,\uparrow\downarrow} $, \`e 
\[
\delta E^{(2)}_{\ket{s} } = \sum_{m\neq s}^{} \frac{\braket{s|\delta \hat{H}|m} \braket{m|\delta \hat{H}|s} }{E_s^{(0)}- E_m^{(0)}} = \frac{2t \cdot 2t}{0 - U}= - \frac{4t^2}{U}
\] 
In $U\gg t$, quindi, l'effetto dell'hopping \`e quello di portare il sistema nello stato di singoletto, quindi il sistema pu\`o essere descritto da un hamiltoniano che proietta gli stati sul singoletto, con un'ampiezza proprio pari a $\delta E^{(2)}_{\ket{s} } $, mentre il tripletto rimane invariato.
Questa operazione di proiezione si pu\`o scrivere tramite il termine di Heisenberg $\hat{\mathbf{S} }_i \cdot \hat{\mathbf{S} }_j$; infatti:
\[
...
\] 
Applicato sul singoletto, questo restituisce $-\frac{3}{4}$; per il tripletto, invece, si ha $\frac{1}{4}$.
In questo modo, il proiettore sul singoletto \`e 
\[
\hat{P}(i,j) = \frac{1}{4} - \hat{\mathbf{S} }_i \cdot \hat{\mathbf{S} }_j
\] 
perch\'e \`e l'identit\`a sul singoletto, mentre si annulla sul tripletto.
Allora, per due siti, si ha l'hamiltoniano efficace
\[
	\hat{H}_\text{eff} (1,2) = - J \hat{\mathbf{S} }_1 \cdot \hat{\mathbf{S} }_2 + \cancel{\text{cost}} 
\] 
con $J = -\frac{4t^2}{U}$ e il termine costante si trascura (relativo a $1 / 4 $ nel proiettore).
Allora, per $N$ siti, si trova proprio l'hamiltoniano di Heisenberg:
\[
\hat{H} = \frac{4t^2}{U}\sum_{\langle i,j \rangle}^{} \hat{\mathbf{S} }_i \cdot \hat{\mathbf{S} }_j
\] 
\subsection{Modello di Ising quantistico 1D}
\`E definito, in dimensione generica, come
\[
\hat{H} = - J \sum_{\langle i,j \rangle}^{} \hat{\sigma }_i ^z \hat{\sigma }_j^z - g \sum_{i}^{} \hat{\sigma }^x_i
\] 
dove le dimensioni entrano nel reticolo, quindi negli indici $i,j$. 
In 1D, per esempio, $\langle i,j \rangle$ si concretizza in $(i,i-1)$ e $(i,i+1)$.
Con questo hamiltoniano, si parla di modello di Ising in \textit{campo trasverso} in quanto compare un termine $\hat{\sigma }^x_i$, che \`e relativo ad un termine di campo magnetico lungo una direzione ($x$ in questo caso) ortogonale al termine di interazione $\hat{\sigma }^z_i\hat{\sigma} ^z_j$.
Si fissa $J>0$ (quindi \`e il sistema \`e ferromagnetico) e si prende $g>0$ per convenzione.
\begin{osservazione}
La differenza principale col caso classico \`e che, classicamente, gli spin puntano in un'unica direzione; qui, gli spin possono essere orientati lungo un'intera sfera unitaria, quindi il caso classico si avrebbe orientando il termine magnetico lungo $z$, invece che lungo $x$.
Il termine di campo genera un effetto che \`e sostanzialmente analogo a quello della temperatura nel caso classico nel far fluttuare l'orientazione degli spin.

Inoltre, non si aggiunge un termine di campo magnetico lungo $z$ perch\'e complicherebbe estremamente la soluzione del sistema.
\end{osservazione}
\subsubsection{Introduzione}
\begin{boxenv}[]
\centering Nel seguito, si d\`a una descrizione qualitativa del sistema e delle caratteristiche che si possono osservare.
\end{boxenv}
\noindent Classicamente, si individuava un valore di temperatura critica; ora si fissa la temperatura a zero e si fanno variare i parametri $g$ e $J$, per l'osservazione precedente.
Si andranno a studiare, quindi, i casi: (a) $g / J \gg 1$; (b) $g / J \ll 1$.
\vspace{.5cm}

\textbf{Caso (a).} Nel caso in cui domina il campo magnetico trasverso, lo stato fondamentale \`e relativo a spin scorrelati tra loro e puntano tutti lungo $x$:
\[
\ket{\psi _\text{gs}} \simeq \prod_j \ket{\leftarrow} _j
\] 
dove
\[
	\begin{split}
		&\ket{\leftarrow} = \frac{1}{\sqrt{2} }\big(\ket{\uparrow} +\ket{\downarrow} \big)\hspace{1cm}\hat{\sigma }^x \ket{\leftarrow} = \ket{\leftarrow} \\
		&\ket{\rightarrow} = \frac{1}{\sqrt{2} }\big(\ket{\uparrow} -\ket{\downarrow} \big)\hspace{1cm}\hat{\sigma }^x \ket{\rightarrow} = -\ket{\rightarrow}
	\end{split}
\] 
cio\`e $\ket{\leftarrow} $ e $\ket{\rightarrow} $ sono gli autovettori di $\hat{\sigma }^x$.
Pi\`u precisamente, tramite teoria perturbativa in $1 / g$, si vede che il correlatore
\[
\braket{\psi _\text{gs}|\hat{\sigma }^z_i \hat{\sigma }^z_j|\psi _\text{gs}} \sim e^{- \lvert i-j \rvert / \xi }
\] 
con $\xi $ certa lunghezza di correlazione finita.
Nel caso in cui $g\to +\infty$, il correlatore tende a zero, in quanto gli spin sono totalmente scorrelati; di fatto, $\braket{\leftarrow|\hat{\sigma }^z|\leftarrow}  =0$.
\vspace{.5cm}

\textbf{Caso (b).} Nel caso di $g=0$, lo stato fondamentale \`e degenere, in quanto vi sono due possibili disposizioni del sistema: tutti spin up o tutti spin down
\[
\ket{\psi _{\text{gs},1} }  = \prod_j \ket{\uparrow} _j \hspace{1cm} \ket{\psi _{\text{gs}, 2} }  =\prod_j \ket{\downarrow} _j
\] 
Cio\`e il sistema \`e invariante per scambio da tutti spin up a tutti spin down, espressa tramite l'operatore $\hat{P}=\prod_j \hat{\sigma }_j^x$:
\[
\hat{P}\ket{\psi _{\text{gs},1} } = \ket{\psi _{\text{gs},2} }  \hspace{1cm} \hat{P} \ket{\psi _{\text{gs},2} } = \ket{\psi _{\text{gs},1} } 
\] 
Infatti, si pu\`o dimostrare, usando le regole di commutazione delle matrice di Pauli, che $[\hat{H}, \hat{P}] = 0$, per cui si pu\`o trovare una base comune per i due:
\[
\ket{\phi _{1,2} } = \frac{1}{\sqrt{2} } \big(\ket{\psi _{\text{gs},1} } \pm \ket{\psi _{\text{gs}} ,2}  \big)
\] 
Inoltre, in questo regime $g=0$, si vede che il correlatore \`e
\[
\braket{\psi _{\text{gs},1 / 2} |\hat{\sigma }_i^z \hat{\sigma }^z_j|\psi _{\text{gs},1 / 2} }  = 1
\] 
con $\psi _{\text{gs},1 / 2}$ che indica l'equivalenza di usare l'uno o l'altro stato.
A $g>0$ (piccolo), invece, tramite teoria perturbativa in $g$, si pu\`o dimostrare che 
\[
\lim_{\lvert i-j \rvert  \to +\infty} \braket{\psi _\text{gs}|\hat{\sigma }^z_i \hat{\sigma }^z_j|\psi _\text{gs}} = m_0^2 \neq 0
\] 
con $0<m_0^2< 1$.
Inoltre, per taglie finite (cio\`e per lunghezza della catena $L<+\infty$), la degenerazione dello stato fondamentale sparisce e si osserva un unico stato fondamentale, che commuta, quindi, con $\hat{P}$.
La richiesta di avere un singolo stato fondamentale implica l'impossibilit\`a di trovare, nello stato fondamentale del sistema, una certa simmetria di spin up e spin down come nel caso $g=0$, altrimenti l'operatore parit\`a non potrebbe mandarlo in se stesso e si avrebbe degenerazione.
La richiesta di commutazione con la parit\`a per questo stato fondamentale si traduce in $\braket{\psi _\text{gs}|\hat{\sigma }_i^2|\psi _\text{gs}} =0$.

In ogni caso, studiando il limite termodinamico, quindi per $L\to +\infty$, si osserva la ricomparsa della degenerazione dello stato fondamentale\footnote{Questa non si manifesta nella comparsa degli stessi $\ket{\psi _{\text{gs},1 / 2} } $ visti in precedenza, ma in qualcosa di pi\`u complicato.}. 
Studiando il correlatore in questo caso, si pu\`o osservare una correzione:
\[
\braket{\psi_\text{gs} |\hat{\sigma }_i^z \hat{\sigma }_j^z|\psi _\text{gs}} \simeq m_0^2 + e^{- \lvert i - j \rvert / \xi }
\] 
dove $\xi  \equiv \xi (g)$ finita\footnote{In realt\`a, anche la $\xi $ introdotta nel caso (a) \`e dipendente da $g$, quindi \`e una $\xi (g)$.}. 
Questo evidenzia come, dopotutto, non ci sia molta differenza tra il caso (a) e il caso (b), a parte la presenza di una magnetizzazione finita nel secondo; tuttavia, \`e possibile dimostrare che i due casi perturbativi non possono essere collegati analiticamente: nel mezzo (nel limite termodinamico), accade un fenomeno non descrivibile perturbativamente, riconducibile ad una transizione di fase.
Si pu\`o vedere, poi, che il punto critico in 1D \`e $(g / J)_c = 1$.
\begin{center}
	\begin{tikzpicture}
		\draw[-stealth](0,0) -- (10,0) node[below right] {\footnotesize $g / J$};
		\draw[thick] (0,-0.10) -- (0,0.10);
		\draw[very thick,red!70!gray] (5,-0.10) -- (5,0.10);
		\node[below] at (0,0) {\footnotesize $0$};
		\node[below,red!60!gray] at (5,0) {\footnotesize $\left(g / J\right) _c$};
		\node[above] at (8.5,0) {\footnotesize $\langle \hat{\sigma }^z_i \hat{\sigma }_j^z \rangle\sim 0$};
		\node[above] at (1.5,0) {\footnotesize $\langle \hat{\sigma }^z_i \hat{\sigma }_j^z \rangle\sim m_0^2$};
		\node[below] at (7.5,0) {\footnotesize paramagnete};
		\node[below] at (7.5,-0.3) {\footnotesize quantistico};
		\node[below] at (2.5,0) {\footnotesize ferromagnete};
		\node[below] at (2.5,-0.3) {\footnotesize quantistico};
	\end{tikzpicture}
\end{center}
\begin{center}
	\begin{minipage}
		{.45\columnwidth}
	Di fatto, si pu\`o vedere che l'andamento del correlatore in $g / J$ \`e analogo a quello che si vedeva nel modello di Ising classico per la magnetizzazione al variare della temperatura.
	Questo evidenzia come, effettivamente, il parametro $g$ nel caso quantistico sortisca un effetto analogo alla temperatura nel caso classico.
	\end{minipage}
	\hfill
	\begin{minipage}
		{.45\columnwidth}
...
% Assi
\draw[->, thick] (0,0) -- (7,0) node[right] {$g / J$};
\draw[->, thick] (0,0) -- (0,5) node[above] {$\langle \hat{\sigma }^z _i \hat{\sigma }^z _j\rangle$};


% Etichetta Tc
\node[below] at (Tc) {$\left(g/J\right)_c $};

% Curva (fase ordinata) - rossa
\draw[thick, blue!30!white]
    (0,3.6)
    .. controls (1.5,3.4) and (2.5,2.2) .. (3.2,0);
\node[left] at (0,3.6) {$1$};
% Linea per T > Tc (fase disordinata) - blu
\draw[thick, blue!50!gray] (3.2,0) -- (6.75,0);
% Punto critico
\coordinate (Tc) at (3.2,0);
\fill (Tc) circle (2pt);
\end{tikzpicture}
	\end{minipage}
\end{center}
Vicino alla transizione e nel limite termodinamico ($L\to +\infty$), si osservano i seguenti fenomeni.
\begin{itemize}
	\item Il gap tra stato fondamentale (spazio degenere in cui vivono gli stati fondamentali) e primo stato eccitato si chiude, con un andamento dato da (ponendo $J=1$)
		\[
		\Delta \sim \lvert g- g_c \rvert ^{z \nu }
		\] 
	Qui, $z$ e $\nu $ sono esponenti critici, con $\nu $ analogo a quello visto per Ising classico, mentre $z$ \`e un nuovo esponente critico legato specificatamente a questo tipo di transizioni di fase.
\item La lunghezza di correlazione diverge come $\xi \sim \lvert g - g_c \rvert ^{\nu }$, analogamente al caso classico.
\end{itemize}
Infine, la scelta di studiare la correlazione lungo $z$ \`e relativa al fatto che, lungo $x$ non si osservano fenomeni critici; di fatto, la magnetizzazione lungo $z$
\[
\psi = M^z = \frac{1}{L} \sum_{i=1}^{L} \braket{\psi _\text{gs}|\hat{\sigma }^z_i|\psi _\text{gs}} 
\] 
si comporta da buon parametro d'ordine, visto che \`e non-nullo nella fase in cui la simmetria \`e rotta e si annulla nella fase simmetrica, mentre $M^x$ non manifesta comportamenti simili.

\subsubsection{Trasformazione di Jordan-Wigner}
Questa soluzione si basa sul mappare queste particelle di spin $1 / 2$ in fermioni spinless, cio\`e passare da questo sistema descritto tramite gli operatori rappresentati dalle matrici di Pauli $\hat{\sigma }_i^{\alpha }$ ad un sistema descritto tramite gli operatori di creazione e distruzione $\hat{c}_i, \ \hat{c}^\dagger _i$.
Mentre le matrici di Pauli soddisfano le regole di commutazione 
\[
	[\hat{\sigma }_i^\alpha , \hat{\sigma }_j ^\beta ] = \delta _{ij} \left[ 2 i \varepsilon _{\alpha  \beta  \gamma} \hat{\sigma }_i^\gamma \right] 
\] 
gli operatori di creazione e distruzione soddisfano le regole di anti-commutazione
\[
	\{\hat{c}_i, \hat{c}_j^\dagger \} = \delta _{ij} \hspace{1cm} \{\hat{c}_i, \hat{c}_j\} = \{\hat{c}_i^\dagger , \hat{c}_j^\dagger \} = 0 
\] 
Il passaggio (mapping) da un sistema all'altro si ottiene tramite la \textit{trasformazione di Jordan-Wigner}.
\begin{esempio}
	[Tentativo di mapping] Si potrebbe pensare di definire gli operatori di scambio 
	\[
	\hat{\sigma }_j ^{\pm} = \frac{1}{2} \left(\hat{\sigma }_j^x \pm i \hat{\sigma }_j^y\right) 
	\] 
	che mandano uno spin up in down e viceversa. 
	In questo, il mapping si potrebbe individuare come 
	\[
	\hat{\sigma }_j^{+}\longmapsto \hat{c}_j^\dagger  \hspace{1cm} \hat{\sigma }_j^- \longmapsto \hat{c}_j
	\] 
cio\`e si associa l'operatore di scambio in spin up con l'operatore di creazione, mentre quello in spin down con l'operatore di distruzione; cos\`i facendo, \`e come se si individuasse $\ket{\downarrow} \equiv \ket{0} $ (vuoto) e $\ket{\uparrow} \equiv \ket{1} $.
Il problema \`e che questa formulazione funziona bene solamente su singolo sito, mentre su pi\`u siti non soddisfano le regole di commutazione desiderate.
\end{esempio}
\noindent Il mapping corretto, invece, si ottiene tramite la \textit{stringa di Jordan-Wigner}:
\begin{equation}
		\hat{\sigma }_j^{-}= \left[ \prod_{i < j} \left(1 - 2 \hat{n}_i\right)  \right] \hat{c}_j = \left[ e^{i \pi \sum_{i<j}^{} \hat{n}_i} \right] \hat{c}_j\hspace{1cm}\hat{\sigma }_j^+ = \hat{c}_j ^\dagger \left[ \prod_{i<j} \left(1-2\hat{n}_i\right)  \right] 
\end{equation}
dove $\hat{n}_i = \hat{c}^\dagger _i \hat{c}_i$ \`e l'operatore numero. 
Si osserva che, nell'espressione di $\hat{\sigma }_j^+$, l'operatore $\hat{c}_j^\dagger $ commuta con la stringa, quindi si potrebbe anche portare a destra, e $\hat{n}_i ^\dagger  = \hat{n}_i$. 
Per comodit\`a, si definisce $\hat{k}_j:=\prod_{i<j} \left(1 - 2 \hat{n}_i\right) $ come la stringa di Jordan-Wigner; in questa, si evidenzia come $i < j$ implichi la necessit\`a di definire un ordinamento, in accordo col fatto che gli operatori fermionici soddisfano delle regole di anti-commutazione.
Infine, si pu\`o dimostrare che il mapping trasforma anche le relative regole di commutazione, ma si omette.
\begin{osservazione}
L'ordinamento \`e banale in 1D, mentre diventa complicato in dimensioni superiori: si vengono a creare interazioni non locali secondo qualunque ordinamento scelto, per cui la trasformazione perde di utili\`a.
\end{osservazione}
\noindent Ora si vede come si trasforma questo modello di Ising nel modello fermionico.
Per semplicit\`a, si scambiano $x$ e $z$, quindi si considera 
\[
\hat{H} = - J \sum_{j}^{} \hat{\sigma }_j^x \hat{\sigma }_{j+1} ^x - g \sum_{j}^{} \hat{\sigma }_j^z
\] 
Tramite gli operatori di alzamento e abbassamento, si ha $\hat{\sigma }_j^x = \hat{\sigma }_j ^+ + \hat{\sigma  }_j^-$, visto che
\[
	\hat{\sigma }^x = \begin{pmatrix} 0&1 \\ 1 &0  \end{pmatrix} \hspace{.5cm}\hat{\sigma }^+ = \begin{pmatrix} 0 & 0 \\ 1 & 0 \end{pmatrix} \hspace{.5cm} \hat{\sigma }^- \begin{pmatrix}  0  & 1 \\ 0 & 0  \end{pmatrix}  
\] 
In questo modo:
\[
	\hat{H} = - J \sum_{j}^{} \Big[\hat{\sigma }_j^+ \hat{\sigma }^- _{j+1} + \hat{\sigma }_j^- \hat{\sigma }_{j+1} ^+ + \hat{\sigma }_j^+ \hat{\sigma }_{j+1} ^+	+ \hat{\sigma }_j ^- \hat{\sigma }_{j+1} ^-\Big]-g \sum_{j}^{} \hat{\sigma }_j^z
\] 
\begin{osservazione}
Si vede che i termini $\hat{\sigma }_j^+ \hat{\sigma }^- _{j+1}$, $ \hat{\sigma }_j^- \hat{\sigma }_{j+1} ^+$ conservano le eccitazioni lungo l'asse $z$, mentre gli altri due termini no; questi ultimi sono detti \textit{termini di anomalia} e sono quelli pi\`u complicati da trattare a livello analitico.
\end{osservazione}
\noindent Applicando la trasformazione di Jordan-Wigner, si ha che
\[
\hat{\sigma }_j^z = 2 \hat{\sigma }_j^+ \hat{\sigma }_j^- - 1 \longmapsto 2 (\hat{k}_j \hat{c}^\dagger _j)(\hat{k}_j \hat{c}_j) - 1 = 2 \hat{n}_j - 1
\] 
dove si \`e usato il fatto che $\hat{c}_j^\dagger \hat{k}_j = \hat{k}_j \hat{c}^\dagger _j$ e che $\hat{k}_j^2 = 1$\footnote{Visto che $1-2 \hat{n}_i$ pu\`o assumere valore pari a $\pm 1$, elevandolo al quadrato (cosa permessa perch\'e ogni fattore commuta con l'altro e si possono avvicinare fattori uguali e scriverli al quadrato), si ottiene necessariamente $1$, quindi $\hat{k}_j^2$ \`e il prodotto di tanti $1$, per cui restituisce $1$.}.
Ora si devono valutare i prodotti:
\[
	\begin{split}
		\hat{\sigma }^+_j \hat{\sigma }^-_{j+1} \longmapsto (\hat{c}^\dagger _j \hat{k}_j ) ( \hat{k}_{j+1} \hat{c}_{j+1} ) &= \hat{c}^\dagger _j \left[ \hat{k}_j \hat{k}_{j+1}  \right] \hat{c}_{j+1} \\
																    &=\hat{c}^\dagger _j \left(\prod_{i<j} 1 - 2 \hat{n}_i\right) \left(\prod_{\ell  < j+1} 1-2 \hat{n}_\ell  \right) \hat{c}_{j+1} \\
																    & = \hat{c}^\dagger _j (1-2\hat{n}_j) \hat{c}_{j+1} 
	\end{split}
\] 
usando lo stesso fatto di prima per cui $\hat{k}_j^2 = 1$ e, dunque, l'unico termine che sopravvive \`e quello dato da $\ell = j$ nella seconda produttoria, che non trova un termine con cui accoppiarsi nella prima.
Ora, usando il fatto che l'operatore di creazione $\hat{c}^\dagger _j$ applicato sullo stato pieno (in cui c'\`e gi\`a un fermione) restituisce $0$, si ha che l'espressione \`e non-banale se e solo se $\hat{n}_j = 0$, cio\`e la posizione $j$-esima \`e vuota. Allora si conclude che 
\[
\hat{\sigma }_j ^+ \hat{\sigma }_{j+1} ^- \longmapsto \hat{c}_j^\dagger \hat{c}_{j+1} 
\] 
Per ragionamenti analoghi, si possono ricavare espressioni simili per i termini rimanenti; cos\`i facendo, l'hamiltoniana di Ising trasformata \`e
\[
	\hat{H} \longmapsto - J \sum_{j}^{} \Big[\hat{c}^\dagger _j \hat{c}_{j+1} + \hat{c}^\dagger _{j+1} \hat{c}_j + \hat{c}_j^\dagger \hat{c}_{j+1} ^\dagger + \hat{c}_{j+1} \hat{c}_j\Big] - g \sum_{j}^{} \left(2 \hat{n}_j -1\right) 
\] 
Visto che $\hat{n}_j = \hat{c}_j^\dagger \hat{c}_j$, l'hamiltoniana trasformata \`e quadratica negli operatori fermionici, per cui non sono presenti veri e propri termini di interazione: questi termini quadratici corrispondono, piuttosto, a spostare fermioni da un sito all'altro.
\begin{osservazione}
Un ragionamento analogo si pu\`o applicare anche al modello di Heisenberg, ma si vede comparire un termine quartico, legato ad un'interazione tra le densit\`a di fermioni, per cui tutta la trattazione a seguire viene meno. 
\end{osservazione}
\begin{osservazione}
	[Esempi in cui la stringa non si annulla]
	Aggiungendo un termine di \textit{campo longitudinale} (quindi lungo $x$ in questa trattazione, cio\`e orientato lungo la direzione di spin), la stringa $\hat{k}_j$ non scompare e si ha la formazione di termini non quadratici:
	\[
	- h \sum_{j}^{} \hat{\sigma }_j^x = - h \sum_{j}^{} (\hat{\sigma }_j^+ + \hat{\sigma }_j^-) \longmapsto - h \sum_{j}^{} \hat{k}_j (\hat{c}_j^\dagger + \hat{c}_j)
	\] 
Un altro caso si osserverebbe se l'accoppiamento non fosse tra primi vicini (come in 2D); in questo caso, infatti, si avrebbe un termine di interazione della forma $-J \sum_{j,\ell }^{} \hat{\sigma }_j^x \hat{\sigma _\ell }^x$, con $\ell >j$, dove 
\[
\hat{\sigma }_j^+ \hat{\sigma }^-_\ell \longmapsto (\hat{k}_j \hat{c}^\dagger _j) (\hat{k}_\ell \hat{c}_\ell ) = \hat{c}^\dagger _j \prod_{j\le m\le \ell } \left(1-2 \hat{n}_m\right)\hat{c}_\ell 
\] 
\end{osservazione}
\subsubsection{Condizioni al bordo periodiche}
\begin{center}
	\begin{minipage}
		{.55\columnwidth}
	La richiesta di avere condizioni al bordo periodiche e avere invarianza per traslazioni porta a necessitare che i siti siano sostanzialmente disposti su un cerchio.
	Indicando con $L$ il numero di siti, il problema, qui, \`e dato dall'interazione tra il termine $L$-esimo e il primo; infatti, tramite trasformazione di Jordan-Wigner, essendo $\hat{k}_1 = 1$, si trova 
	\[
	\hat{\sigma }^+_L \hat{\sigma }^-_1 \longmapsto \hat{k}_L \hat{c}^\dagger _L \hat{c}_1
	\] 
	\end{minipage}
	\hfill
	\begin{minipage}
	...
	\begin{tikzpicture}
		\draw (0,0) circle (2);
		\draw[red!60!gray, very thick] ({2*cos(120)},{2*sin(120)}) arc[start angle=120, end angle=150, radius=2];
		\foreach \i in {0,30,...,360}{
			\draw[fill] ({2*cos(\i)}, {2*sin(\i)}) circle (1.5pt);
		}
		\node[above] at (0,2) {\footnotesize $2$};
		\node[above] at ({2*cos(60)},{2*sin(60)}) {\footnotesize $3$};
		\node[above] at ({2*cos(120)},{2*sin(120)}) {\footnotesize $1$};
		\node[above left] at ({2*cos(150)},{2*sin(150)}) {\footnotesize $L$};
		\node[left] at ({2*cos(180)},{2*sin(180)}) {\footnotesize $L-1$};
	\end{tikzpicture}
	\end{minipage}
\end{center}
Il termine $\hat{k}_L$ coinvolge una produttoria fino a $L-1$, che si pu\`o espandere fino a $L$ aggiungendo un segno negativo legato al fatto che, essendo presente $\hat{c}^\dagger _L$ prima dell'operatore numero, il sito $L$ \`e pieno e $\hat{n}_L = 1$, quindi il prodotto aggiuntivo porta un $-1$.
In definitiva, si ha
\[
\hat{\sigma }_L^+ \hat{\sigma }_1 ^ - \longmapsto \left[ \prod_{i=1} ^L \left(1-2\hat{n}_i\right)  \right] \hat{c}^\dagger _L \hat{c}_1
\] 
Questo prodotto nella stringa \`e indice della parit\`a del numero totale di fermioni presenti nella catena, indicato con $N_f$; infatti, si aggiunge un fattore $-1$ per ogni fermione presente, per cui si ha $\hat{\sigma }_L^+ \hat{\sigma }_1 ^ - \longmapsto - (-1)^{N_f} \hat{c}^\dagger _L \hat{c}_1$.
Questo segno negativo dato dalla parit\`a o meno della catena pu\`o portare la comparsa di un legame con segno opposto tra il sito $L$ e $1$:
\begin{itemize}
	\item per $N_f$ dispari, si hanno condizioni al bordo periodiche (PBC) e 
		\[
		\hat{H}_\text{pbc} = - J \sum_{j=1}^{L} \left(\hat{c}_j^\dagger \hat{c}_{j+1} + \ldots\right)  - g \sum_{j=1}^{L} \left(2\hat{n}_j - 1\right)
		\] 
	\item per $N_f$ pari, si hanno condizioni al bordo anti-periodiche (ABC) e 	
		\[
		\hat{H}_\text{abc} = - J \sum_{j=1}^{L-1} \left(\hat{c}^\dagger _{j} \hat{c}_{j+1} \ldots\right) - g \sum_{j=1}^{L} (2\hat{n}_j - 1) + J\left(\hat{c}^\dagger _L \hat{c}_1 +\ldots\right) 
		\] 
\end{itemize}
con $\hat{c}_{L+1} \equiv \hat{c}_1$ e $\hat{c}_{L+1} ^\dagger \equiv \hat{c}^\dagger _1$.
\begin{osservazione}
Nel limite termodinamico, cio\`e per $L\to +\infty$, questa differenza scompare, ma \`e molto rilevante altrimenti.
Infatti, si vedr\`a che questo termine aggiuntivo \`e responsabile per la comparsa della degenerazione nel caso ferromagnetico a $L$ finito.
\end{osservazione}
\subsubsection{Diagonalizzazione dell'hamiltoniano}
Avendo invarianza per traslazioni, si passa in trasformata di Fourier, scrivendo
\[
\hat{c}_j = \frac{1}{\sqrt{L} }\sum_{k}^{} \hat{d}_k e^{- i\frac{2\pi }{L} k j} ,\  j = 1 ,\ldots, L
\] 
La scelta dei $k$ dipende dalla parit\`a $(-1)^{N_f}$: $\hat{c}_{L+1} = \pm \hat{c}_1$, con $+$ se $N_f$ \`e dispari (PBC), o $-$ se $N_f$ \`e pari (ABC).
Queste diverse possibili condizioni al bordo, in trasformata di Fourier, si traducono nel richiedere che, per $j \to j + L$ (dopo un giro completo), $e^{-2 \pi i k} = \pm 1$ a seconda del caso; per convenzione, perch\'e questa condizione sia verificata, si prende
\begin{equation}\label{kval}
\begin{split}
	&j = \begin{cases}
		-\frac{L}{2} +1 ,\, -\frac{L}{2}+2,\ldots,\, \frac{L}{2}-1,\, \frac{L}{2} & \big(L \text{ pari}\big)\\
		-\frac{L-1}{2} ,\, -\frac{L-3}{2},\ldots,\, \frac{L-3}{2},\, \frac{L-1}{2} & \big(L \text{ dispari}\big)
	\end{cases}\\
	&\Rightarrow k = \begin{cases}
		-\frac{L}{2} +1 ,\, -\frac{L}{2}+2,\ldots,\, \frac{L}{2}-1,\, \frac{L}{2} & \big(N_f+L \text{ dispari}\big)\\
		-\frac{L-1}{2} ,\, -\frac{L-3}{2},\ldots,\, \frac{L-3}{2},\, \frac{L-1}{2} & \big(N_f+L \text{ pari}\big)
	\end{cases}
\end{split}
\end{equation}
Trascurando momentaneamente i problemi legati alla condizione di periodicit\`a, in generale, si ha che $\hat{H}=\sum_{k>0}^{}\hat{H}_k$, dove $\hat{H}_k$ accoppia i modi per $-k$ e $k$.
Di fatto, passando in trasformata di Fourier, si trova 
\[
\hat{H} = - J \sum_{k}^{} \left[ \hat{d}^\dagger _k \hat{d}_k \left(e^{i \frac{2\pi}{L}k} + e^{-i \frac{2\pi }{L}k} + \frac{2g}{J}\right) + e^{-i \frac{2\pi}{L}k}\left(\hat{d}^\dagger _k \hat{d}^\dagger _{-k} + \hat{d}_k \hat{d}_{-k} \right) - \frac{g}{J}  \right] 
\] 
dove si sono svolti passaggi analoghi a quanto visto per il tight binding.
Allora, raggruppando i termini con $k$ e $-k$, si ha
\[
	\hat{H}_k = \underbracket{- 2 \left[ J \cos \left( \frac{2\pi k}{L}\right) +g  \right]}_{= f_k}  \left(\hat{d}^\dagger _k \hat{d}_k -\hat{d}_{-k} \hat{d}^\dagger _{-k} \right)+ \underbracket{2i J \sin \left(\frac{2\pi k }{L}\right) }_{-i g_k}  \left(\hat{d}^\dagger _k \hat{d}^\dagger _{-k} - \hat{d}_{-k} \hat{d}_k\right) 
\] 
Questo si pu\`o scrivere in un modo pi\`u sintetico facendo uso della notazione spinoriale:
\[
	\hat{H}_k = \begin{pmatrix} \hat{d}^\dagger _k & \hat{d}_{-k}  \end{pmatrix} \begin{pmatrix} f_k & - i g_k \\ i g_k & - f_k \end{pmatrix} \begin{pmatrix} \hat{d}_k \\ \hat{d}^\dagger _{-k}  \end{pmatrix} \equiv \pmb{\Psi} _k^\dagger \mathcal{H} _k \pmb{\Psi} _k
\] 
dove 
\[
	\pmb{\Psi} _k = \begin{pmatrix} \hat{d}_k \\ \hat{d}^\dagger _k \end{pmatrix} \hspace{1cm} \mathcal{H} _k = \begin{pmatrix} f_ k & - i g_k \\ i g_k & - f_k \end{pmatrix} = f_k \hat{\sigma }^z + g_k \hat{\sigma }^y
\] 
La matrice $\mathcal{H} _k$ si diagonalizza tramite una rotazione in $x$, individuata da $\hat{R}_x (\theta _k)$, con $\theta _k = \arctan ( g_k / f_k)$ angolo reale; pi\`u precisamente
\[
	\hat{R}_x (\theta _k) = e^{i \frac{\theta _k}{2}\hat{\sigma }^x} = \cos \left(\frac{\theta _k}{2}\right) \operatorname{Id}  + i \sin \left(\frac{\theta _k}{2}\right) \hat{\sigma }^x = \begin{pmatrix} \cos \theta  _ k / 2 &  - i \sin \theta  _k / 2\\ -i \sin \theta _k / 2 & \cos \theta _k / 2\end{pmatrix} 
\] 
Pertanto, facendo uso del fatto che la rotazione \`e unitaria
\[
	\hat{H}_k = \pmb{\Psi} _k ^\dagger \mathcal{H} _k \pmb{\Psi} _k = \pmb{\Psi} _k^\dagger \underbracket{\hat{R}_x (\theta _k) \hat{R}_x^\dagger (\theta _k)}_{=1} \mathcal{H} _k\underbracket{ \hat{R}_x (\theta _k) \hat{R}^\dagger _x (\theta _k) }_{=1} \pmb{\Psi} _k = \pmb{\Phi} _k^\dagger  \begin{pmatrix} \varepsilon _k & 0 \\ 0 & - \varepsilon _k\end{pmatrix} \pmb{\Phi} _k
\] 
con
\[
	\begin{split}
		&\pmb{\Phi} _k = \hat{R}^\dagger _k (\theta _k) \pmb{\Psi} _k = \begin{pmatrix} \cos \theta  _ k / 2 & - i \sin \theta  _k / 2 \\ - i \sin \theta _k / 2 & \cos \theta _k / 2 \end{pmatrix} \begin{pmatrix} \hat{d}_k \\ \hat{d}^\dagger _{-k}  \end{pmatrix} \equiv \begin{pmatrix} \hat{b}_k \\ \hat{b}^\dagger _{-k}  \end{pmatrix} \\
		& \varepsilon _k = 2 J \sqrt{1 + \frac{g^2}{J^2} + \frac{2g}{J}\cos \left(\frac{2\pi k}{L}\right) }
	\end{split}
\] 
Qui, gli operatori $\hat{b}_k, \ \hat{b}^\dagger _{-k} $ sono relativi alle quasi-particelle di Bogoliubov, tramite cui si diagonalizza il problema, analoghe a quelle viste per i bosoni.
Le espressioni per questi operatori si ottengono definendo 
\[
u_k = \cos \theta _k / 2 = f_k / \varepsilon _k \hspace{1cm} v_ k = \sin \theta _k / 2  = g_ k / \varepsilon _k
\] 
e usando la \textit{trasformazione di Bogoliubov} 
\[
\begin{cases}
	\hat{b}_k = u _k \hat{d}_k - i v_k \hat{d}^\dagger _{-k} \\
	\hat{b}^\dagger _{-k}  = - i v _k \hat{d}_k + u _k \hat{d}^\dagger _{-k} 
\end{cases}
\] 
Si pu\`o dimostrare, facendo uso del fatto che $u_k^2 + v_k ^2 = 1$, che $\hat{b}_k$ e $\hat{b}^\dagger _k$ soddisfano le regole di anti-commutazione, quindi sono operatori fermionici.
In definitiva, si ha
\[
\hat{H} _k = \varepsilon _k \left(\hat{b}_k^\dagger \hat{b}_k + \hat{b}^\dagger _{-k} \hat{b}_{-k} -1\right) 
\] 
\subsubsection{Stato fondamentale e stati eccitati}
Lo stato fondamentale si vede essere, analogamente a quanto visto per i bosoni, lo stato di vuoto delle quasi particelle, indicato con $\ket{\Omega } $, che, quindi, soddisfa $\hat{b}_k \ket{\Omega } = 0, \ \forall k$.
\begin{center}
...
\begin{axis}[
    ...
    xticklabels={$ \pi $},
    ytick=\empty,
    xlabel={$ \dfrac{2\pi \tilde{k}}{L} $},
    xlabel style={at={(axis description cs:1.02,0.03)},anchor=west},
    ylabel={$ \varepsilon_{\tilde{k}} $},
...
% g != 1
\def\g{1.6}
\addplot[thick, smooth, domain=0:pi] {2*sqrt(1+\g*\g-2*\g*cos(deg(x)))};
\node[anchor=west] at (axis cs:1.35,4.55) {$g\neq 1$};

% g = 1
\addplot[thick, smooth, domain=0:pi] {2*sqrt(2-2*cos(deg(x)))};
\node[anchor=west] at (axis cs:1.55,2.55) {$g=1$};


% mark the left value 2|g-1|
\node[anchor=east] at (axis cs:0,1.2) {$2|g-1|$};

% annotations
\node[rotate=30] at (axis cs:0.50,2.5) {\scriptsize quadratico $\propto \tilde{k}^2$};
\node[rotate=30] at (axis cs:0.55,0.55) {\scriptsize lineare $\propto \tilde{k}$};

\end{axis}
\end{tikzpicture}
\captionof{figure}{Andamento dell'energia delle quasi-particelle del sistema.}
\end{center}
Ora si studia l'andamento dell'energia.
Per semplicit\`a, si definisce una sua versione traslata per $\tilde{k}= k -  L / 2$, da cui $\varepsilon _{\tilde{k}} = 2 \sqrt{1 + g^2 - 2g \cos( 2\pi \tilde{k} / L )}  $, con $J = 1$.
	\begin{center}
		\begin{minipage}
			{.45\columnwidth}
	Si vede che, per $g=1$, si ha un sistema gapless per cui passare al primo stato eccitato costa arbitrariamente poco in energia, al contrario del caso per $g\neq 1$.
	Analizzando il comportamento del gap $\Delta $ al variare di $g$, si osserva una discontinuit\`a per $g=1$, relativa ad un punto critico, nel quale il sistema \`e gapless.
		\end{minipage}
		\hfill
		\begin{minipage}
			{.45\columnwidth}
		\centering
		\begin{tikzpicture}[scale=1.3]
		\draw[blue!60!gray] (0,2) -- (2,0) -- (4,2);
		\draw[-stealth] (-.25,0) -- (5,0) node[below] {\footnotesize$g$};
		\draw[-stealth] (0,-.25) -- (0,3) node[above] {\footnotesize $\Delta $};
		\draw[thick] (0.1,2) -- (-0.1,2) node[left] {\footnotesize $2$};
		\draw[thick] (2,0.1) -- (2,-.1) node[below] {\footnotesize $1$};
		\node[blue!60!gray] at (3.5,2.8) {\scriptsize $\Delta = 2 \lvert g-1 \rvert $};
		\draw[blue!70!gray,dashed,thick] (2,0) circle (4pt);
	\end{tikzpicture}
		\end{minipage}
	\end{center}
Per taglia finita, dunque, si osserva che
\[
\lim_{\tilde{k} \to 0} \varepsilon _{\tilde{k}} = \begin{cases}
	\displaystyle \frac{4\pi}{L}\tilde{k} &,\ g=1 \hspace{.25cm} (\Delta \sim 1 / L)\\
	\\
	\displaystyle 2\lvert 1-g \rvert + \frac{4\pi^2 g}{\lvert g-1 \rvert L^2}\tilde{k}^2&,\ g\neq 1
\end{cases}
\] 

\begin{osservazione}
Per tutta questa trattazione, non si \`e considerato il problema (relativo al caso di taglia finita) relativo a $N_f$ pari o dispari e si tratter\`a opportunamente pi\`u avanti.
\end{osservazione}
\subsubsection{Accoppiamento dei momenti e stati fondamentali}

Ora si considera il problema dell'accoppiamento dei momenti legato alle condizioni al contorno. 
Tenendo in considerazione i valori di $k$ possibili stabiliti in \ref{kval} a seconda della parit\`a di $N_f$, le possibilit\`a sono riassunte nella seguente tabella.
\begin{table}[h!]
	\centering
	\begin{tabular}{c| l | l |}
	\multicolumn{1}{c}{} & \multicolumn{1}{c}{$N_f$ pari} & \multicolumn{1}{c}{$N_f$ dispari} \\
		\cline{2-3}
	$L$ pari & \makecell[l]{Si sceglie $k$ per $N_f + L $ pari\\ ogni termine \`e semi-intero \\ si possono accoppiare $k$ con $-k$\\ $\Rightarrow \hat{H}_\text{abc} = \sum_{k>0}^{} \hat{H}_k$} & \makecell[l]{Rimangono spaiati $k=0, L / 2$\\ $\Rightarrow \hat{H}_\text{pbc} = \sum_{k>0}^{} \hat{H}_k + \hat{H}_0 + \hat{H}_{L / 2} $}\\
		\cline{2-3}
	$L$ dispari & \makecell[l]{Rimane spaiato $k= L / 2$\\ $\Rightarrow \hat{H}_\text{abc}= \sum_{k>0}^{} \hat{H}_k + \hat{H}_{L / 2} $} &\makecell[l]{Rimane spaiato $k=0$\\ $\Rightarrow \hat{H}_\text{pbc} = \sum_{k>0}^{} \hat{H}_k + \hat{H}_0$} \\
		\cline{2-3}
	\end{tabular}
\end{table}

\noindent Sostituendo i relativi valori, si vede che $\hat{H}_0 = - 2 (J+g) \hat{n}_0 + g $ e $ \hat{H}_{L / 2}  = 2 ( J - g) \hat{n}_{L / 2}  + g$, con $\hat{n}_0 = \hat{d}^\dagger _0 \hat{d}_0$ e $\hat{n}_{L / 2}  = \hat{d}^\dagger _{L / 2}  \hat{d}_{L / 2} $.
Ora, il sistema che si sta studiando ha un totale di $L$ siti, per cui lo spazio di Hilbert $\mathcal{H} $ relativo ha dimensione $2^L$.
Il numero di fermioni, per\`o, non \`e fissato; per tale ragione, $\mathcal{H} $ contiene due sottospazi disgiunti, uno relativo a $N_f$ pari, l'altro relativo a $N_f$ dispari.
Questo si traduce in due hamiltoniani differenti, a seconda del sottospazio in cui ci si trova; l'idea \`e studiare gli stati fondamentali del sistema relativi ai due sottospazi di Hilbert distinti e confrontare i loro valori energetici.
Formalmente, si scrive che $\mathcal{H}  = \mathcal{H} _{+}  \oplus \mathcal{H} _-$, con $\mathcal{H} _+$ relativo a $N_f$ pari e $\mathcal{H} _-$ relativo a $N_f$ dispari: nel primo, si avr\`a $\hat{H}_\text{abc}$; nel secondo, $\hat{H}_\text{pbc}$.
\begin{center}
	\begin{minipage}
		{.45\columnwidth}
	Per fare ci\`o, si osserva, preliminarmente, che, secondo $\hat{H}_0$, l'energia necessaria per avere una particella a $k=0$ \`e $\varepsilon _0 = -2 (J+g)$; per $\hat{H}_{L / 2} $, l'energia per avere una particella a $k=L / 2$ \`e $\varepsilon _{L / 2} = 2 (J-g)$.
	Per il seguito, si assume, per semplicit\`a, $J=1$, permesso dal fatto che \`e una costante di accoppiamento arbitraria e positiva.
	In questo caso, si ottiene il grafico a destra per le energie.
	\end{minipage}
	\hfill
	\begin{minipage}
		{.45\columnwidth}
	\centering
	\begin{tikzpicture}
	\draw[-stealth] (-3,0) -- (3,0) node[below] {\footnotesize $g$};
	\draw[-stealth] (0,-3) -- (0,3) node[above] {\footnotesize $\varepsilon$};
	\draw[thick](0.1,2) -- (-0.1,2) node[left] {\footnotesize $2$};
	\draw[thick](0.1,-2) -- (-0.1,-2) node[left] {\footnotesize $-2$};
	\draw[thick](1, 0.1) -- (1, -0.1) node[below] {\footnotesize $1$};
	\draw[thick](-1, 0.1) -- (-1, -0.1) node[below left] {\footnotesize $-1$};

	\coordinate (A) at (0,2);
	\coordinate (B) at (1,0);
	\draw[thick] ($(A)!-0.25!(B)$) -- ($(A)!1.5!(B)$);
	\draw[dashed, thick] ($(A)!-0.25!(B)$)--($(A)!-0.5!(B)$);
	\draw[dashed,thick]($(A)!1.5!(B)$) -- ($(A)!1.75!(B)$);

	\coordinate (C) at (-1,0);
	\coordinate (D) at (0,-2);
	\draw[thick] ($(C)!-0.75!(D)$) -- ($(C)!1.25!(D)$);
	\draw[dashed, thick] ($(C)!-0.75!(D)$)--($(C)!-1!(D)$);
	\draw[dashed,thick]($(C)!1.25!(D)$) -- ($(C)!1.5!(D)$);

	\node at (1.8,0.75) {\scriptsize $\varepsilon _{L / 2} = 2(1-g) $};
	\node at (1.5,-2.3) {\scriptsize $\varepsilon _{0} = -2(1+g) $};
	\end{tikzpicture}
	\end{minipage}
\end{center}

\textbf{Caso $\mathbf{g>1}$.} Questo corrisponde ad una fase paramagnetica.
Dal grafico sopra, \`e evidente che $\varepsilon _{L / 2} , \varepsilon _0 < 0$, quindi i modi $k=0, L / 2$ sono energeticamente favoriti da $\hat{H}_0$ e $\hat{H}_{L / 2} $.
		
		Se $L$ \`e pari, si devono distinguere i casi $N_f$ pari e $N_f$ dispari.
		Nel primo caso, l'hamiltoniano \`e $\sum_{k>0}^{} \hat{H}_k$ e lo stato fondamentale \`e quello di vuoto delle quasi-particelle.
		Nel secondo caso, invece, si ha $\sum_{k>0}^{} \hat{H}_k + \hat{H}_0 + \hat{H}_{ L / 2}$, per cui sorge un problema: gli $\hat{H}_k$ restituiscono energia minima per tutti gli altri modi vuoti, mentre gli altri due termini vogliono aggiungere particelle con modi $k=0,L / 2$; cos\`i facendo, per\`o, si finisce in $N_f$ pari.
		Quindi, il ground state in questo caso non \`e quello a energia idealmente pi\`u bassa; piuttosto, il sistema preferir\`a riempire con una singola quasi-particella il modo a energia minore, ossia quello per $k=0$.
		Mettendo tutto insieme, si vede che i due ground state, per $N_f$ pari e dispari, non si trovano a contatto, non sono in competizione, in quanto la differenza di energia tra i due \`e finita.

		Se $L$ \`e dispari, si ha una condizione analoga: se $N_f$ pari, si considera $\hat{H}_{\text{abc}} $, ma la minimizzazione ideale richiederebbe di avere il vuoto per gli $\hat{H}_k$ e una particella in $L / 2$ per $\hat{H}_{L / 2} $, che porterebbe a $N_f$ dispari, quindi questo minimo non \`e mai raggiungibile; se $N_f$ \`e dispari, si ha una situazione identica, ma con $\hat{H}_0$, quindi, in questo caso, il minimo ideale si pu\`o raggiungere perch\'e si prende il vuoto per ogni $k$ accoppiati e una particella in $k=0$. 
\vspace{.5cm}

\textbf{Caso $\mathbf{g<-1}$.} Anche questo caso corrisponde ad una fase paramagnetica. 
Si vede dal grafico che $\varepsilon _0, \ \varepsilon _{L / 2}  > 0$.
Per $L$ pari, di nuovo $\hat{H}_\text{abc}$ permette di raggiungere il minimo ideale; se $N_f$ dispari, per\`o, visto che $\varepsilon _0, \ \varepsilon _{L / 2}  > 0$, non si vorrebbe mettere neanche una particella, ma $0$ non \`e pari, quindi il ground state non \`e ammesso.
Verificando per ragionamenti analoghi, anche per $L$ dispari si ha un caso in cui il ground state \`e ammesso e un altro in cui no.
\vspace{.5cm}

\textbf{Caso $\mathbf{-1 < g< 1} $.} Questo \`e il caso della fase ferromagnetica.
Qui succede qualcosa di diverso rispetto ai casi precedenti: entrambe le opzioni, $N_f$ pari e dispari, ammettono ground state.
Questo \`e dato dal fatto che, in questo range, $\varepsilon _0 < 0$, mentre $\varepsilon _{L / 2} > 0$, per cui:
\begin{itemize}
	\item se $L$ \`e pari, $\hat{H}_\text{abc}$ ammette ground state come al solito, mentre anche $\hat{H}_\text{pbc}$ lo ammette perch\'e, questa volta, la minimizzazione dell'energia \`e legata solo ad aggiungere una particella in $k=0$, quindi il vincolo $N_f$ dispari \`e ancora soddisfatto;
	\item se $L$ \`e dispari, invece, $\hat{H}_\text{abc}$ ammette ground state perch\'e non si vuole una particella per $k = L / 2$, in quanto aumenterebbe l'energia, quindi $N_f$ pari \`e soddisfatto, mentre $\hat{H}_\text{pbc}$ ha ground state perch\'e $\varepsilon _0 < 0 $ permette di minimizzare l'energia avendo una particella in $k=0$ e vuoto per tutto il resto.
\end{itemize}
\vspace{.5cm}
La presenza di due ground state \`e il motivo per cui si presenta la degenerazione nella fase ferromagnetica nel limite termodinamico: nel caso di taglia finita, i due ground state appartenenti ai due sottospazi di Hilbert differenti sono separati da un gap energetico finito, la cui differenza \`e un esponenziale in $L$; mandando $L\to +\infty$, allora, questa differenza tende a zero.
Nella fase paramagnetica, invece, gli stati sono separati da un gap che non dipende da $L$, per cui anche nel limite termodinamico, rimangono distinti.
\begin{osservazione}
La fisica del sistema sarebbe rimasta identica, nel caso in cui non si fosse assunta invarianza per traslazioni, ma i calcoli si sarebbero complicati notevolmente.
\end{osservazione}
\begin{osservazione}
Volendo, \`e possibile studiare come si popolano gli stati eccitati, fenomeno che segue la distribuzione di Fermi-Dirac per effetto di eccitazione del sistema legato ad una temperatura finita.
\end{osservazione}
\subsubsection{Funzioni di correlazione}
Si fa uso del teorema di Wick per concludere che, avendo a che fare con un hamiltoniano diagonalizzabile e quadratico, \`e sufficiente calcolare i correlatori a due punti (correlatori della forma $\langle c c^\dagger  \rangle, \langle c c \rangle,\ldots$) per esprimere la funzione di correlazione.
Per calcolare questi correlatori, si introducono gli \textit{operatori di Majorana}:
\[
\hat{\gamma}_{2j - 1}  = \left(\prod_{i < j} \hat{\sigma }_i ^z\right) \hat{\sigma }_j^x \hspace{1cm} \hat{\gamma}_{2j}  = \left(\prod _{i < j} \hat{\sigma }_i^z\right) \hat{\sigma }_j^y
\] 
che soddisfano $\hat{\gamma}_\ell  = \hat{\gamma}_{\ell } ^\dagger $, con $\ell = -L , - L + 1 ,\ldots, L - 1$; il fatto che siano $2L$ \`e legato alla loro autoaggiuntezza.
Assumendo $L$ dispari, si considera 
\[
j = - \frac{L-1}{2} , \ - \frac{L-3}{2}, \ \ldots,\ \frac{L-1}{2} 
\] 
In termini degli operatori fermionici, questi si scrivono come 
\begin{equation}\label{mdef}
\hat{\gamma}_{2j - 1}  = \hat{c}^\dagger _j + \hat{c}_j \hspace{1cm} \hat{\gamma}_{2j}  = -i \left(\hat{c}^\dagger _j - \hat{c}_j\right) 
\end{equation}
Conoscere il correlatore di questi operatori permetter\`a di scrivere qualunque funzione di correlazione, ma \`e prima necessario calcolare i correlatori a due punti dei $\hat{\gamma}_\ell $. 
Allora, si costruisce la matrice di correlazione a due punti di Majorana
\[
	(\Gamma_L)_{m,n} = \langle \hat{\gamma}_ m \hat{\gamma}_n \rangle  , \ m,n = - L ,\ldots, L -1 
\] 
Per calcolare questo correlatore sul ground state, \`e necessario tradurre l'azione degli operatori di Majorana in termini degli operatori di Bogoliubov, in quanto si sa che il ground state \`e il vuoto delle quasi-particelle.
Infatti, si sa che il ground state \`e tale per cui $\hat{b}_k \ket{\text{gs}} = 0 , \ \forall k$, che \`e equivalente a $\bra{\text{gs}} \hat{b}^\dagger _k=0, \ \forall k$, per cui
\[
\langle \hat{b}_k \hat{b}_{k'}  \rangle= 0 \hspace{1cm} \langle \hat{b}^\dagger _k \hat{b}_{k'}  \rangle = 0 \hspace{1cm} \langle \hat{b}^\dagger _k \hat{b}^\dagger _{k'}  \rangle= 0
\] 
per ogni $k$ sul ground state.
L'unico non nullo, evidentemente, \`e $\langle \hat{b}_k \hat{b}_{k'}^\dagger  \rangle = \delta _{k k'} $.
\begin{osservazione}
In uno stato termico, cio\`e volendo calcolare il valor medio su uno stato eccitato a temperatura $\beta $, si deve passare per la traccia; in ogni caso, vale $\langle \hat{b}^\dagger _k \hat{b}^\dagger _{k'}  \rangle_\beta = \langle \hat{b}_k \hat{b}_{k'}  \rangle_\beta  = 0$.
Invece
\[
\langle \hat{b}_k \hat{b}_{k'} ^\dagger  \rangle_\beta  = \frac{\delta _{kk'} }{1 + e^{\beta  \varepsilon _k}}= \delta _{k k'} - \langle \hat{b}_{k'} \hat{b}^\dagger _k \rangle_\beta 
\] 
\end{osservazione}
\noindent Allora si scrivono $\hat{\gamma}_\ell $ in funzione dei $\hat{b}_k$; si segue lo schema seguente per arrivare al risultato finale.
\begin{center}
	\begin{tikzpicture}
		\node at (0,0) {$\left\{ \hat{\gamma}_{\ell }  \right\} $};
		\node at (3,0) {$\{ \hat{c}_j  \} $};
		\node at (6,0) {$\{ \hat{d}_k \} $};
		\node at (9,0) {$\{ \hat{b}_k  \} $};
		\draw[-stealth] (0.5,0) -- (2.5,0);
		\draw[-stealth] (3.5,0) -- (5.5,0);
		\draw[-stealth] (6.5,0) -- (8.5,0);
		\node[above] at (1.5,0) {\footnotesize def.};
		\node[above] at (4.5,0) {\footnotesize Fourier};
		\node[above] at (7.5,0) {\footnotesize Bogoliubov};
	\end{tikzpicture}
\end{center}
Mettendo insieme tutti i passaggi, si ottiene che
\[
	\begin{pmatrix} \gamma_{2 j -1}  \\ \gamma_{2j}  \end{pmatrix} = \frac{1}{\sqrt{L} } \sum_{k}^{} e^{i \frac{2\pi }{L}k_j} \underbracket{\begin{pmatrix} 1 & 1 \\ i & - i \end{pmatrix}}_{(\star)}  \begin{pmatrix} u_k & i v_k \\ i v_k & u_k  \end{pmatrix} \begin{pmatrix} \hat{b}_k \\ \hat{b}_{-k} ^\dagger \end{pmatrix} 
\] 
dove la matrice $(\star)$ serve per scambiare i $\hat{c}_j$ per le relazioni in \ref{mdef} e la matrice con gli $u_k$ e $v_k$ deriva dall'espressione degli operatori di Bogoliubov.
Facendo il conto, si pu\`o vedere che 
\[
	(\Gamma_L)_{m,n} =\delta _{m,n} + i (\Lambda )_{m,n}  \hspace{1cm} \Lambda = \begin{pmatrix} \Pi _0 & \Pi _1 & \cdots & \Pi_{L-1}\\ -\Pi_1 & \Pi_0 & \cdots & \Pi _{L-2} \\ \vdots & \vdots & \ddots & \vdots \\ -\Pi_{L-1}  & - \Pi _{L-2} & \cdots & \Pi _0\end{pmatrix} 
\] 
Si nota che $\Lambda $ \`e anti-simmetria a blocchi perch\'e relativa a fermioni; ragionamenti simili si possono vedere anche per bosoni.
I blocchi sono
\[
	\Pi_j = \begin{pmatrix} 0 & g(j) \\ - g(j) & 0  \end{pmatrix}  \hspace{.5cm} g(j) = \frac{1}{L}\sum_{k}^{} e^{i \frac{2\pi}{L}k_j}e^{i \frac{\theta _k}{2}}\xrightarrow{\, \ L\to +\infty \ \, } \frac{1}{2\pi} \int _0^{2\pi} d\phi  \ e^{i \phi_j} \frac{J \cos \phi  + g - i J \sin\phi }{\lvert J \cos \phi  + g - i J \sin \phi  \rvert }
\] 
Per scrivere $\langle \hat{\sigma}^\alpha  _i \hat{\sigma }_j^\beta \rangle$, si riprendono le definizioni degli operatori di Majorana e si invertono:
\[
\hat{\sigma }_j^x = \bigg(\prod_{i < j} \hat{\sigma }_i^z\bigg) \hat{\gamma}_{2j-1} \hspace{1cm}\hat{\sigma }_j^y = \bigg(\prod_{i < j} \hat{\sigma }_i^z\bigg) \hat{\gamma}_{2j}  \hspace{1cm} i \hat{\sigma }_j^z = \hat{\sigma }_j^x \hat{\sigma }_j^y = \hat{\gamma}_{2j-1} \hat{\gamma}_{2j} 
\] 
visto che la catena al quadrato fa $1$.
Allora
\[
\langle \hat{\sigma }^x_j \hat{\sigma }_\ell ^x \rangle= \big\langle \hat{\gamma}_{2j-1} \hat{\gamma}_{2j-1} \hat{\gamma}_{2j}  \hat{\gamma}_{2j+1} \ldots \hat{\gamma}_{2\ell  -1}   \big\rangle=\big\langle \hat{\gamma}_{2j} \ldots \hat{\gamma}_{2\ell -1}  \big\rangle
\] 
dove si \`e usato che
\[
	\hat{\gamma}_{2j-1} \hat{\gamma}_{2j-1} = \left(\hat{c}^\dagger _j + \hat{c}_j\right) \left(\hat{c}_j^\dagger + \hat{c}_j\right)  = \cancel{\hat{c}_j^\dagger \hat{c}_j^\dagger} + \underbracket{\hat{c}_j ^\dagger \hat{c}_j + \hat{c}_j \hat{c}_j ^\dagger}_{=1}   +\cancel{ \hat{c}_j \hat{c}_j}= 1
\] 
Il teorema di Wick permette di concludere che questo correlatore si scrive come prodotto di correlatori a due punti; cos\`i facendo, si arriva a calcolare
\[
\lim_{\lvert \ell -j \rvert  \to +\infty} \langle \hat{\sigma }_j^x \hat{\sigma }_\ell ^x \rangle = \psi ^2 \hspace{1cm} \psi  = \begin{cases}
	(1-g^2 ) ^{1 / \delta } &,\ \lvert g \rvert <1\\
	0&,\ \lvert g \rvert >1
\end{cases}
\] 
con $\psi $ parametro d'ordine della transizione.

\subsubsection{Modello di Ising aperto}
Si considera il modello di Ising con condizioni al contorno aperte, ossia con hamiltoniano della forma
\[
\hat{H} = - J \sum_{j=1}^{L-1} \hat{\sigma }_j ^x \hat{\sigma }^x _{j+1} - g \sum_{j}^{} \hat{\sigma }_j^z
\] 
Si fissa $g=0$.
In questo modo, ripercorrendo i passi fatti in precedenza con la trasformazione di Jordan-Wigner, si ha
\[
	\begin{split}
		\hat{H}&= - J \sum_{j=1}^{L-1} \left(\hat{c}^\dagger _j \hat{c}_{j+1} + \hat{c}_j^\dagger \hat{c}^\dagger _{j+1} + \hat{c}^\dagger _{j+1} \hat{c}_j + \hat{c}_{j+1} \hat{c}_j\right) \\
		       &= -J \sum_{j=1}^{L-1} \left(\hat{c}^\dagger _j - \hat{c}_j\right) \left(\hat{c}^\dagger _{j+1} + \hat{c}_{j+1} \right) 
	\end{split}
\] 
Usando gli operatori di Majorana, si pu\`o scrivere, ulteriormente
\[
\hat{H} = i J \sum_{j=1}^{L-1} \hat{\gamma}_{j,2} \hat{\gamma}_{j+1,1} 
\] 
con $\hat{\gamma}_{j,1} := \hat{\gamma}_{2j-1} $ e $\hat{\gamma}_{j,2} := \hat{\gamma}_{2j} $.
In questo modo, ad ogni sito, si associano due operatori di Majorana: l'hamiltoniano collega il secondo operatore di Majorana del sito $j$, col primo operatore del sito $j+1$.
Geometricamente, questo fatto si pu\`o schematizzare come segue.
\begin{center}
	\begin{tikzpicture}
		\draw (0,0) -- (10,0);
		\foreach \i in {0,2,4,6,8,10}{
			\draw[fill] (\i,0) circle (1.5pt);
			\draw[-stealth] (\i,-0.25) -- (\i,-0.75);
			\draw[thick, dashed, blue!60!gray] (\i,-1.2) ellipse (.40 and .20);
			\draw[fill,blue!60!gray] ({\i - 0.15},-1.2) circle (1.5pt);
			\draw[fill,blue!60!gray] ({\i + 0.15},-1.2) circle (1.5pt);
		}
		\node[above] at (0,0.15) {\scriptsize fermioni};
		\node[blue!60!gray] at (-1.25,-1.2) {\scriptsize Majorana};
		\foreach \j in {0,2,4,6,8}{
			\draw[very thick,blue!60!gray,opacity=0.5] ({\j+0.15},-1.2) -- ({\j+2-0.15},-1.2);
		}
		\draw[very thick,red!60!gray,opacity=0.6] (-0.15,-1.2) -- (-0.4,-1.2);
		\draw[very thick,red!60!gray,opacity=0.6,dotted] (-0.4,-1.2) -- (-0.6,-1.2);
		\draw[very thick,red!60!gray,opacity=0.6] ({10+0.15},-1.2) -- ({10+0.4},-1.2);
		\draw[very thick,red!60!gray,opacity=0.6,dotted] ({10+0.4},-1.2) -- ({10+0.6},-1.2);
		\node[red!60!gray] at ({10+1.5},-1) {\scriptsize fermione};
		\node[red!60!gray] at ({10+1.5},-1.3) {\scriptsize delocalizzato};
	\end{tikzpicture}
\end{center}
Il punto interessante \`e che il primo operatore del primo sito e l'ultimo operatore dell'ultimo sito non compaiono nell'hamiltoniano, visto che si sono scelte condizioni al bordo aperte: questi due contribuiscono alla formazione di un modo a energia zero.

Pi\`u nel dettaglio, la somma di due Majorana forma un fermione:
\[
\hat{c}_j^\dagger = \frac{1}{2}\left(\hat{\gamma}_{j,1} + i \hat{\gamma}_{j,2} \right) \hspace{1cm} \hat{c}_j = \frac{1}{2}\left(\hat{\gamma}_{j,1} - i \hat{\gamma}_{j , 2} \right)  
\] 
Questo significa che si pu\`o far comparire, andando a sommare e sottrarre gli operatori ai bordi che non figurano nell'hamiltoniano, un fermione che \`e delocalizzato per met\`a sul bordo sinistro e per met\`a sul destro:
\[
\hat{\tilde{c}}^\dagger = \frac{1}{2}\left(\hat{\gamma}_{1,1} + i \hat{\gamma}_{L,2} \right) 
\] 
Inoltre, \`e a energia zero, in quanto non figura esplicitamente nell'hamiltoniano.
L'idea particolarmente interessante dietro questo artificio \`e il suo utilizzo in computazione quantistica, per esempio: avendo un fermione in due zone arbitrariamente distanti tra loro, viene naturale immaginare che possa essere resistente a perturbazioni locali nel sistema.
\begin{osservazione}
Questa costruzione non \`e sempre possibile; per esempio, in questo caso, si \`e dovuto prendere $g=0$.
\end{osservazione}
\subsection{Mapping classico-quantistico}
Si costruisce un mapping tra il modello di Ising 1D classico in un Ising \textit{0D} quantistico, cio\`e un sistema quantistico composto da un unico spin $1 / 2$.
Si riprende l'hamitloaniana del modello di Ising classico in 1D:
\[
H = -J \sum_{j}^{} S_j S_{j+1}  - g \sum_{j}^{} S_j
\] 
con $S_j = \pm 1$.
La soluzione esatta si \`e ottenuta tramite le matrici di trasferimento\footnote{Nota: qualche variazione nel segno di $g$ sull'anti-diagonale rispetto a quanto trattato in origine.}
\[
	\hat{T} = \begin{pmatrix} e^{\beta (J+g)}& e^{\beta (-J - g)}\\ e^{\beta (-J + g)} & e^{\beta (J-g)} \end{pmatrix} \implies Z = \operatorname{Tr} \hat{T}^N
\] 
Si spezza $\hat{T}$ in una matrice che contiene solo $J$ e in una che contiene solo il campo esterno $g$: $\hat{T} = \hat{T}_1\hat{T}_2$, con
\[
	\hat{T}_1 = \begin{pmatrix} e^{\beta J} & e^{-\beta J} \\ e^{-\beta J} & e^{\beta J} \end{pmatrix} = e^{\beta J} \left(\hat{\mathds{1}} + e^{-2 \beta J}\hat{\sigma }^x\right) \hspace{1cm} \hat{T}_2 = \begin{pmatrix} e^{\beta g}& 0 \\ 0 & e^{-\beta g} \end{pmatrix} 
\] 
Dal modello classico, si \`e visto che
\[
	\xi  = - \frac{a}{\log\left[\tanh \left(\beta J\right) \right] } \xrightarrow{\, \ \beta J \to +\infty \ \, } \frac{a}{2} e^{2 \beta J}
\] 
In questo modo, nel limite $\beta \gg 1$, si pu\`o scrivere che
\[
	\hat{T}_1 \simeq  \exp\left[ a \left( -E_0 \hat{\mathds{1}}+ \frac{1}{2\xi }\hat{\sigma }^x \right)  \right] \hspace{1cm} \hat{T}_2 \simeq \exp\Big[ a \tilde{g} \hat{\sigma }^z \Big] 
\] 
con $E_0 = -\beta J / a$ e $\tilde{g} = \beta g / a$.
Si impone che $\hat{T} = \hat{T}_1 \hat{T}_2 \stackrel{!}{=} e^{a \hat{O}}$; si vedr\`a che $\hat{O}$ \`e l'hamiltoniano del modello quantistico su cui si sta mappando l'Ising 1D classico.
Assumendo $a \ll 1$, visto che $\hat{T}_1 $ e $\hat{T}_2$ sono nella forma $e^{a \hat{O}_1}$ e $e^{a \hat{O}_2}$, si pu\`o approssimare la formula di Baker-Campbell-Hausdorff come
\[
	e^{a \hat{O}_1}e^{a \hat{O}_2} \simeq e^{a (\hat{O}_1 + \hat{O}_2) }\implies \hat{O} = \hat{O}_1 + \hat{O}_2 = - E_0 \hat{\mathds{1}} + \frac{1}{2\xi }\hat{\sigma }^x + \tilde{g}\hat{\sigma }^z
\] 
Infine, definendo $\hat{H}_q = - \hat{O}$, si ha $\hat{T} = \hat{T}_1 \hat{T}_2 = e^{-a \hat{H}_q}$.
Allora, la funzione di partizione si scrive come
\[
Z = \operatorname{Tr} e^{-a N\hat{H}_q} = \operatorname{Tr} e^{-\hat{H}_q / T}
\] 
con $T = 1 / aN = 1 / \tau $\footnote{Si pu\`o vedere $\tau $ come un tempo immaginario.}
Per $\Delta  = 1 / \xi $, si vede che il mapping ottenuto \`e 
\[
	H \longrightarrow \hat{H}_q = E_0 \hat{\mathds{1}} - \frac{\Delta }{2} \hat{\sigma }^x - \tilde{g}\hat{\sigma }^z
\] 
dove $\Delta /2$ rappresenta un termine di campo lungo $x$, mentre $\tilde{g}$ un termine di campo lungo $z$.
Lo spazio di Hilbert $\mathcal{H} $ del sistema quantistico \`e ottenuto come span di spin $1 / 2$, cio\`e $\mathcal{H} = \operatorname{Span}  \left\{ \ket{\uparrow} ,\ket{\downarrow}  \right\} $.

Pi\`u in generale, si possono definire mapping analoghi tra sistemi a $D$-dimensioni classiche in sistemi a $(D-1)$-dimensioni quantistiche, dove la dimensione che collassa \`e quella temporale.
