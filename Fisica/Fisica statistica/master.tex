...
\] 
dove $A(b,i)$ rappresenta l'area del circuito individuato da $b$ e $i$, mentre $\chi (b,i)$ \`e una funzione caratteristica che scarta i circuiti da non considerare.
Si ha $A(b,i)\le b^2 / 16$ perch\'e il circuito pi\`u grande costruibile \`e quello quadrato che occupa tutto il reticolo, mentre il numero dei possibili circuiti di lunghezza $b$, $n$, soddisfa
\[
...
si considera che
\[
\langle \chi (b,i) \rangle = \frac{\sum_{S}^{*} e^{-\beta E(\left\{ S \right\} )}}{\sum_{S}^{} e^{-\beta E(S)}}
\] 
dove l'apice $*$ indica che si considerano solo delle configurazioni ammissibili, cio\`e quelle che, al loro interno, contengono solamente spin down e li racchiudono in senso orario.
\begin{center}
	\begin{minipage}
		{.45\columnwidth}
...
\[
\frac{\langle N_- \rangle}{N}\le \frac{4y^2}{12(1-y)^3}\left(1-\frac{3}{4}y + \frac{1}{4}y^2\right)\ , \ \, \text{ con }  \ \ \ y = 9 e^{-4 \beta J}
\] 
Per $T\ll 1\Rightarrow \beta \gg 1$, allora, $y \ll 1$, per cui $\langle N_- \rangle< \frac{N}{2}$.
\begin{boxenv}[]
\centering Riprendere dalla lezione 16
\end{boxenv}
