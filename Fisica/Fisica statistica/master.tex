...
\subsubsection{Argomento di Peierls}
...
\subsubsection{Soluzione esatta del modello di Ising}
...
\begin{split}
	...
	\end{split}\\
	\\
	&\Rightarrow \langle Sn \, S_{n+r}  \rangle \sim e^{-ar / \xi }\ , \ \ \text{ con } \ \, \xi = - \frac{a}{\log\big[ \tanh(\beta J) \big] }
\end{split}
\] 
\begin{center}
	\begin{minipage}
...
% Curva principale: xi = -1/log(tanh(x))
\addplot[blue, thick] {-1/ln(tanh(x))};

% Estensione tratteggiata (asintotica)
\addplot[blue, dashed, domain=1.5:2.5] {-1/ln(tanh(x))};

% Annotazioni
\node at (axis cs:0.2,1.2) {\footnotesize $T \to +\infty$};
\node at (axis cs:1.4,4.4) {\footnotesize $T \to 0$};
...
\end{center}
\begin{boxenv}[]
\centering Riprendere da 1:28:15, lezione 15.
\end{boxenv}
...
\subsection{Integrale nell'argomento di Peierls}\label{peint}
