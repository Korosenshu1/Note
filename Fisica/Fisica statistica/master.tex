...
\[
	\begin{split}
		\hat{H} = &  - t_1 \sum_{j,\sigma }^{}\sum_{k,q}^{} \left\{ e^{-ikx_j}e^{iqx_j}\hat{c}^\dagger _{k\sigma } \hat{d}_{q\sigma } +e^{ikx_j}e^{-iqx_j}\hat{d}^\dagger _{q\sigma } \hat{c}_{k\sigma }  \right\} \\
			  & - t_2 \sum_{j,\sigma }^{} \sum_{k,q}^{} \left\{ e^{-ik(x_j +2a)}e^{iqx_j}\hat{c}^\dagger _{k\sigma } \hat{d}_{q\sigma } +  e^{ik(x_j +2a)}e^{-iqx_j} \hat{d}^\dagger_{q\sigma } \hat{c} _{k\sigma }\right\} 
	\end{split}
\] 
...
\begin{split}
	...
	&\Rightarrow k = \begin{cases}
		-\frac{L}{2} +1 ,\, -\frac{L}{2}+2,\ldots,\, \frac{L}{2}-1,\, \frac{L}{2} & \big(N_f+L \text{ pari}\big)\\
		-\frac{L-1}{2} ,\, -\frac{L-3}{2},\ldots,\, \frac{L-3}{2},\, \frac{L-1}{2} & \big(N_f+L \text{ dispari}\big)
	\end{cases}
\end{split}
\end{equation}
...
Allora\footnote{{\color{red} Controllare un fattore $2$.}}
\[
...
\] 
\subsubsection{Stato fondamentale e stati eccitati}
Lo stato fondamentale si vede essere, analogamente a quanto visto per i bosoni, lo stato di vuoto delle quasi particelle, indicato con $\ket{\Omega } $, che, quindi, soddisfa $\hat{b}_k \ket{\Omega } , \ \forall k$.
\begin{center}
...
Questo si traduce in due hamiltoniani differenti, a seconda del sottospazio in cui ci si trova; l'idea \`e studiare gli stati fondamentali del sistema relativi ai due sottospazio di Hilbert distinti e confrontare i loro valori energetici.
