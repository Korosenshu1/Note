...
\begin{enumerate}[(1).]
	...
	\item $\psi \sim h^{1 / \delta }$ implica che 
		\[
		\psi  = - \frac{\partial f}{\partial h}  = - \lvert t \rvert ^{\frac{d-D_h}{D_t}} \tilde{f}\left(\frac{h}{\lvert t \rvert ^{D_h / D_t}}\right)  \sim h^{1 / \delta } \sim h ^{1 / \delta }
		\] 
...
\] 
Per\`o questo non \`e un caso realistico: con anche la minima interazione, $k_T$ \`e finita.
...
\[
n^{(1)} (\mathbf{r} ,\mathbf{r} ) \equiv n^{(1)}(\mathbf{r} ) = \langle \hat{\psi }^\dagger (\mathbf{r}) \hat{\psi }(\mathbf{r)}  \rangle
\] 
e rappresentano la densit\`a di particelle nel punto $\mathbf{r} $.
I termini fuori diagonale, invece, permettono di ricavare 
\[
n(\mathbf{p} ) = \langle \hat{\psi }^\dagger (\mathbf{p} ) \hat{\psi }(\mathbf{p} ) \rangle = \frac{1}{(2\pi\hbar )^3} \int d\mathbf{R}  \int d\mathbf{s}  \ e^{\frac{i}{\hbar } \mathbf{p} \cdot \mathbf{s} }n^{(1)}\left(\mathbf{R} +\frac{\mathbf{s} }{2}, \mathbf{R} -\frac{\mathbf{s} }{2}\right) 
\] 
dove si \`e esplicitato che
\[
\hat{\psi }(\mathbf{p} ) = \int d\mathbf{r} \ \braket{\mathbf{p} |\mathbf{r} } \hat{\psi }(\mathbf{r} ) = \frac{1}{(2\pi\hbar )^{ 3 / 2}} \int d\mathbf{r}  \ e^{\frac{i}{\hbar }\mathbf{p} \cdot \mathbf{r} }\hat{\psi }(\mathbf{r} )
\] 
e si \`e usato il cambio di variabili in centro di massa e relativa: $\mathbf{R}  = \frac{1}{2}(\mathbf{r} +\mathbf{r} ')$ e $\mathbf{s} = \mathbf{r} -\mathbf{r} '$.
Tramite trasformata di Fourier, si trova
\[
n^{(1)}(\mathbf{s} ) = \frac{1}{V}\int d\mathbf{p} \ n(\mathbf{p} ) e^{-\frac{i}{\hbar }\mathbf{p} \cdot \mathbf{s} }
\] 
In questo modo, si vede che:
\begin{itemize}
	\item in assenza di BEC, la $n(\mathbf{p} )$ \`e una funzione regolare per $\lvert \mathbf{p} \rvert  \to 0$ e $n^{(1)}(\mathbf{s} ) \to 0$, per $\lvert \mathbf{s}  \rvert \to +\infty$;
