...
\end{osservazione}
Sotto approssimazione di $\psi_i $ piccoli, si pu\`o scrivere la $f(\psi )$ sviluppando con Taylor il termine con $\log\left(\cosh \psi\right) $, da cui si ricava
\[
...
\[
	g(\vec{r}_i) = \frac{1}{N} \sum_{\vec{q}}^{} \widetilde{g}(\vec{q}) e^{i \vec{q} \cdot \vec{r}_i}
\] 
...
\[
	\begin{split}
		f(\psi ) &= \frac{1}{2N^2} \sum_{\vec{q},\vec{q}'}^{} \psi _{\vec{q}} \psi _{\vec{q}'}  \sum_{i,j}^{} J^{-1}_{ij} e^{i \vec{q}\cdot \vec{r}_i + i \vec{q}' \cdot \vec{r}_j} \\
...
\[
	\begin{split}
		f (\psi ) &= \frac{1}{2N^2} \sum_{\vec{q},\vec{q}'}^{} \psi _{\vec{q}} \psi _{\vec{q}'}  \sum_{i}^{} J^{-1}_{ij_0} e^{i \vec{q}\cdot (\vec{r}_i -\vec{r}_{j_0} )}\overbracket{\sum_{j}^{} e^{i(\vec{q}+\vec{q}') \cdot \vec{r}_j}}^{= N \delta _{\vec{q}, -\vec{q}'} } \\
			  &= \frac{1}{2N} \sum_{\vec{q}}^{} \psi _{\vec{q}}  \psi _{-\vec{q}}  J^{-1}(q)
	\end{split}
\] 
con 
\[
J^{-1}(q) := \sum_{i}^{} J^{-1}_{ij_0} e^{i \vec{q}\cdot (\vec{r}_i -\vec{r}_{j_0} )}
\] 
Per comprendere il significato di $J^{-1}(q)$, si guarda la trasformata di Fourier di $J$:
\[
	\begin{split}
		J (\vec{q},\vec{q}') &= \sum_{i,j}^{} J_{ij} e^{-i  \vec{q}\cdot \vec{r}_i} e^{- \vec{q}' \cdot \vec{r}_j} = \sum_{i,j}^{} J_{ij} e^{-i \vec{q}\cdot (\vec{r}_i - \vec{r}_j)} e ^{-i (\vec{q} +\vec{q}') \cdot \vec{r}_j}\\
				     &= \sum_{i}^{} J_{ij_0} e^{-i \vec{q}\cdot (\vec{r}_i -\vec{r}_{j_0} )} \delta _{\vec{q},\vec{q}'} = J(\vec{q})
	\end{split}
\] 
dove si sono utilizzate le stesse argomentazioni del calcolo precedente. 
Quindi, si vede che $J(\vec{q})$ coincide proprio con la trasformata di Fourier di $J$.
...
\[
	f(\psi ) \simeq  \frac{1}{2} \sum_{i}^{} \left(\frac{1}{y \beta J}-1\right) \psi _i^2 + {\color{asdf}\frac{\widetilde{c}}{\beta J}\sum_{\langle i,j \rangle}^{} (\psi _i-\psi _j)^2 }+ \frac{1}{12} \sum_{i}^{} \psi _i^4
\] 
con $y = 2d$ \textit{numero di coordinazione}, cio\`e conta quanti primi vicini ci sono, data la dimensione $d$ del reticolo (assumendolo quadrato), dove $\widetilde{c}$ incorpora tutti i fattori non inclusi. 
Inoltre, $\widetilde{c}/ \beta J > 0$, cos\`i come $\frac{1}{12}>0$; al contrario
\[
...
si ha che
\[
\psi (\vec{x}) \sim \chi \lambda 
\] 
Si usa ancora la trasformata di Fourier:
\[
\psi (\vec{q}) = \int d^d \vec{x} \ e^{-i \vec{q}\cdot \vec{x}}\psi (\vec{x}) \hspace{1cm} \psi (\vec{x}) = \frac{1}{(2\pi)^d} \int d^d \vec{q} \ e^{i \vec{q}\cdot \vec{x}}
\] 
...
\[
	...
	\end{split}
\] 
dove $O_n$ \`e una matrice relativa al fattore $S_n$ nell'espressione alla prima riga, che conta gli stati dello spin $n$-esimo; questa si pu\`o scrivere come
\[
...
Si vede, dunque, che per $T\to $, si ha $\xi \to +\infty$; in questo caso, il sistema \`e completamente disordinato perch\'e la correlazione \`e assente e gli spin si orientano in modo del tutto casuale.
...
Si ha $A(b,i)\le b^2 / 16$ perch\'e il circuito pi\`u grande costruibile \`e quello quadrato che occupa tutto il reticolo\footnote{Si nota che $(b / 4)^2$ \`e l'area di un quadrato di perimetro $b$.}, mentre il numero dei possibili circuiti di lunghezza $b$, $n$, soddisfa
\[
...
\[
\sum_{i=1}^{+\infty} i^2 y ^i = \frac{y^2+y}{(1-y)^3}
\] 
risultato ottenuto valutando il seguente risultato per la serie geometria 
\[
	\left(y \frac{d}{dy}\right) ^2 \sum_{i=0}^{+\infty} y^i = (1-y)^{-1}
\] 
Mettendo tutto insieme, si trova che
\[
\frac{\langle N_- \rangle}{N}\le \frac{4y^2}{12(1-y)^3}\left(1-\frac{3}{4}y + \frac{1}{4}y^2\right)
\] 
Per $T\ll 1\Rightarrow \beta \gg 1$, allora, $y \ll 1$, per cui $\langle N_- \rangle< \frac{N}{2}$.
Questo dimostra che, sotto una certa temperatura, il sistema acquisisce una magnetizzazione non-nulla.
\begin{osservazione}
...
\end{osservazione}
\subsection{Universalit\`a e gruppo di rinormalizzazione}
\subsubsection{Leggi di scala}
Gli esponenti trovati, per $t = (T-T_c)/T_c$, nel caso di sistemi magnetici, sono
\[
	\begin{array}
		{l l l}
	...
	\end{array}
\] 
con $\psi $ parametro d'ordine, $\chi $ suscettivit\`a, $\xi $ lunghezza di scala, $h$ campo magnetico, $d$ dimensione fisica dello spazio in cui si trova il sistema.
...
\begin{cases}
	...
	\beta \delta  = \beta  + \gamma &\text{(Widom)}\\
	\gamma = \nu (2-\eta) & \text{(Fisher)}\\
	2 - \alpha  = \nu  d & \text{(Josephson)}
\end{cases}
\] 
...
\begin{osservazione}
Se $\xi $ diverge nel punto critico, allora il sistema diventa \textit{self-similare}: indipendente dalla scala considerata del sistema, questo presenta sempre le stesse caratteristiche, analogamente ai frattali.
\end{osservazione}
\begin{boxenv}[]
...
\begin{enumerate}[(1).]
	\item \textit{Relazione di Josephson}.
		Chiamando $f = F / V$, si ha 
		\[
			c_V = - T \Eval{\frac{\partial ^2 f}{\partial T^2} }{V}{} \sim \lvert t \rvert ^{-\alpha }
		\] 
		dove $c_V = C_V / m$ \`e il calore specifico a volume costante (con $m$ massa).
		Allora
		\[
		f \sim \lvert t \rvert ^{2 - \alpha }
		\] 
		vicino al punto critico.
		Essendo, per l'osservazione (1)	precedente, $f \sim\xi ^{-d}$ e $\xi \sim \lvert t \rvert ^{-\nu }$, si ha $\lvert t \rvert ^{\nu d} \sim \lvert t \rvert ^{2 - \alpha }$, da cui si ottiene la relazione $2 - \alpha = \nu  d$.
...
\end{enumerate}
\begin{boxenv}[]
\centering Riprendere da lezione 17
\end{boxenv}
