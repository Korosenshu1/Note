...
Infine, definendo $f_\alpha (z) = \sum_{n=1}^{+\infty} (-1)^{n+1}\frac{z^n}{n^\alpha }$, si pu\`o scrivere che $\log\mathcal{Z} = \frac{pV}{\kappa _BT}= \frac{V}{\lambda _T^3} f_{5 / 2} (z)$, da cui
\[
	\begin{cases}
		\displaystyle \langle N \rangle= z \frac{\partial }{\partial z} \log \mathcal{Z} \\
		\\
...
\begin{cases}
	...
	\\
	\displaystyle \langle N \rangle = z \frac{\partial }{\partial z}  \log\mathcal{Z} = z \frac{V}{\lambda _T^3} g_{3 / 2} (z) {\color{asdf}\ +\ \frac{1}{V}\frac{z}{1-z}}
\end{cases}
\] 
...
\[
\langle N \rangle= z \frac{\partial }{\partial z}  \log\mathcal{Z}  = - z \frac{\partial }{\partial z} \sum_{\mathbf{p} }^{} \left(1-ze^{-\beta \varepsilon _\mathbf{p} }\right) = \sum_{\mathbf{p} }^{} \frac{1}{z^{-1}e^{\beta \varepsilon _\mathbf{p} }-1}
\] 
\end{osservazione}
\subsection{Limite classico}
...
\] 
con $u_\mathbf{p} (\mathbf{r} ) = \frac{1}{\sqrt{V} }e^{-\frac{i}{\hbar }\mathbf{p} \cdot \mathbf{r} }$ autostati di singola particella, $\mathbf{p} = \frac{2\pi \hbar }{L}$ con $\mathbf{n} \in \mathbb{Z}^3$ (con $V=L^3$); la somma su $\pi$ si riferisce alla somma su tutte le possibili permutazioni possibili di $N$ elementi (in questo caso, permutazioni delle posizioni delle $N$ particelle) e, infine, $\delta _\pi = 1$ sempre per bosoni, mentre \`e pari a $-1$ ogni qualvolta si esegue un numero dispari di scambi di due fermioni.
Secondo questa formula, nel caso dei fermioni per esempio, la funzione d'onda \`e ottenuta con il determinante di Slater come annunciato:
\[
...
\end{osservazione}
\begin{boxenv}[]
\centering Riprendere da lezione 20 45:18
\end{boxenv}
