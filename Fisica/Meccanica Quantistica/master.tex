...
\subsubsection{Hamiltoniano del sistema risultante}

In generale, l'hamiltoniano risultante \`e $\mathcal{H} = \mathcal{H}_1 + \mathcal{H}_2 + \mathcal{H}_{12}$. Se $\ket{\psi } \in \mathcal{H}$, l'evoluzione temporale \`e data da:
\begin{equation}
	\rho (t) = \ket{\psi (t)} \bra{\psi (t)} = e^{- \frac{i}{\hbar } \hat{H}t} \ket{\psi (0)} \bra{\psi (0)} e^{ \frac{i}{\hbar }\hat{H}t} = e^{-\frac{i}{\hbar }\hat{H}t} \rho (0) e^{\frac{i}{\hbar }\hat{H}t} 
\end{equation}
L'evoluzione del singolo sottosistema si immagina indipendente da interazione, quindi si fanno evolvere separatamente come $\rho ^{(1)} (t) = e^{-\frac{i}{\hbar }\hat{H}_1t} \rho ^{(1)} (t=0) e^{\frac{i}{\hbar }\hat{H}_1t} $. 

Se $\hat{O}$ \`e un osservabile agente su $\mathcal{H}$, il suo valore di aspettazione su $1$ \`e dato da $\operatorname{tr} ^{(1)} \rho ^{(1)} \hat{O}$.
