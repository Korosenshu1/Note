\documentclass[10pt, a4paper]{scrartcl} % Packages
%\usepackage{stix}
...
\] 
Significa che per ogni rotazione, $j$ \`e fissato. Si calcolano elementi di $D^j_{m,m'} $; si pu\`o dimostrare che\footnote{La seconda uguaglianza non \`e dimostrata, \`e da prendere per buona.}:
\[
...
\[
	...
	\end{split} \iff[\hat{H},\hat{S}_3] = 0
\] 
Andando a calcolare $\braket{\epsilon _i|\hat{S}_3|\epsilon _i} $ con gli $\ket{\epsilon _i} $ definiti in eq. $\ref{auten1}$, si trova $\epsilon _1 = 1$, $ \epsilon _2 = -1$, $\epsilon _3 = 0 $ e $\epsilon _4 = 0 $, quindi ha risolto la degenerazione osservata prima.
