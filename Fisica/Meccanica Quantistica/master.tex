...
Valgono le relazioni di commutazione $[\hat{J}_a, \hat{J}_b] = i\hbar \varepsilon _{abc} \hat{J}_c$.
\begin{proof}
Si considera l'operatore sullo spazio delle funzioni d'onda
\[
\hat{U}_R = \hat{U}_{R_1} \hat{U}_{R_2} \hat{U}_{R_3} \hat{U}_{R_4} = e^{ \frac{i}{\hbar }\varepsilon \hat{J}_a} e^{ \frac{i}{\hbar }\varepsilon \hat{J}_b} e^{- \frac{i}{\hbar }\varepsilon \hat{J}_a} e^{- \frac{i}{\hbar }\varepsilon \hat{J}_b} 
\] 
con $\varepsilon $ parametro della rotazione. Si assume $\varepsilon \simeq 0$ e si sviluppano esponenziali al secondo ordine (primo ordine non-banale), cos\`i si ha:
\[
	\hat{U}_R \simeq \operatorname{Id} + \varepsilon ^2 [\hat{J}_a,\hat{J}_b]
\] 

\end{proof}
\begin{center}
	\color{red} Da correggere e terminare
\end{center}
