...
Nel 1918, Schr\"odinger ipotizza che il termine cosmologico possa essere interpretato come il tensore energia-impulso di un fluido a pressione $P < 0$; in questo modo, la costante cosmologica non \`e pi\`u una forzatura matematica, ma acquisisce un significato ben preciso e si possono riscrivere le equazioni di Einstein come
\[
	G_{\mu  \nu  }  = 8 \pi G T_{\mu  \nu  }  \hspace{1cm} T_{\mu \nu }  = T_{\mu \nu } ^\text{materia} + T_{\mu \nu }^\text{(P>0)}
\] 
\subsubsection{Evidenze dell'espansione cosmologica}
Tra il 1912 e il 1917, molte misurazioni delle velocit\`a delle galassie erano state ottenute da Slipher, tramite analisi spettrale delle righe di emissione.
Si osserv\`o, inoltre, che la maggior parte delle galassie mostrava un red-shift nello spettro, mentre alcune, come M31, un blue-shift.
...
\begin{equation}
	v = H_0d
\end{equation}
con $H_0 \approx 70 \text{ km s}^{-1} \text{ Mpc}^{-1}$ costante di Hubble.
